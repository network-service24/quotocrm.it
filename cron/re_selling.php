...
require $_SERVER['DOCUMENT_ROOT'].'/class/PHPMailer/PHPMailerAutoload.php';
        ...
        $sel = "SELECT hospitality_guest.*, hospitality_giorni_reselling.numero_giorni 
                FROM hospitality_guest 
                INNER JOIN hospitality_giorni_reselling ON hospitality_giorni_reselling.idsito = hospitality_guest.idsito
                WHERE 1 = 1
                ...
                AND hospitality_giorni_reselling.abilita = 1 
                AND hospitality_guest.DataArrivo =  (CURRENT_DATE + INTERVAL hospitality_giorni_reselling.numero_giorni  DAY)";
        $qy  = $dbMysqli->query($sel);
        $tot = sizeof($qy);
        if($tot > 0){                          
            
            $myfile = fopen($path_cron.'log/log_reselling.txt', 'w'); 
            ...
            foreach($qy as $key => $dati){
                ...
                if($dati['DataArrivo'] == $data){
                            ...
                            switch($TipoVacanza){
                                case"Family":
                                    $oggetto   = $etichetta['OGGETTO_RESELLING_FAMILY'];
                                    $testomail = $etichetta['TESTOMAIL_RESELLING_FAMILY'];
                                break;
                                case"Business":
                                case"Fiera":
                                    $oggetto   = $etichetta['OGGETTO_RESELLING_BUSINESS'];
                                    $testomail = $etichetta['TESTOMAIL_RESELLING_BUSINESS'];
                                break;
                                case"Benessere":
                                    $oggetto   = $etichetta['OGGETTO_RESELLING_BENESSERE'];
                                    $testomail = $etichetta['TESTOMAIL_RESELLING_BENESSERE'];
                                break;
                                case"Bike":
                                case"Sport":
                                    $oggetto   = $etichetta['OGGETTO_RESELLING_SPORT'];
                                    $testomail = $etichetta['TESTOMAIL_RESELLING_SPORT'];
                                break; 
                                case"Divertimento":
                                case"Romantico":
                                case"Culturale":
                                case"Enogastronomico":
                                    $oggetto   = $etichetta['OGGETTO_RESELLING'];
                                    $testomail = $etichetta['TESTOMAIL_RESELLING'];
                                break; 
                                default:
                                    $oggetto   = $etichetta['OGGETTO_RESELLING'];
                                    $testomail = $etichetta['TESTOMAIL_RESELLING'];
                                ...
                                $mail->isHTML(true);
                                $mail->Subject = str_replace("[cliente]",$Nome.' '.$Cognome,strip_tags($oggetto)).' | '.ucfirst($rows['nome']);

                                include $path_cron.'email_template/reselling_mail.php';
                                ...
                                $dbMysqli->query("INSERT  INTO hospitality_traccia_email_cron (IdRichiesta,Idsito,DataAzione,TipoReInvio) VALUES ('".$IdRichiesta."','".$IdSito."','".date('Y-m-d H:i:s')."','UpSelling')");
