...
function random_file_name( $real_file_name ) {
 ...
 if(isset($_FILES['file_logo']['name'])){
    ...
    $filename = random_file_name($_FILES['file_logo']['name']);
 

    $location = BASE_PATH_SITO."uploads/loghi_superuser/".$filename;
    ...
    if(in_array(strtolower($imageFileType), $valid_extensions)) {
       ...
       if(move_uploaded_file($_FILES['file_logo']['tmp_name'],$location)){
          $response = $location;
       }
    }
    echo $filename;
