...
/**
 * CRM and CMS
 * @author Marcello Visigalli <a marcello.visigalli@gevento.com >
 * @version 3.0
 * @name SuiteWeb
...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$select  = "SELECT 
					hospitality_eventi_lang.*	
				FROM 
                    hospitality_eventi_lang 
				WHERE 
                    hospitality_eventi_lang.idsito = ".$_REQUEST['idsito']." 
                AND
                    hospitality_eventi_lang.Id_eventi = ".$_REQUEST['id']." ";
	...
	foreach($rec as $key => $row){
                        ...
                        $modale = '<div class="modal fade" id="ModaleUpdateTestoEvento'.$row['Id'].'" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content" style="width:100%">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica contenuti per l\'Evento: '.urldecode($_REQUEST['evento']).'</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body  p-l-50 p-r-50">

                                                                    <form method="POST" id="form_up_evento'.$row['Id'].'" name="form_up_evento" action="'.BASE_URL_SITO.'ajax/generiche/modifica_testo_evento.php">
                                                                        ...
                                                                        <div class="form-group">  
                                                                            <div class="row">
                                                                                <div class="col-md-12 text-center">
                                                                                    ...
                                                                                    <input type="hidden" name="Id_eventi"  value="'.$row['Id_eventi'].'">
                                                                                    <input type="hidden" name="evento"  value="'.urlencode($_REQUEST['evento']).'">
                                                                                    <input type="hidden" name="idsito"  value="'.$row['idsito'].'">
                                                                                    <input type="hidden" name="action"  value="mod_evento">
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['Id'].'").on("click",function(){
                                                            $("#ModaleUpdateTestoEvento'.$row['Id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                               
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="#" id="delete_testo_evento'.$row['Id'].'"><i class="fa fa-times text-red"></i> Elimina</a>
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#delete_testo_evento'.$row['Id'].'").on("click",function(){
                                                            if (window.confirm("ATTENZIONE: Sicuro di voler eliminare questo Record?")){
                                                                $.ajax({
                                                                    url: "'.BASE_URL_SITO.'ajax/generiche/delete_testo_evento.php",
                                                                    type: "POST",
                                                                    data: "action=del_t_evento&idsito='.$row['idsito'].'&Id='.$row['Id'].'",
                                                                    dataType: "html",
                                                                    success: function(data) {
                                                                        $("#add_eventi").DataTable().ajax.reload();    
                                                                    }
                                                                });
                                                                return false;
                                                            }
