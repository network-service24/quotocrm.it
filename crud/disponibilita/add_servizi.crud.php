...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$select  = "SELECT 
					hospitality_servizi_camere_lingua.*	
				FROM 
					hospitality_servizi_camere_lingua 
				WHERE 
					hospitality_servizi_camere_lingua.idsito = ".$_REQUEST['idsito']." 
                AND
                    hospitality_servizi_camere_lingua.servizi_id = ".$_REQUEST['id']." ";
                                                                    ...
                                                                    <script>
                                                                        $(document).ready(function() {
                                                                            $("#form_up_servizi'.$row['Id'].'").submit(function () {   
                                                                                ...
                                                                                $.ajax({
                                                                                    url: "'.BASE_URL_SITO.'ajax/disponibilita/modifica_testo_servizio.php",
                                                                                    type: "POST",
                                                                                    data: "action=mod_servizi&idsito='.$row['idsito'].'&Id="+Id+"&lingua="+lingua+"&Servizio="+Servizio+"",
                                                                                    dataType: "html",
                                                    ...
                                                    $(document).ready(function(){ 
                                                        $("#delete_testo_servizi'.$row['Id'].'").on("click",function(){
                                                            if (window.confirm("ATTENZIONE: Sicuro di voler eliminare questo Record?")){
                                                                $.ajax({
                                                                    url: "'.BASE_URL_SITO.'ajax/disponibilita/delete_testo_servizio.php",
                                                                    type: "POST",
