...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$select  = "SELECT 
					hospitality_camere_testo.*	
				FROM 
					hospitality_camere_testo 
				WHERE 
					hospitality_camere_testo.idsito = ".$_REQUEST['idsito']." 
                AND
                    hospitality_camere_testo.camere_id = ".$_REQUEST['id']." ";
	...
	foreach($rec as $key => $row){
                        ...
                        $modale = '<div class="modal fade" id="ModaleUpdateTestoCamere'.$row['Id'].'" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content" style="width:100%">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica contenuti per la Camera '.urldecode($_REQUEST['camera']).'</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body  p-l-50 p-r-50">

                                                                    <form method="POST" id="form_up_camere'.$row['Id'].'" name="form_up_camere" action="'.BASE_URL_SITO.'ajax/disponibilita/modifica_testo_camera.php">
                                                                        ...
                                                                        <div class="form-group">
                                                                         ...
                                                                         <div class="form-group">
                                                                            <div class="row">
                                                                                ...
                                                                                <div class="col-md-7">
                                                                                    <input type="text" name="Camera" id="Camera'.$row['Id'].'" value="'.$row['Camera'].'" class="form-control" required>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        <div class="form-group">
                                                                        ...
                                                                        <div class="form-group">  
                                                                            <div class="row">
                                                                                <div class="col-md-12 text-center">
                                                                                    ...
                                                                                    <input type="hidden" name="camere_id"  id="camere_id'.$row['Id'].'" value="'.$row['camere_id'].'">
                                                                                    <input type="hidden" name="idsito"  value="'.$row['idsito'].'">
                                                                                    <input type="hidden" name="action"  value="mod_camera">
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['Id'].'").on("click",function(){
                                                            $("#ModaleUpdateTestoCamere'.$row['Id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                               
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="#" id="delete_testo_camere'.$row['Id'].'"><i class="fa fa-times text-red"></i> Elimina</a>
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#delete_testo_camere'.$row['Id'].'").on("click",function(){
                                                            if (window.confirm("ATTENZIONE: Sicuro di voler eliminare questo Record?")){
                                                                $.ajax({
                                                                    url: "'.BASE_URL_SITO.'ajax/disponibilita/delete_testo_camere.php",
                                                                    type: "POST",
                                                                    data: "action=del_t_camere&idsito='.$row['idsito'].'&Id='.$row['Id'].'",
                                                                    dataType: "html",
                                                                    success: function(data) {
                                                                        $("#add_camere").DataTable().ajax.reload();    
                                                                    }
                                                                });
                                                                return false;
                                                            }
