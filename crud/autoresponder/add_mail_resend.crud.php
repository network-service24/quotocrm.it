                                            ...
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content" style="width:100%">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica contenuti per la Mail di Recall Conferme</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body  p-l-50 p-r-50">

                                                                    <form method="POST" id="form_up_mail'.$row['id'].'" name="form_up_mail" method="POST"  action="'.BASE_URL_SITO.'ajax/autoresponder/modifica_testo_mail_resend.php">
                                                    ...
                                                    $(document).ready(function(){ 
                                                        $("#delete_testo_mail'.$row['id'].'").on("click",function(){
                                                            if (window.confirm("ATTENZIONE: Sicuro di voler eliminare questo Record?")){
                                                                $.ajax({
                                                                    url: "'.BASE_URL_SITO.'ajax/autoresponder/delete_testo_mail_resend.php",
                                                                    type: "POST",
                                                                    data: "action=del_t_mail&idsito='.$row['idsito'].'&id='.$row['id'].'",
                                                                    dataType: "html",
                                                                    success: function(data) {
                                                                        $("#add_mail_resend").DataTable().ajax.reload();    
                                                                    }
                                                                });
                                                                return false;
                                                            }
