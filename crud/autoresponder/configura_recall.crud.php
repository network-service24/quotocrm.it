...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$select2  = "SELECT 
                ...
                AND
                    hospitality_dizionario.etichetta = 'TESTOMAIL_RECALL_PREVENTIVI' OR hospitality_dizionario.etichetta = 'OGGETTO_RECALL_PREVENTIVI'
                AND
    ...
    foreach($res as $ky => $rows){
    	...
    	$select  = "SELECT 
					hospitality_giorni_recall_preventivi.*	
				FROM 
					hospitality_giorni_recall_preventivi 
				WHERE 
					hospitality_giorni_recall_preventivi.idsito = ".$_REQUEST['idsito']." ";

	$rec = $dbMysqli->query($select);

	foreach($rec as $key => $row){

                           $modale .=' <div class="modal fade" id="ModaleUpdateRecall'.$row['id'].'" tabindex="-1" role="dialog" aria-labelledby="ModaleLabel" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica  Setting Recall Preventivi</h4>
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-8">
                                                            <form method="POST" id="form_mod_recall'.$row['id'].'" name="form_mod_recall">
                                                            ...
                                                            <script>
                                                                $(document).ready(function() {
                                                                    ...
                                                                    $("#form_mod_recall'.$row['id'].'").submit(function () {   
                                                                        var  numero_giorni  = $("#numero_giorni'.$row['id'].' option:selected").val(); 
                                                                        var  id             = $("#id'.$row['id'].'").val(); 
                                                                        var  abilita        = $("#abilita'.$row['id'].'").val(); 
                                                                        $.ajax({
                                                                            url: "'.BASE_URL_SITO.'ajax/autoresponder/modifica_recall.php",
                                                                            type: "POST",
                                                                            data: "action=mod_recall&id="+id+"&idsito='.$row['idsito'].'&numero_giorni="+numero_giorni+"&abilita="+abilita+"",
                                                                            dataType: "html",
                                                                            success: function(data) {
                                                                                $("#ModaleUpdateRecall'.$row['id'].'").modal("hide");
                                                                                $("#recall_preventivi").DataTable().ajax.reload();    
                                                                            }
                                                                        });
											    ...
											    <script>
                                                    $(document).ready(function(){ 
                                                        $("#check_abilita'.$row['id'].'").on("click",function(){
                                                            $.ajax({
                                                                url: "'.BASE_URL_SITO.'ajax/autoresponder/switch_recall.php",
                                                                type: "POST",
                                                                data: "action=switch_recall&idsito='.$row['idsito'].'&id='.$row['id'].'&abilita=1",
                                                                dataType: "html",
                                                                success: function(data) {
                                                                    $("#recall_preventivi").DataTable().ajax.reload();    
                                                                }
                                                        ...
                                                        $("#check_disabilita'.$row['id'].'").on("click",function(){
                                                            $.ajax({
                                                                url: "'.BASE_URL_SITO.'ajax/autoresponder/switch_recall.php",
                                                                type: "POST",
                                                                data: "action=switch_recall&idsito='.$row['idsito'].'&id='.$row['id'].'&abilita=0",
                                                                dataType: "html",
                                                                success: function(data) {
                                                                    $("#recall_preventivi").DataTable().ajax.reload();    
                                                                }
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['id'].'").on("click",function(){
                                                            $("#ModaleUpdateRecall'.$row['id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                            
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="'.BASE_URL_SITO.'autoresponder-configura_oggetto_recall/'.$id_dizionario[0].'/"><i class="fa fa-comment-o text-green"></i> Gestione testi Oggetto </a>  
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="'.BASE_URL_SITO.'autoresponder-configura_mail_recall/'.$id_dizionario[1].'/"><i class="fa fa-comment-o text-green"></i> Gestione testi E-Mail </a>                                                                                                   
                                            </div>
										</div>';                                              
							...
							$data[] = array(
                                        ...
                                        "oggetto"            =>    $fun->ControlloTestiInseritiOggettoRecall($id_dizionario[0],$row['idsito']),
                                        "testo"              =>    $fun->ControlloTestiInseritiMailRecall($id_dizionario[1],$row['idsito']),
