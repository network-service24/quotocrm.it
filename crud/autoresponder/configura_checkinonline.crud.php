...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$select2  = "SELECT 
                ...
                AND
                    hospitality_dizionario.etichetta = 'TESTOMAIL_CHECKIN' OR hospitality_dizionario.etichetta = 'OGGETTO_CHECKIN'
                AND
    ...
    foreach($res as $ky => $rows){
    	...
    	$select  = "SELECT 
					hospitality_giorni_checkinonline.*	
				FROM 
					hospitality_giorni_checkinonline 
				WHERE 
					hospitality_giorni_checkinonline.idsito = ".$_REQUEST['idsito']." ";

	$rec = $dbMysqli->query($select);

	foreach($rec as $key => $row){

                           $modale .=' <div class="modal fade" id="ModaleUpdateCheckinonline'.$row['id'].'" tabindex="-1" role="dialog" aria-labelledby="ModaleLabel" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica  Setting Check-in online </h4>
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-8">
                                                            <form method="POST" id="form_mod_checkinonline'.$row['id'].'" name="form_mod_checkinonline">
                                                                <div class="form-group">  
                                                                    <div class="row">
                                                                        <div class="col-md-3">
                                                                            <label>Numero giorni PRIMA della Scadenza</label>
                                                                        </div>
                                                                        <div class="col-md-7">                                            	                                                     
                                                            ...
                                                            <script>
                                                                $(document).ready(function() {
                                                                    ...
                                                                    $("#form_mod_checkinonline'.$row['id'].'").submit(function () {   
                                                                        var  numero_giorni  = $("#numero_giorni'.$row['id'].' option:selected").val(); 
                                                                        var  id             = $("#id'.$row['id'].'").val(); 
                                                                        var  abilita        = $("#abilita'.$row['id'].'").val(); 
                                                                        $.ajax({
                                                                            url: "'.BASE_URL_SITO.'ajax/autoresponder/modifica_checkinonline.php",
                                                                            type: "POST",
                                                                            data: "action=mod_checkinonline&id="+id+"&idsito='.$row['idsito'].'&numero_giorni="+numero_giorni+"&abilita="+abilita+"",
                                                                            dataType: "html",
                                                                            success: function(data) {
                                                                                $("#ModaleUpdateCheckinonline'.$row['id'].'").modal("hide");
                                                                                $("#checkinonline").DataTable().ajax.reload();    
                                                                            }
                                                                        });
											    ...
											    <script>
                                                    $(document).ready(function(){ 
                                                        $("#check_abilita'.$row['id'].'").on("click",function(){
                                                            $.ajax({
                                                                url: "'.BASE_URL_SITO.'ajax/autoresponder/switch_checkinonline.php",
                                                                type: "POST",
                                                                data: "action=switch_checkinonline&idsito='.$row['idsito'].'&id='.$row['id'].'&abilita=1",
                                                                dataType: "html",
                                                                success: function(data) {
                                                                    $("#checkinonline").DataTable().ajax.reload();    
                                                                }
                                                        ...
                                                        $("#check_disabilita'.$row['id'].'").on("click",function(){
                                                            $.ajax({
                                                                url: "'.BASE_URL_SITO.'ajax/autoresponder/switch_checkinonline.php",
                                                                type: "POST",
                                                                data: "action=switch_checkinonline&idsito='.$row['idsito'].'&id='.$row['id'].'&abilita=0",
                                                                dataType: "html",
                                                                success: function(data) {
                                                                    $("#checkinonline").DataTable().ajax.reload();    
                                                                }
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['id'].'").on("click",function(){
                                                            $("#ModaleUpdateCheckinonline'.$row['id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                            
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="'.BASE_URL_SITO.'autoresponder-configura_oggetto_checkin/'.$id_dizionario[0].'/"><i class="fa fa-comment-o text-green"></i> Gestione testi Oggetto </a>  
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="'.BASE_URL_SITO.'autoresponder-configura_mail_checkin/'.$id_dizionario[1].'/"><i class="fa fa-comment-o text-green"></i> Gestione testi E-Mail </a>                                                                                                   
                                            </div>
										</div>';                                              
							...
							$data[] = array(
                                        ...
                                        "oggetto"            =>    $fun->ControlloTestiInseritiOggettoCheckinonline($id_dizionario[0],$row['idsito']),
                                        "testo"              =>    $fun->ControlloTestiInseritiMailCheckinonline($id_dizionario[1],$row['idsito']),
