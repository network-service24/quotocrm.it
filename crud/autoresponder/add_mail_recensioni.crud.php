...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	foreach($rec as $key => $row){
                        ...
                        $modale = '<div class="modal fade" id="ModaleUpdateTestoMail'.$row['id'].'" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content" style="width:100%">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica contenuti per la Mail del modulo Recensioni</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body  p-l-50 p-r-50">

                                                                    <form method="POST" id="form_up_mail'.$row['id'].'" name="form_up_mail" method="POST"  action="'.BASE_URL_SITO.'ajax/autoresponder/modifica_testo_mail_recensioni.php">
                                                                        ...
                                                                        <div class="form-group">  
                                                                            <div class="row">
                                                                                <div class="col-md-12 text-center">
                                                                                    ...
                                                                                    <input type="hidden" name="action"  value="mod_mail">
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['id'].'").on("click",function(){
                                                            $("#ModaleUpdateTestoMail'.$row['id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                               
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="#" id="delete_testo_mail'.$row['id'].'"><i class="fa fa-times text-red"></i> Elimina</a>
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#delete_testo_mail'.$row['id'].'").on("click",function(){
                                                            if (window.confirm("ATTENZIONE: Sicuro di voler eliminare questo Record?")){
                                                                $.ajax({
                                                                    url: "'.BASE_URL_SITO.'ajax/autoresponder/delete_testo_mail_recensioni.php",
                                                                    type: "POST",
                                                                    data: "action=del_t_mail&idsito='.$row['idsito'].'&id='.$row['id'].'",
                                                                    dataType: "html",
                                                                    success: function(data) {
                                                                        $("#add_mail_recensioni").DataTable().ajax.reload();    
                                                                    }
                                                                });
                                                                return false;
                                                            }
