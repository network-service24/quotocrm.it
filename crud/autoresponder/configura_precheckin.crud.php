...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$data        = array();
    	...
    	$select  = "SELECT 
					hospitality_giorni_precheckin.*	
				FROM 
					hospitality_giorni_precheckin 
				WHERE 
					hospitality_giorni_precheckin.idsito = ".$_REQUEST['idsito']." ";

	$rec = $dbMysqli->query($select);

	foreach($rec as $key => $row){

                           $modale .=' <div class="modal fade" id="ModaleUpdatePrecheckin'.$row['id'].'" tabindex="-1" role="dialog" aria-labelledby="ModaleLabel" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica Setting Info Utili </h4>
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-8">
                                                            <form method="POST" id="form_mod_precheckin'.$row['id'].'" name="form_mod_precheckin">
                                                                <div class="form-group">  
                                                                    <div class="row">
                                                                        <div class="col-md-3">
                                                                            <label>Numero giorni PRIMA del Check-in</label>
                                                                        </div>
                                                                        <div class="col-md-7">                                            	                                                     
                                                            ...
                                                            <script>
                                                                $(document).ready(function() {
                                                                    ...
                                                                    $("#form_mod_precheckin'.$row['id'].'").submit(function () {   
                                                                        var  numero_giorni  = $("#numero_giorni'.$row['id'].' option:selected").val(); 
                                                                        var  id             = $("#id'.$row['id'].'").val(); 
                                                                        var  abilita        = $("#abilita'.$row['id'].'").val(); 
                                                                        $.ajax({
                                                                            url: "'.BASE_URL_SITO.'ajax/autoresponder/modifica_precheckin.php",
                                                                            type: "POST",
                                                                            data: "action=mod_precheckin&id="+id+"&idsito='.$row['idsito'].'&numero_giorni="+numero_giorni+"&abilita="+abilita+"",
                                                                            dataType: "html",
                                                                            success: function(data) {
                                                                                $("#ModaleUpdatePrecheckin'.$row['id'].'").modal("hide");
                                                                                $("#precheckin").DataTable().ajax.reload();    
                                                                            }
                                                                        });
											    ...
											    <script>
                                                    $(document).ready(function(){ 
                                                        $("#check_abilita'.$row['id'].'").on("click",function(){
                                                            $.ajax({
                                                                url: "'.BASE_URL_SITO.'ajax/autoresponder/switch_precheckin.php",
                                                                type: "POST",
                                                                data: "action=switch_precheckin&idsito='.$row['idsito'].'&id='.$row['id'].'&abilita=1",
                                                                dataType: "html",
                                                                success: function(data) {
                                                                    $("#precheckin").DataTable().ajax.reload();    
                                                                }
                                                        ...
                                                        $("#check_disabilita'.$row['id'].'").on("click",function(){
                                                            $.ajax({
                                                                url: "'.BASE_URL_SITO.'ajax/autoresponder/switch_precheckin.php",
                                                                type: "POST",
                                                                data: "action=switch_precheckin&idsito='.$row['idsito'].'&id='.$row['id'].'&abilita=0",
                                                                dataType: "html",
                                                                success: function(data) {
                                                                    $("#precheckin").DataTable().ajax.reload();    
                                                                }
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['id'].'").on("click",function(){
                                                            $("#ModaleUpdatePrecheckin'.$row['id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                            
