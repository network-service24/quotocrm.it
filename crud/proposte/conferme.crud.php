	...
	foreach($rec as $key => $row){
					...
					if($check_pms5==1 || $check_pmsB==1 || $check_pmsE==1){ 
							$data[] = array(
								...
								"fonte"          => $bg_fonte,
								"tipo"           => $bg_tipo,
								"data"           => $fun->gira_data($row['DataRichiesta']),
								...
								"scadenza"       => ($row['DataScadenza'] < date('Y-m-d')?'<span class="text-gray">'.$fun->gira_data($row['DataScadenza']).'</span>':$fun->gira_data($row['DataScadenza']).'<br>'.$giorno_scad),
								"check"          => $check,
								"pms"            => $buttonPMS,
