...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	foreach($rec as $key => $row){
							...
							if($check_pms5==1){
								$buttonPMS = $fun->buottonPms5Stelle($row['Id'],$row['idsito'],'conferme');
							}
							if($check_pmsE==1){
								$buttonPMS = $fun->buottonPmsEricsoft($row['Id'],$row['idsito'],'conferme');
							}
							if($check_pmsB==1){
								$buttonPMS = $fun->buottonPmsBedzzle($row['Id'],$row['idsito'],'conferme');
							}
							...
							$func_chat_column   = '	<script>
														$(function(){
															func_chat_column('.$row['NumeroPrenotazione'].',"'.($row['DataInvio']==''?'null':$row['DataInvio']).'","'.($row['DataScadenza']==''?'null':$row['DataScadenza']).'","'.($row['DataChiuso']==''?'null':$row['DataChiuso']).'","'.($row['DataArrivo']==''?'null':$row['DataArrivo']).'",'.$row['idsito'].','.$row['Id'].',"conferme");
														})													
													</script>
													<div id="func_chat_column_pre'.$row['Id'].'"></div>
													<div id="func_chat_column'.$row['Id'].'"></div>'; 

							// picon check per pagamenti, recall e voucher
							$check = '<ul class="check"><li>'.$fun->utenti_online($row['idsito'],$row['Id']).'</li><li>'.$fun->func_cc($row['Id'],$row['idsito']).'</li><li>'.$fun->check_voucher_recupero_send($row['Id'],$row['idsito']).'</li><li>'.$fun->re_email_send($row['Id'],$row['idsito']).'</li></ul>';
					...
					if($check_pms5==1 || $check_pmsB==1 || $check_pmsE==1){ 
							$data[] = array(
								...
								"scadenza"       => '<span class="ordinamento">'.$row['DataScadenza'].'</span>'.($row['DataScadenza'] < date('Y-m-d')?'<span class="text-gray">'.$fun->gira_data($row['DataScadenza']).'</span>':$fun->gira_data($row['DataScadenza']).'<br>'.$giorno_scad),
								"check"          => $check,
								"pms"            => $buttonPMS,
