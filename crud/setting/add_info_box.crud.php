...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$select  = "SELECT 
				...
				FROM 
					hospitality_info_box_lang 
				WHERE 
					hospitality_info_box_lang.idsito = ".$_REQUEST['idsito']." 
                AND
                    hospitality_info_box_lang.Id_info_box = ".$_REQUEST['id']." ";
	...
	foreach($rec as $key => $row){
                        ...
                        $modale = '<div class="modal fade" id="ModaleUpdateTestoInfoBox'.$row['Id'].'" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content" style="width:100%">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica contenuti email per Info box</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body  p-l-50 p-r-50">

                                                                    <form method="POST" id="form_up_info_box'.$row['Id'].'" name="form_up_info_box" method="POST" action="'.BASE_URL_SITO.'ajax/generici/modifica_testo_info_box.php">
                                                                        ...
                                                                        <div class="form-group">  
                                                                            <div class="row">
                                                                                <div class="col-md-12 text-center">
                                                                                    ...
                                                                                    <input type="hidden" name="Id_info_box"  value="'.$row['Id_info_box'].'">
                                                                                    <input type="hidden" name="idsito"  value="'.$row['idsito'].'">
                                                                                    <input type="hidden" name="action"  value="mod_info_box">
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['Id'].'").on("click",function(){
                                                            $("#ModaleUpdateTestoInfoBox'.$row['Id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                               
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item waves-effect waves-light" href="#" id="delete_testo_info_box'.$row['Id'].'"><i class="fa fa-times text-red"></i> Elimina</a>
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#delete_testo_info_box'.$row['Id'].'").on("click",function(){
                                                            if (window.confirm("ATTENZIONE: Sicuro di voler eliminare questo Record?")){
                                                                $.ajax({
                                                                    url: "'.BASE_URL_SITO.'ajax/generici/delete_testo_info_box.php",
                                                                    type: "POST",
                                                                    data: "action=del_t_ib&idsito='.$row['idsito'].'&id='.$row['Id'].'",
                                                                    dataType: "html",
                                                                    success: function(data) {
                                                                        $("#add_info_box").DataTable().ajax.reload();    
                                                                    }
                                                                });
                                                                return false;
                                                            }
