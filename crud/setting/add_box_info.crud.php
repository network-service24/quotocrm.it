...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
	...
	$select  = "SELECT 
				...
				FROM 
					hospitality_boxinfo_checkin_lang 
				WHERE 
					hospitality_boxinfo_checkin_lang.idsito = ".$_REQUEST['idsito']." 
                AND
                    hospitality_boxinfo_checkin_lang.Id_infohotel = ".$_REQUEST['id']." ";
	...
	foreach($rec as $key => $row){
                        ...
                        $modale = '<div class="modal fade" id="ModaleUpdateTestoBoxInfo'.$row['Id'].'" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content" style="width:100%">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Modifica contenuti per Box Info modulo Check-In Online</h4>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                                                    </div>
                                                    <div class="modal-body  p-l-50 p-r-50">

                                                                    <form method="POST" id="form_up_box_info'.$row['Id'].'" name="form_up_box_info" method="POST" action="'.BASE_URL_SITO.'ajax/generici/modifica_testo_box_info.php">
                                                                        ...
                                                                        <div class="form-group">  
                                                                            <div class="row">
                                                                                <div class="col-md-12 text-center">
                                                                                    ...
                                                                                    <input type="hidden" name="Id_infohotel"  value="'.$row['Id_infohotel'].'">
                                                                                    <input type="hidden" name="idsito"  value="'.$row['idsito'].'">
                                                                                    <input type="hidden" name="action"  value="mod_box_info">
                                                ...
                                                <script>
                                                    $(document).ready(function(){ 
                                                        $("#modifica'.$row['Id'].'").on("click",function(){
                                                            $("#ModaleUpdateTestoBoxInfo'.$row['Id'].'").modal("show"); 
                                                        });
                                                    });
                                                </script>                                               
                                            </div>
