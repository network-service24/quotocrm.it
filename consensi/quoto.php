...
include($_SERVER['DOCUMENT_ROOT']."/include/function.inc.php");
        ...
        $valori       = explode('#',$val);
        $id_richiesta = $valori[0];
        $email_utente = $valori[1];    
        /**
         * QUERY PER ESTRAPOLARE LA RIHCIESTA INTERESSATA
         */
        $query ='SELECT hospitality_guest.* FROM hospitality_guest WHERE Id = "'.$id_richiesta.'"';
        $res   = $dbMysqli->query($query);
        $r     = $res[0];

        $lingua          = $r['Lingua'];
        $Ip              = $r['Ip'];   
        $modulo_inviato  = 'Conferma Nr.'.$r['NumeroPrenotazione'].' del '.gira_data($r['DataRichiesta']);
        $browser         = $r['Agent'];     
        ...
        $text['data']['it']         = 'Data';  
        $text['modulo']['it']       = 'Modulo inviato';  
        $text['accettati']['it']    = 'Consensi accettati';  
        $text['info_attuale']['it'] = 'Leggi informativa attuale su QUOTO di '.$rw['nome']; 
        $text['esercita']['it']     = 'Esercita i tuoi diritti'; 
        $text['cancellate']['it']   = 'Cancellate questa mia iscrizione o richiesta archiviata dal software di Email Marketing che utilizzate <small><em>(diritto di cancellazione)</em></small><br><em>(esterno a QUOTO, pechè QUOTO non ha funzioni di invio Newsletter)</em>'; 
        ...
        $text['data']['en']         = 'Date';  
        $text['modulo']['en']       = 'Form sent';  
        $text['accettati']['en']    = 'Consensus accepted';  
        $text['info_attuale']['en'] = 'Read current information on QUOTO of '.$rw['nome']; 
        ...
        $text['data']['fr']         = 'Date';  
        $text['modulo']['fr']       = 'Formulaire envoyé';  
        $text['accettati']['fr']    = 'Consensus accepté';  
        $text['info_attuale']['fr'] = 'Lire les informations actuelles sur QUOTO de '.$rw['nome']; 
        ...
        $text['data']['de']         = 'Datum';  
        $text['modulo']['de']       = 'Formular gesendet';  
        $text['accettati']['de']    = 'Konsens akzeptiert';  
        $text['info_attuale']['de'] = 'Lesen Sie aktuelle Informationen auf QUOTO von '.$rw['nome']; 
        ...
        /**
         * CODICE PER COMPIALARE IL LINK ALL'INFORMATIVA, SE NELLA RPIVACY DEL CLIENTE SU SUITEWEB C'E' 
         * IUBENDA ALLORA PRIMA CASISTICA
         * SE NOSTRA PRIVACY SI APRE MODALE CON I CONTENUTI INSERITI DA SUITEWEB
         */
        $select       = "SELECT 
                          ...
                          <div class="modal-dialog modal-lg">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title" id="myModalLabel">Informativa</h4>
        ...
        if($_REQUEST['action']=='send_email_hotel'){
            if($_REQUEST['email_destinatario']!='' && $_REQUEST['email_mittente']!='' && $_REQUEST['id_richiesta']!=''){
                ...
                $msg .= "\r\n";
                $msg .= 'Oggetto notifica: '.$_REQUEST['oggetto_notifica']."\r\n";
                $msg .= 'Identificativo notifica: '.$_REQUEST['id_richiesta']."\r\n";
                $msg .= "\r\n";
            if($_REQUEST['delete']==0 && $_REQUEST['export']==0 && $_REQUEST['marketing']==$check_marketing){
                ...
                $msg .= "\r\n";
                $msg .= 'Dalla vostra area dedicata in QUOTO "Profila ed Esporta", avete max 30 giorni di tempo  per poter apportare le modifiche richieste dall\'utente!';
                $msg .= "\r\n";
                $msg .= "\r\n";
                $msg .= 'Questa e-mail e\' stata inviata da un utente presente nella vostra raccolta dati del software QUOTO - By Network Service s.r.l';                
                    ...
                    $subject = 'Modifiche consensi in QUOTO per utente: '.$_REQUEST['email_destinatario'];
                    $message = $msg;
                    ...
                    header('Location:'.BASE_URL_SITO.'consensi/quoto.php?val='.base64_encode($_REQUEST['id_richiesta'].'#'.$_REQUEST['email_mittente']).'&res=sent');
            }
        }
...
<html>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <table class="table table-striped">
                       <tr>
                         ...
                         <tr>
                            <td style="white-space: nowrap;"><small><?=gira_data($r['DataRichiesta'])?></small></td>
                                <td>
                                    <small><?=$Ip?></small><br>
                                    <small><?=$browser?></small>                                   
                                </td>
                    ...
                    <div class="col-md-8">
                        ...
                        <table class="table table-striped">
                           ...
                           <tr>
                               ...
                               <th class="col-md-2"><?=$text['cellulare'][$lingua]?></th>                                
                            </tr>
                            <tr>
                                <td style="white-space: nowrap;"><small><?=($r['Email']!=''?$r['Email']:'')?></small></td> 
                                <td style="white-space: nowrap;"><small><?=($r['Cellulare']!=''?$r['Cellulare']:'')?></small></td>                                                         
                            </tr> 
                           <tr>
                               <th class="col-md-2"><?=$text['arrivo'][$lingua]?></th> 
                               <th class="col-md-2"><?=$text['partenza'][$lingua]?></th>                              
                            </tr>
                            <tr>
                                <td style="white-space: nowrap;"><small><?=($r['DataArrivo']!=''?$Data_arrivo:'')?></small></td>
                                <td style="white-space: nowrap;"><small><?=($r['DataPartenza']!=''?$Data_partenza:'')?></small></td>                                                          
                            </tr>  
                           <tr>
                               <th class="col-md-2"><?=$text['adulti'][$lingua]?></th>  
                               <th class="col-md-2"><?=$text['bambini'][$lingua]?></th>                             
                            </tr>
                            <tr>
                                <td style="white-space: nowrap;"><small><?=($r['NumeroAdulti']!=''?$r['NumeroAdulti']:'')?></small></td> 
                                <td style="white-space: nowrap;">
                                    <small><?=($r['NumeroBambini']!=''?$r['NumeroBambini']:'')?></small>
                                    <small><?=($r['NumeroBambini']!=''?' '.$text['eta'][$lingua].': ':'')?></small>
                                    <small>
                                        <?=($r['EtaBambini1']!=''?' '.$r['EtaBambini1']:'')?> 
                                        <?=($r['EtaBambini2']!=''?', '.$r['EtaBambini2']:'')?>
                                        <?=($r['EtaBambini3']!=''?', '.$r['EtaBambini3']:'')?>
                                        <?=($r['EtaBambini4']!=''?', '.$r['EtaBambini4']:'')?>
                                        <?=($r['EtaBambini5']!=''?', '.$r['EtaBambini5']:'')?>
                                        <?=($r['EtaBambini6']!=''?', '.$r['EtaBambini6']:'')?>
            ...
            <div class="col-md-12">
                <form action="<?=BASE_URL_SITO.'consensi/quoto.php?val='.$_REQUEST['val']?>" id="change_diritti" method="POST"> 
                    <table class="table">
                           <tr>
                               <td>                                                    
                                   <div class="checkbox">
                                    ...
                                    <input type="hidden" name="email_destinatario" value="<?=$rw['email']?>">
