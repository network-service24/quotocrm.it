...
if($_REQUEST['azione'] == 'send') {
        ...
        $chek_l_t = $fun->check_landing_template($idsito,$IdRichiesta);
        if($chek_l_t != 'smart'){
            $chek_l_t = $fun->check_landing_type_template($idsito,$IdRichiesta);
        }

        if($grafica != 'default'){
            $grafica = $fun->check_landing_type_template($idsito,$IdRichiesta);
        }

        if($chek_l_t!=''){

            if($chek_l_t=='default'){
                $link = (BASE_URL_LANDING.$directory_sito.'/'.base64_encode($IdRichiesta.'_'.$idsito.'_c').'/questionario/');                          
            }else{
                $link = (BASE_URL_LANDING.$chek_l_t.'/'.$directory_sito.'/'.base64_encode($IdRichiesta.'_'.$idsito.'_c').'/questionario/');
            } 
            ...
            if($grafica=='default'){
                ...
                $link = (BASE_URL_LANDING.$grafica.'/'.$directory_sito.'/'.base64_encode($IdRichiesta.'_'.$idsito.'_c').'/questionario/');
            }                
