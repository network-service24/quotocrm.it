...
function tot_fatturato($n_format=null){
	...
	$query = 'SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                ...
                AND (YEAR(hospitality_guest.DataRichiesta) = "'.$anno.'" OR YEAR(hospitality_guest.DataChiuso) = "'.$anno.'")';
  ...
  function prenotazioni_per_camera(){
      ...
      if(($tot)>0){
        ...
        $and = ' AND YEAR(DataArrivo) = "'.$anno.'" AND YEAR(DataPartenza) = "'.$anno.'" ';
