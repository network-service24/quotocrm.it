...
$sorgente      =      explode("-",$_REQUEST['sorgente']);
if(count($sorgente)==3){
...
$sql      = "   SELECT 
                ...
                AND
                    hospitality_custom_dimension.source = '".$source."'
                AND
                    hospitality_custom_dimension.medium = '".$medium."' 
                AND 
...
if($tot){
    ...
    echo '  <div class="row">
            <div class="col-md-1 nowrap">'.$record['NumeroPrenotazione'].'</div>
            <div class="col-md-3">'.$source.'-'.$medium.'</div>
            <div class="col-md-2 nowrap">'.($record['DataChiuso']!=''?'<small class="text-green">Preno confermata</small>':'<small class="text-orange">Conferma in trattativa</small>').'</div>
