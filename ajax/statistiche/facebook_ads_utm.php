...
$totalePPCn = $stat->fatt_utm($idsito,$filter_query,'facebook','social');

$rws9n      = $stat->campagne_utm($idsito,'facebook','social');
...
if ($totCampn > 0) {
    ...
    foreach ($rws9n as $key9n => $value9n) {
        ...
        $numero_campagne   = $stat->CountRicevuteCampagnaAds($idsito,$value9n['Campagna'],'facebook','social',$filter_query_p);
        $numero_prev_send  = $stat->CountInviateCampagnaAds($idsito,$value9n['Campagna'],'facebook','social',$filter_query_p);
        $preno_chiuse      = $stat->CountConfermateCampagnaAds($idsito,$value9n['Campagna'],'facebook','social',$filter_query);
        $totalePerCampagna = $stat->fatturato_per_campagna($idsito, $value9n['Campagna'],'facebook','social',$filter_query);
