...
include($_SERVER['DOCUMENT_ROOT']."/include/declaration.inc.php");
    ...
    if($_REQUEST['action'] == 'send_email_no_dispo') {
                ...
                switch($Lingua){
                    ...
                    break;
                    case "en":
                        $oggetto = 'Dear '.$Nome.' '.$Cognome.' we do not have availability for your stay request';
                        $noreply = 'This email was sent automatically by the software, do not reply to this email!';   
                    break;
                    case "fr":
                        $oggetto = 'Cher '.$Nome.' '.$Cognome.' nous n\'avons pas de disponibilité pour votre demande de séjour'; 
                        $noreply = 'Ce courriel a été envoyé automatiquement par le logiciel, ne répond pas à cet email!';  
                    break;
                    case "de":
                        $oggetto = 'Lieber '.$Nome.' '.$Cognome.' Wir haben keine Verfügbarkeit für Ihre Aufenthaltsanfrage';
            ...
            ##LOGS OPERAZIONI
            $log->lwrite('idsito = '.$_REQUEST['idsito'].', operatore = '.$_SESSION['user_accesso'].', IP = '.$_SERVER['REMOTE_ADDR'].', numero_prenotazione = '.$NumeroPrenotazione.', motivazione_no_dispo = '.$_REQUEST['motivo']);
            $log->lclose(); 
