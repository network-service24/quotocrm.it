...
function equalizza(id) {
    var highestBox = 400;
    var heigthRow = $("#infobox1").height();
    var new_height = (heigthRow - 20);
    if (highestBox > heigthRow) {
        $('#' + id).attr("style", "height:" + new_height + "px;overflow-y:auto;overflow-x:auto;");
        $('#' + id).addClass("scroll");
    } else {
        $('#' + id).attr("style", "height:" + heigthRow + "px;overflow-y:auto;overflow-x:auto;");
        $('#' + id).addClass("scroll");
    }
...
function validator_content(url) {
    if (window.confirm("ATTENZIONE: Sicuro di voler svuotare i contenuti della landing?")) {
        location.href = url;
    } else {
...
function validator_send(url) {
    if (window.confirm("ATTENZIONE: Sicuro di voler spedire la mail?")) {
        location.href = url;
    } else {
 ...
 /* alert sul mettere richieste nel cestino*/
  ...
  function validator_vaucher(url) {
    ...
    /* alert sulla disdetta di una prenotazione */
  function validator_disdetta(url) {
      if (window.confirm("ATTENZIONE: Sicuro di voler disdirre questa prenotazione?")) {
          location.href = url;
      } else {
