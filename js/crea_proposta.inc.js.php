...
function calcola_totale5() {
    ...
    function get_listino(n,r){
        ...
        $.ajax({
            type: "POST",
            url: "<?=BASE_URL_SITO?>ajax/listini/<?=($fun->check_listino_parity(IDSITO)==1?'get_listino_parity.php':'get_listino.php')?>",
...
function contenuto_landing(){
    ...
    var id_richiesta  = $("#Id").val();
    var idsito = <?=IDSITO?>;
    var lingua = $('#Lingua option:selected').val();
    $.ajax({        
        type: "POST",         
        url: "<?=BASE_URL_SITO?>ajax/preventivi/content_landing.php",        
        data: "tiporichiesta=" + tiporichiesta + "&idsito=" + idsito + "&lingua=" + lingua + "&id_richiesta=" + id_richiesta,
        dataType: "html",        
        success: function(data){
  
                //.instances['Testo'].destroy();           
                setTimeout(function() { 
                    $("#Testo").html(''+data+'');   
                }, 1000);  
/*                 setTimeout(function() { 
...
function enable_htmlarea(id){                                                            
    CKEDITOR.replace(id);
    $(".textarea").wysihtml5();	  												
}
function disable_htmlarea(id){                                                            
    CKEDITOR.instances[id].destroy();										
}

$( document ).ready(function() {

    /*CONTROLLO SE CI SONO BAMBNI QUINDI SI APRE INPUT DELLA ETA*/    
    if($(".NumeroBambini_1_1").val() != ''){
        $(".EtaBambini1_1").css("display","block");
    }else{
        $(".EtaBambini1_1").css("display","none");
    }
    if($(".NumeroBambini_2_1").val() != ''){
        $(".EtaBambini2_1").css("display","block");
    }else{
        $(".EtaBambini2_1").css("display","none");
    }
    if($(".NumeroBambini_3_1").val() != ''){
        $(".EtaBambini3_1").css("display","block");
    }else{
        $(".EtaBambini3_1").css("display","none");
    }
    if($(".NumeroBambini_4_1").val() != ''){
        $(".EtaBambini4_1").css("display","block");
    }else{
        $(".EtaBambini4_1").css("display","none");
    }
    if($(".NumeroBambini_5_1").val() != ''){
        $(".EtaBambini5_1").css("display","block");
    }else{
        $(".EtaBambini5_1").css("display","none");
    } 
   ...
   /*CONTROLLO DEL NUMERO DI TELEFONO*/    
    ...
    /*DIV (NOTE) SCROLL IN MODIFICA PROPOSTA PROVENIENTE DA SITO WEB*/                                               
        var offsetPixels = 100; // cambia con l'altezza della sidebar
    ...
    $("#TipoRichiesta").on("change",function(){
        ...
        <?php if($_SERVER['REQUEST_URI']=='/modifica_proposta/edit/'.$_REQUEST['param']){?>
            ...
            <?}?>
        <?}?>
/** check controllo emnail lato server sul CNAME */
<?php if(check_configurazioni(IDSITO,'check_verify_email')== 1){?>

        $("#bottone_salva").attr("disabled","true");
        $("#bottone_salva").removeClass("btn-success");
        $("#bottone_salva").addClass("btn-disabled");
        
        if($('#Email').val() != ''){
            var EmailCliente = $('#Email').val().trim();
            var EmailOperatore = "<?=EMAILHOTEL?>";
            if(EmailCliente.length>=2){
                $.ajax({
                    ...
                    success: function(data){
                        var classe = '';
                        if(data == 'valid'){
                            $("#check_email").html('<div class="alert alert-success text-center f-11 lineHeigth m-t-5">Dominio email valido ed esistente</div>');
                            $("#bottone_salva").removeAttr("disabled");
                            $("#bottone_salva").addClass("btn-success");
                            $("#bottone_salva").removeClass("btn-disabled");
                        }else{
                            $("#check_email").html('<div class="alert alert-danger text-center f-11 lineHeigth m-t-5">Dominio email non valido ed inesistente</div>');
                            $("#bottone_salva").attr("disabled","true");
                            $("#bottone_salva").removeClass("btn-success");
                            $("#bottone_salva").addClass("btn-disabled");
                        }
    ...
    //ulteriore validazione per il campo TipoVancanza(target) perchè è una select multipla e particolare con framework in JS

    $("#ctrl_form").on("submit",function(e){
        if (!$(this).valid()){
            if($("#TipoVacanza").val().length>=2){
                $("span.select2-selection").removeClass("error");
            }else{
                $("span.select2-selection").addClass("error");
            }
            if($("#TipoSoggiorno_1_1").val().length>=2){
                $("#TipoSoggiorno_1_1").removeClass("error");
            }else{
                $("#TipoSoggiorno_1_1").addClass("error");
                e.preventDefault();
            }
            if($("#TipoCamere_1_1").val().length>=2){
                $("#TipoCamere_1_1").removeClass("error");
            }else{
                $("#TipoCamere_1_1").addClass("error");
                e.preventDefault();
            }
            if($("#NumeroAdulti_1_1").val()!=''){
                $("#NumeroAdulti_1_1").removeClass("error");
            }else{
                $("#NumeroAdulti_1_1").addClass("error");
            }
        }else{
            $("span.select2-selection").removeClass("error");
            $("#TipoSoggiorno_1_1").removeClass("error");
    ...
    //loading form crea proposta
    $("#ctrl_form").on("submit",function(){
        if ($(this).valid()){
            $("#view_form_loading").html('<div class="clearfix">&nbsp;</div><div class="row"><div class="col-md-12 text-center"><img src="<?=BASE_URL_SITO?>img/loader_blue.gif" alt="Salvataggio in corso" style="height:100px"></div></div><div class="row"><div class="col-md-12 text-center"><small>Salvataggio in corso..., attendere il suo termine!</small></div></div><div class="clearfix"  style="height:10px">&nbsp;</div>');
            $("#menu_proposta").hide();
        }
    })
    ...
    /** slidetoggle per visualizzare la rubrica di ricerca anagrafica */
    $("#rubrica").on("click",function(){
        $( "#myAutocomplete" ).slideToggle(); 
    })
    /** lista anagrafiche per autocomplete */
    var src = {<?=$lista_nomi?>}
...
/*         $('#output').html(
        ...
        $.ajax({        
            type: "POST",         
            url: "<?=BASE_URL_SITO?>ajax/crea_proposta/storico_cliente.php",        
            data: "idsito=" + idsito + "&nome=" + Nome + "&cognome=" + Cognome+ "&email=" + Email,
            dataType: "html",        
            success: function(msg){
                $("#storico_cliente").html(msg);   
            },
            error: function(){
                alert("Chiamata fallita, si prega di riprovare..."); 
            }
   ...
   <?php if($_REQUEST['provenienza']!='' && $_REQUEST['provenienza']=='rubrica'){?>
        provenienza_rubrica();
    <?}?>
    <?php if(strstr($_SERVER['REQUEST_URI'],'/modifica_proposta/')){?>
        var TipoRich    =  $("#TipoRichiesta").val();
        if(TipoRich=='Preventivo'){
            provenienza_rubrica();
        }
    <?}?>
    <?php if($_SERVER['REQUEST_URI']=='/crea_proposta/'){?>
    ...
    $('#myAutocomplete').autocomplete({
        ...
        highlightClass: 'text-primary f-w-600',
        treshold: 2,
    });
    $('#myAutocomplete').on("focusout keyout", function() {
        setTimeout(function(){
            $('#myAutocomplete').val("");
        }, 2000);
        setTimeout(function(){
            $( "#myAutocomplete" ).hide('slow'); 
        }, 3000);
    });
    ...
    $("#id_template").imagepicker({
          hide_select : true,
          show_label:   true,
    })
/** solo in fase di modifica è possbile cambiare il testo associato al template */
<?php if($_SERVER['REQUEST_URI']=='/modifica_proposta/edit/'.$_REQUEST['param']){?>
    $("#id_template").on("change",function(){
        ...
        $.ajax({        
            type: "POST",         
            url: "<?=BASE_URL_SITO?>ajax/preventivi/associa_contenuto_landing.php",        
            data: "tiporichiesta=" + tiporichiesta + "&idsito=" + idsito + "&lingua=" + lingua + "&id_richiesta=" + id_richiesta+ "&id_template=" + id_template+ "&template=" + template,
            dataType: "html",        
            success: function(data){

               //CKEDITOR.instances['Testo'].destroy();
                setTimeout(function() { 
                    $("#Testo").html(''+data+'');   
                }, 1000);  
/*                 setTimeout(function() { 
...
<?}?>
    ...
    /** assegnare un templet in base al target */
    $("#TipoVacanza_").on("change",function(){
        ...
        var array_t = $("#TipoVacanza").val().toLowerCase();

        $("#id_template option").each(function(){

           var etichetta = $(this).text();

            var myMatch = array_t.search(etichetta.toLowerCase());
...
/*                                         setTimeout(function() { 
        ...
        $("#IMGTHUMB").find("div.thumbnail").each(function(){
    ...
    /**
    ** Gestione delle icone template
    ** possibilità di rendere invisibile un template 
    ** direttemente dal crea o modifica proposta
    */
    $("#IMGTHUMB").find("li").each(function(){

        var nomeTemplate = $(this).text();
        var nomeTemplate = (nomeTemplate.toLowerCase());
        var id_campo = nomeTemplate.replace(" ","");

        $(this).prepend("<i class='fa fa-eye-slash text-gray chiudiTemplate cursore' data-toggle='tooltip' data-html='true' title='Nascondi questo template!<br> Dal menù \"CONFIGURAZIONI<br> CONTENUTI & TEMPLATE\" puoi renderlo nuovamente visibile!' id='"+id_campo+"' data-id='"+nomeTemplate+"'></i>");

    });
        $(".chiudiTemplate").on("click",function(){
            var nome = $(this).data("id");
            if(confirm("Sei sicuro di voler nascondere questo template?") == true){ 
                $.ajax({
                        type: "POST",
                        url: "<?=BASE_URL_SITO?>ajax/templates/switch_per_nome_template.php",
                        data: "action=switch_t&idsito=<?=IDSITO?>&nome="+nome+"",
                        dataType: "html",
                            success: function(data){
    ...
    /* associazione target con template*/
...
/**
** associa infoBoxTag in base al template
*/

        var idsito = <?=IDSITO?>;
        var id_template = $("#id_template option:selected").val();
        $.ajax({        
            type: "POST",         
            url: "<?=BASE_URL_SITO?>ajax/preventivi/get_lista_infoboxtag.php",        
            data: "idsito=" + idsito + "&id_template=" + id_template,
            dataType: "html",        
            success: function(data){

                $("#id_infobox").html(''+data+'');  
    ...
    /** switch caret down and up in bae all'accordion delle proposte */
    $("a[href='#collapse1']").on("click", function(){
        if($("#collapse1").hasClass('show')==false){
            $("a[href='#collapse1']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse1']").find("i").addClass("fa-caret-up");   

            $("a[href='#collapse2']").find("i").addClass("fa-caret-down");
            $("a[href='#collapse2']").find("i").removeClass("fa-caret-up");  
            $("a[href='#collapse3']").find("i").addClass("fa-caret-down");
            $("a[href='#collapse3']").find("i").removeClass("fa-caret-up");  
            $("a[href='#collapse4']").find("i").addClass("fa-caret-down");
            $("a[href='#collapse4']").find("i").removeClass("fa-caret-up");  
            $("a[href='#collapse5']").find("i").addClass("fa-caret-down");
            $("a[href='#collapse5']").find("i").removeClass("fa-caret-up");  

        }else{
            $("a[href='#collapse1']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse1']").find("i").addClass("fa-caret-up");  

            $("a[href='#collapse2']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse2']").find("i").addClass("fa-caret-up");  
            $("a[href='#collapse3']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse3']").find("i").addClass("fa-caret-up");  
            $("a[href='#collapse4']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse4']").find("i").addClass("fa-caret-up");  
            $("a[href='#collapse5']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse5']").find("i").addClass("fa-caret-up");             

        }
    });
     $("a[href='#collapse2']").on("click", function(){
        if($("#collapse2").hasClass('show')==false){
            $("a[href='#collapse2']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse2']").find("i").addClass("fa-caret-up");  

            $("a[href='#collapse1']").find("i").addClass("fa-caret-down");
            $("a[href='#collapse1']").find("i").removeClass("fa-caret-up");  
            $("a[href='#collapse3']").find("i").addClass("fa-caret-down");
            $("a[href='#collapse3']").find("i").removeClass("fa-caret-up");  
            $("a[href='#collapse4']").find("i").addClass("fa-caret-down");
            $("a[href='#collapse4']").find("i").removeClass("fa-caret-up");  
    ...
    $("a[href='#collapse3']").on("click", function(){
        if($("#collapse3").hasClass('show')==false){
            $("a[href='#collapse3']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse3']").find("i").addClass("fa-caret-up");   
    ...
    $("a[href='#collapse4']").on("click", function(){
        if($("#collapse4").hasClass('show')==false){
            $("a[href='#collapse4']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse4']").find("i").addClass("fa-caret-up");  
    ...
    $("a[href='#collapse5']").on("click", function(){
        if($("#collapse5").hasClass('show')==false){
            $("a[href='#collapse5']").find("i").removeClass("fa-caret-down");
            $("a[href='#collapse5']").find("i").addClass("fa-caret-up"); 
...
/** controllo per cambio select tipo camere in base alla configurazione in quoto dei plugin abilitato o disabilitato */
...
/* switch tra importo e percentuale sull'acconto 1° proposta */
  $("#AccontoPercentuale_1").change(function(){
      ...
      }else if($("#AccontoPercentuale_1").val() == "garanzia"){
            $("#acconto_l1").html('<input type="hidden" id="AccontoImporto_1" name="AccontoImporto1"  value="0.1">');
            $("#AccontoPercentuale_1").val();
      }else{
...
/* switch tra importo e percentuale sull'acconto 2° proposta */
  $("#AccontoPercentuale_2").change(function(){
      ...
      }else if($("#AccontoPercentuale_2").val() == "garanzia"){
            $("#acconto_l2").html('<input type="hidden" id="AccontoImporto_2" name="AccontoImporto2"  value="0.1">');
            $("#AccontoPercentuale_2").val();          
      }else{
...
/* switch tra importo e percentuale sull'acconto 3° proposta */
  $("#AccontoPercentuale_3").change(function(){
      ...
      }else if($("#AccontoPercentuale_3").val() == "garanzia"){
            $("#acconto_l3").html('<input type="hidden" id="AccontoImporto_3" name="AccontoImporto3"  value="0.1">');
            $("#AccontoPercentuale_3").val();          
      }else{
...
/* switch tra importo e percentuale sull'acconto 4° proposta */
  $("#AccontoPercentuale_4").change(function(){
    if($("#AccontoPercentuale_4").val() == "importo"){
      ...
      }else if($("#AccontoPercentuale_4").val() == "garanzia"){
            $("#acconto_l4").html('<input type="hidden" id="AccontoImporto_4" name="AccontoImporto4"  value="0.1">');
            $("#AccontoPercentuale_4").val();        
    }else{
...
$("#AccontoPercentuale_5").change(function(){
    if($("#AccontoPercentuale_5").val() == "importo"){
      ...
      }else if($("#AccontoPercentuale_5").val() == "garanzia"){
            $("#acconto_l5").html('<input type="hidden" id="AccontoImporto_5" name="AccontoImporto5"  value="0.1">');
            $("#AccontoPercentuale_5").val();        
    }else{
...
/* calcolo percentuale sconto 1° proposta */
  ...
  $("#SC_1").change(function(){
        ...
        $('.prezzo1').each( function() {
            ...
            $("#Imponibile_1").html('<p><small><small>Sconto '+valore_sconto1 +'% pari a € '+sconto1+' sul totale camere € '+totale_camere1+'</small></small></p>');
  ...
  $("#SC_2").change(function(){
    var valore_sconto2 = $("#SC_2").val();
        ...
        $('.prezzo2').each( function() {
            ...
            $("#Imponibile_2").html('<p><small><small>Sconto '+valore_sconto2 +'% pari a € '+sconto2+' sul totale camere € '+totale_camere2+'</small></small></p>');
  ...
  $("#SC_3").change(function(){
    var valore_sconto3 = $("#SC_3").val();
        ...
        $('.prezzo3').each( function() {
            ...
            $("#Imponibile_3").html('<p><small><small>Sconto '+valore_sconto3 +'% pari a € '+sconto3+' sul totale camere € '+totale_camere3+'</small></small></p>');
    ...
    $("#SC_4").change(function(){
        ...
        $('.prezzo4').each( function() {
            ...
            $("#Imponibile_4").html('<p><small><small>Sconto '+valore_sconto4 +'% pari a € '+sconto4+' sul totale camere € '+totale_camere4+'</small></small></p>');
...
/* calcolo percentuale sconto 5° proposta */
    ...
    $("#SC_5").change(function(){
        ...
        $('.prezzo5').each( function() {
            ...
            $("#Imponibile_5").html('<p><small><small>Sconto '+valore_sconto5 +'% pari a € '+sconto5+' sul totale camere € '+totale_camere5+'</small></small></p>');
...
/* drag and drop della modale calcolatrice */
  $(".draggable").draggabilly();
    ...
    if($('#DataArrivo').val()!='' && $('#DataPartenza').val()!=''){
        ...
        let dateEnd = moment($('#DataArrivo').val());
        let dateStart = moment($('#DataPartenza').val());
        let dateDiff = dateStart.diff(dateEnd, 'days');
        
        $('#notti').html('per notti: ' +dateDiff+'');
    }
  $("#DataPartenza").focusout(function(){
        let End = moment($('#DataArrivo').val());
        let Start = moment($('#DataPartenza').val());
        let Nnotti = Start.diff(End, 'days');
      if(Start >= End){
          $("#notti").html('per notti: ' +Nnotti+'');
      }else{
          $("#notti").html('<text class="text-gray f-14 p-s-30">....la data di partenza è inferiore alla data di arrivo!</text>');
      }
        ...
        */
        ...
        $('#ChiPrenota').change(function() {
            ...
            $.ajax({
                ...
                dataType: "html",
                success: function(msg){
                    /** vecchio sistema con select email operatore */
                    //$("#EmailSegretaria").html(msg);
                     /** nuovo sistema con campo hidden email operatore */
        ...
        $('#NumeroBambini').change(function() {
          ...
          $('#SB_booking1B').click(function() {
            var promotionCode = $("#PromotionCode1").val();
            ...
            var adulti2   = $("#NumeroAdultiB1_2").val();
            var bambini2  = $("#NumeroBambiniB1_2").val();
            var eta1_2     = $("#EtaB1B1_2").val();
            var eta2_2     = $("#EtaB2B1_2").val();
            var eta3_2     = $("#EtaB3B1_2").val();
            var eta4_2     = $("#EtaB4B1_2").val();
            var eta5_2     = $("#EtaB5B1_2").val();
            var eta6_2     = $("#EtaB6B1_2").val();
            var adulti3   = $("#NumeroAdultiB1_3").val();
            var bambini3  = $("#NumeroBambiniB1_3").val();
            var eta1_3     = $("#EtaB1B1_3").val();
            var eta2_3     = $("#EtaB2B1").val();
            var eta3_3     = $("#EtaB3B1").val();
            var eta4_3     = $("#EtaB4B1").val();
            var eta5_3     = $("#EtaB5B1").val();
            var eta6_3     = $("#EtaB6B1").val();
            var idsito   = <?=IDSITO?>;
            var proposta = '1';
            ...
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_SITO?>ajax/booking/check_SB_booking.php",
                data: "proposta=" + proposta + "&promotionCode=" + promotionCode + "&start=" + start + "&end=" + end + "&numero_camere=" + numero_camere + "&adulti=" + adulti + "&bambini=" + bambini + "&eta1=" + eta1 + "&eta2=" + eta2 + "&eta3=" + eta3 + "&eta4=" + eta4 + "&eta5=" + eta5 + "&eta6=" + eta6 + "&adulti2=" + adulti2 + "&bambini2=" + bambini2 + "&eta1_2=" + eta1_2 + "&eta2_2=" + eta2_2 + "&eta3_2=" + eta3_2 + "&eta4_2=" + eta4_2 + "&eta5_2=" + eta5_2 + "&eta6_2=" + eta6_2 + "&adulti3=" + adulti3 + "&bambini3=" + bambini3 + "&eta1_3=" + eta1_3 + "&eta2_3=" + eta2_3 + "&eta3_3=" + eta3_3 + "&eta4_3=" + eta4_3 + "&eta5_3=" + eta5_3 + "&eta6_3=" + eta6_3 + "&idsito=" + idsito,
                dataType: "html",
                success: function(data){
        ...
        $('#SB_booking1E').click(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_SITO?>ajax/booking/check_ericsoft_booking.php",
        ...
        $('#SB_booking1Bedzzle').click(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_SITO?>ajax/booking/check_bedzzle_booking.php",
                data: "proposta=" + proposta + "&start=" + start + "&end=" + end + "&numero_camere=" + numero_camere + "&adulti=" + adulti + "&bambini=" + bambini + "&eta1=" + eta1 + "&eta2=" + eta2 + "&eta3=" + eta3 + "&eta4=" + eta4 + "&eta5=" + eta5 + "&eta6=" + eta6 + "&adulti2=" + adulti2 + "&bambini2=" + bambini2 + "&eta1_2=" + eta1_2 + "&eta2_2=" + eta2_2 + "&eta3_2=" + eta3_2 + "&eta4_2=" + eta4_2 + "&eta5_2=" + eta5_2 + "&eta6_2=" + eta6_2 + "&adulti3=" + adulti3 + "&bambini3=" + bambini3 + "&eta1_3=" + eta1_3 + "&eta2_3=" + eta2_3 + "&eta3_3=" + eta3_3 + "&eta4_3=" + eta4_3 + "&eta5_3=" + eta5_3 + "&eta6_3=" + eta6_3 + "&idsito=" + idsito,
                dataType: "html",
                success: function(data){
        ...
        /** CARICAMENTO DEI PACCHETTI SUL ONLOAD DELLA PAGINA */
      <?php if($_SERVER['REQUEST_URI']=='/crea_proposta')  {?>
        var Lingua = $('#Lingua option:selected').val();
        var idsito = <?=IDSITO?>;
            ...
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_SITO?>ajax/crea_proposta/lista_pacchetto.php",
                data: "lingua=" + Lingua + "&idsito=" + idsito,
                dataType: "html",
                success: function(data){
        ...
        <?}?>    
        /** CARICAMENTO DEI PACCHETTI SUL ON CHANGE DELLA PAGINA */
        ...
        $('#NomeProposta_1').change(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_SITO?>ajax/crea_proposta/descr_pacchetto.php",
        ...
        /** CARICAMENTO DELLE TARIFFE SUL ONLOAD DELLA PAGINA */
        var Lingua = $('#Lingua option:selected').val();
        var idsito = <?=IDSITO?>;
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_SITO?>ajax/crea_proposta/lista_tariffe.php",
                data: "lingua=" + Lingua + "&idsito=" + idsito,
                dataType: "html",
                success: function(data){
                        $("#load_db_tariffe").html(data);
                },
                error: function(){
                    alert("Chiamata fallita, si prega di riprovare...");
                }
            });
        /** CARICAMENTO DELLE TARIFFE SUL ON CHANGE DELLA PAGINA */
        $('#Lingua').change(function() {
            var Lingua = $('#Lingua option:selected').val();
            var idsito = <?=IDSITO?>;
                $.ajax({
        ...
        /** CARICAMENTO DEI PREFISSI SUL ON CHANGE DELLA PAGINA */
        ...
        $('#EtichettaTariffa_1').change(function() {
            ...
            $.ajax({
                type: "POST",
                url: "<?=BASE_URL_SITO?>ajax/crea_proposta/descr_tariffa.php",
        ...
        $('#SB_booking2B').click(function() {
            var promotionCode = $("#PromotionCode2").val();
            ...
            var adulti2   = $("#NumeroAdultiB2_2").val();
            var bambini2  = $("#NumeroBambiniB2_2").val();
            var eta1_2     = $("#EtaB1B2_2").val();
            var eta2_2     = $("#EtaB2B2_2").val();
            var eta3_2     = $("#EtaB3B2_2").val();
            var eta4_2     = $("#EtaB4B2_2").val();
            var eta5_2     = $("#EtaB5B2_2").val();
            var eta6_2     = $("#EtaB6B2_2").val();
            var adulti3   = $("#NumeroAdultiB2_3").val();
            var bambini3  = $("#NumeroBambiniB2_3").val();
            var eta1_3     = $("#EtaB1B2_3").val();
            var eta2_3     = $("#EtaB2B2_3").val();
            var eta3_3    = $("#EtaB3B2_3").val();
            var eta4_3     = $("#EtaB4B2_3").val();
            var eta5_3     = $("#EtaB5B2_3").val();
            var eta6_3     = $("#EtaB6B2_3").val();            
            var idsito   = <?=IDSITO?>;
            var proposta = '2';
        ...
        $('#SB_booking3B').click(function() {
            var promotionCode = $("#PromotionCode3").val();
            ...
            var adulti2   = $("#NumeroAdultiB3_2").val();
            var bambini2  = $("#NumeroBambiniB3_2").val();
            var eta1_2     = $("#EtaB1B3_2").val();
            var eta2_2     = $("#EtaB2B3_2").val();
            var eta3_2     = $("#EtaB3B3_2").val();
            var eta4_2     = $("#EtaB4B3_2").val();
            var eta5_2     = $("#EtaB5B3_2").val();
            var eta6_2     = $("#EtaB6B3_2").val();
            var adulti3   = $("#NumeroAdultiB3_3").val();
            var bambini3  = $("#NumeroBambiniB3_3").val();
            var eta1_3     = $("#EtaB1B3_3").val();
            var eta2_3     = $("#EtaB2B3_3").val();
            var eta3_3     = $("#EtaB3B3_3").val();
            var eta4_3     = $("#EtaB4B3_3").val();
            var eta5_3     = $("#EtaB5B3_3").val();
            var eta6_3     = $("#EtaB6B3_3").val();           
            var idsito   = <?=IDSITO?>;
            var proposta = '3';
       ...
       $('#SB_booking3Bedzzle').click(function() {
        ...
        $('#SB_booking4B').click(function() {
            var promotionCode = $("#PromotionCode4").val();
            ...
            var adulti2   = $("#NumeroAdultiB4_2").val();
            var bambini2  = $("#NumeroBambiniB4_2").val();
            var eta1_2     = $("#EtaB1B4_2").val();
            var eta2_2     = $("#EtaB2B4_2").val();
            var eta3_2     = $("#EtaB3B4_2").val();
            var eta4_2     = $("#EtaB4B4_2").val();
            var eta5_2     = $("#EtaB5B4_2").val();
            var eta6_2     = $("#EtaB6B4_2").val();
            var adulti3   = $("#NumeroAdultiB4_3").val();
            var bambini3  = $("#NumeroBambiniB4_3").val();
            var eta1_3     = $("#EtaB1B4_3").val();
            var eta2_3     = $("#EtaB2B4_3").val();
            var eta3_3     = $("#EtaB3B4_3").val();
            var eta4_3     = $("#EtaB4B4_3").val();
            var eta5_3     = $("#EtaB5B4_3").val();
            var eta6_3     = $("#EtaB6B4_3").val();            
            var idsito   = <?=IDSITO?>;
            var proposta = '4';
        ...
        $('#SB_booking5B').click(function() {
            var promotionCode = $("#PromotionCode5").val();
            ...
            var adulti2   = $("#NumeroAdultiB5_2").val();
            var bambini2  = $("#NumeroBambiniB5_2").val();
            var eta1_2     = $("#EtaB1B5_2").val();
            var eta2_2     = $("#EtaB2B5_2").val();
            var eta3_2     = $("#EtaB3B5_2").val();
            var eta4_2     = $("#EtaB4B5_2").val();
            var eta5_2     = $("#EtaB5B5_2").val();
            var eta6_2     = $("#EtaB6B5_2").val();
            var adulti3   = $("#NumeroAdultiB5_3").val();
            var bambini3  = $("#NumeroBambiniB5_3").val();
            var eta1_3     = $("#EtaB1B5_3").val();
            var eta2_3     = $("#EtaB2B5_3").val();
            var eta3_3     = $("#EtaB3B5_3").val();
            var eta4_3     = $("#EtaB4B5_3").val();
            var eta5_3     = $("#EtaB5B5_3").val();
            var eta6_3     = $("#EtaB6B5_3").val();            
            var idsito   = <?=IDSITO?>;
            var proposta = '5';
