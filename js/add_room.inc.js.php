...
function room_fields(n_proposta,id) {
    ...
    divtest.innerHTML = '<tr>'
                        ...
                        +'<td class="td25 no-border-top no-border-bottom"><div class="form-group"><input type="hidden" name="NumeroCamere' + n_proposta + '[]" id="NumeroCamere_' + n_proposta + '_' + room + '"><select name="TipoCamere' + n_proposta + '[]" id="TipoCamere_' + n_proposta + '_' + room + '" class="<?=$stile_chosen?> form-control" <?php echo ($fun->check_listini(IDSITO)==1?'onChange="get_listino(\' + n_proposta + \',\' + room + \');"':'')?>><?=$fun->lista_camere(IDSITO)?></select></div></td>'
                        +'<td class="td10 no-border-top no-border-bottom"><div class="form-group"><select name="NumAdulti' + n_proposta + '[]" id="NumeroAdulti_' + n_proposta + '_' + room + '" class="form-control" <?php echo ($fun->check_listini(IDSITO)==1?'onChange="get_listino(\' + n_proposta + \',\' + room + \');"':'')?>><?=$NumeroAdulti?></select></div></td>'
                        ...
                        +'<td class="td30 no-border-top no-border-bottom"><div class="posizione_spiegazione_prezzo" id="spiegazione_prezzo_' + n_proposta + '_' + room + '"></div><div class="form-group"><div class="input-group"><input type="text" name="Prezzo' + n_proposta + '[]" id="Prezzo_' + n_proposta + '_' + room + '"  class="prezzo' + n_proposta + ' form-control" placeholder="Prezzo 000.00" <?=(($fun->check_simplebooking(IDSITO)==0 && $fun->check_ericsoftbooking(IDSITO)==0 && $fun->check_bedzzlebooking(IDSITO)==0)?($fun->check_listini(IDSITO)==1?'onfocus="get_listino(\' + n_proposta + \',\' + room + \');"':''):'')?> onkeyup="calcola_totale' + n_proposta + '();"><span class="input-group-addon"><i class="fa fa-minus" onclick="remove_room_fields(' + room + ');"></i></span></div></div></td>'
                        +'</tr>';
