...
function tot_clienti($idstatus){
	...
	$rw = $dbMysqli->query('SELECT COUNT(anagrafica.idanagra) as tot_clienti FROM anagrafica  WHERE anagrafica.id_status = '.$idstatus.' AND anagrafica.id_stato != ""');
	$rw = $rw[0];
...
function tot_utenti($status){
	...
	$rwr = $dbMysqli->query('SELECT COUNT(utenti.idutente) as tot_utenti FROM utenti INNER JOIN siti ON siti.idsito = utenti.idsito INNER JOIN anagrafica ON anagrafica.idanagra = utenti.idanagra WHERE siti.data_start_hospitality != "" AND utenti.status = '.$status);
	$rwr = $rwr[0];
	          ...
	          $guida .='<li class="header-notification">
                            <div class="dropdown-primary dropdown">
                                ...
                                <ul class="show-notification notification-view dropdown-menu" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut">
                                        ...
                                        <li>
                                            <a href="" class="btn_mod link_istruzioni" data-toggle="modal" data-target="#video" data-titolo="Webinar - Nuove funzionalità di Quoto!" data-idvideo="bjfLHzGYt00">
                                                <i class="fa fa-youtube-play info text-red"></i> Webinar - Nuove funzionalità di Quoto!
                                            </a>
                                        </li>
                                 <!--      <li>
