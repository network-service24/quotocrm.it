                            ...
                            <div class="main-body">
                                ...
                                <? include_module('breadcrumb.inc.php'); ?>
                                    <div class="page-body">
                                            ...
                                            <div class="modal-dialog modal-lg" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <table id="modal-details-table" class="datatable table table-striped table-bordered f-12"></table>
                                                    </div>
                                        ...
                                        <h5>Statistiche di dettaglio metodo <strong><?php echo $method ?></strong> canale <strong><?php echo $channel ?></strong></h5>
                                        <div class="clearfix m-b-20"></div>
                                        <div class="card">
                                            <div class="card-block">
                                                <div class="row">
                                                    <div class="col-xs-12 col-md-12 col-lg-12 text-right">
                                                        <a href="<?php echo $link_breadcrumb ?>" class="btn btn-default btn-sm  f-left"><< Indietro</a>
                                                        <div class="pull-right form-inline">
                                                            <div class="form-group mr-5 f-left">
                                                                <label for="filter_date_range">Periodo</label>
                                                                <input type="text" id="filter_date_range" name="filter_date_range" autocomplete="off" class="daterange form-control m-l-10"
                                                                    value="<?php echo $startDate . ' - ' . $endDate ?>"/>
                                                            </div>
                                                            <label class="checkbox-inline mr-3 f-left">
                                                                <input type="checkbox" id="filter_compare" value="true" <?php if ($compare != 'false') echo 'checked="checked"' ?> class="m-r-10"> Confronta con
                                                            </label>
                                                            <select class="form-control mr-3 f-left" id="filter_compare_type">
                                        ...
                                        <a href="<?php echo $link_breadcrumb ?>" class="btn btn-default btn-sm"><< Indietro</a>
                                        <!--   <div class="card mt-5">
                                                <div class="card-block">
                                                    <div class="row">
                                                        <div class="col-xs-12 col-md-12 col-lg-12">
...
<link rel="stylesheet" type="text/css" media="all" href="<?php echo BASE_URL_SITO ?>plugin/daterangepicker-3.1.0/daterangepicker.css"/>
<link rel="stylesheet" type="text/css" media="all" href="<?php echo BASE_URL_SITO ?>plugin/datatables/jquery.dataTables.css"/>
<link rel="stylesheet" type="text/css" href="<?php echo BASE_URL_SITO ?>css/dataTables.bootstrap4.min.css">

<script>
    // Mi salvo il jQuery originale per cambiarlo e poi riprenderlo
    var jQueryOriginal = jQuery || $ || window.jQuery;
</script>
<script type="text/javascript" src="<?php echo BASE_URL_SITO ?>plugin/jquery-3.3.1/jquery.js"></script>
<script type="text/javascript">
    // sostituisco jQuery con una versione aggiornata
    var j = jQuery.noConflict();
</script>
<script type="text/javascript" src="<?php echo BASE_URL_SITO ?>plugin/moment-2.29.1/moment-with-locales.js"></script>
<script type="text/javascript" src="<?php echo BASE_URL_SITO ?>plugin/daterangepicker-3.1.0/daterangepicker.js"></script>
<script type="text/javascript" src="<?php echo BASE_URL_SITO ?>plugin/chartjs-3.4.1/chart.min.js"></script>
<script type="text/javascript" src="<?php echo BASE_URL_SITO ?>plugin/chartjs-adapter-moment-1.0.0/chartjs-adapter-moment-1.0.0.js"></script>
<script type="text/javascript" src="<?php echo BASE_URL_SITO ?>js/jquery.dataTables.min.js"></script>         
<script type="text/javascript" src="<?php echo BASE_URL_SITO ?>js/dataTables.bootstrap4.min.js"></script>
...
<script>
    j(function () {
        ...
        function updateChart(isCompare, start, end) {
            ...
            j.post({
                url: '<?php echo BASE_URL_SITO ?>ajax/statistiche/statistiche_new_graph_campaign.php',
                data: {
        ...
        dataTableConfig.columnDefs = [
            ...
            },
            {
                targets: [2, 3, 4],
                render: function (data, type, row, meta) {
                    if (row[0] != 'nessuna sorgente' && data != 0) {
        ...
        dataTableConfig.footerCallback = function (row, data, start, end, display) {
            ...
            api.columns([2, 3, 4], {page: 'current'}).every(function () {
                calculateFooterSum(this, 0, true);
            });
            api.columns([5], {page: 'current'}).every(function () {
                calculateFooterConversionPerc(this, 3, 4, true);
            });
            api.columns([1, 6, 7, 8], {page: 'current'}).every(function () {
                calculateFooterSum(this, 2, true, '€ ');
            });
        };
        let tableStats = j('#table_stats').DataTable(dataTableConfig);
        // tableStats.on("draw.dt", function () {
        //     console.log("tableStats.on('draw.dt') - fadeOut");
        ...
        function updateTable(isCompare, start, end) {
            ...
            //console.log("updateTable - fadeIn");
            j("#tab-loading").fadeIn();
            j.post({
                url: '<?php echo BASE_URL_SITO ?>ajax/statistiche/statistiche_new_table_campaign.php',
                data: {
                    'id_sito': <?php echo IDSITO ?>,
                ...
                success: function (result) {
                    ...
                    tableStats.draw();
                    j('#tab-loading').fadeOut();
                    //console.log("updateTable - success - fadeOut");
                }
            })
                .fail(function (err) {
                    j('#tab-loading').fadeOut();
                    //console.log("updateTable - fail - fadeOut");
                });
        }
...
/*         function updateTableAnnulled(isCompare, start, end) {
            ...
            j.post({
                url: '<?php echo BASE_URL_SITO ?>ajax/statistiche_new_table_campaign.php',
                data: {
                    'id_sito': <?php echo IDSITO ?>,
                    'method': '<?php echo $method ?>',
                    'channel': '<?php echo $channel ?>',
                ...
                success: function (result) {
                    ...
                    j(tableStats.column(8).header()).html('<span data-toggle="tooltip" title="Indicazione orientativa, stimata dal fatturato totale proveniente dal sito.">Fatturato Telefonico <i class="fa fa-info-circle" aria-hidden="true"></i></span>');

                    // Aggiungo il tooltip al titolo della colonna "Fatturato BE"
                    j(tableStats.column(7).header()).html('<span data-toggle="tooltip" title="Transazioni diverse da Quoto! rilevate da Google Analytics">Fatturato BE <i class="fa fa-info-circle" aria-hidden="true"></i></span>');

                    // Aggiungo il tooltip al titolo della colonna "Budget"
                    j(tableStats.column(1).header()).html('<span data-toggle="tooltip" title="Attenzione: la sincronizzazione dei dati del budget ADS Facebook viene eseguita ogni lunedi alle 6 del mattino, perciò potrebbero non comprendere l\'intero importo.">Budget investito <i class="fa fa-info-circle" aria-hidden="true"></i></span>');
                }
            });
        } */
        ...
        j('body').on('click', '.show-detail', function () {
            ...
            const campaign = row.find('td:eq(0)').text();
            const type = colIndex === 2 ? 'richieste' : colIndex === 3 ? 'inviati' : colIndex === 4 ? 'confermati' : '';
            ...
            j('#modal-details-table').DataTable({
                ajax: '<?php echo BASE_URL_SITO ?>ajax/statistiche/statistiche_new_details_campaign.php/?' + encodeQueryData(params),
                destroy: true,
                columns: [
                    {title: "Id"},
            ...
            $("#modal-details-table").css("width","100%");
            return false;
        });
