...
if ($_REQUEST['action'] == 'request_date') {
    ...
    $filter_ads_universal = " AND (hospitality_adCost_transactionRevenue.datastart >= '$DataRichiesta_dal' AND hospitality_adCost_transactionRevenue.dataend <= '$DataRichiesta_al')";
...
$legendaSn_BOX .= ' <div id="loading_fb"></div>
                    <div id="facebook_ads"></div>' . "\r\n";

$legendaSn_BOX .= ' <script>
                            $(document).ready(function() {
                                $("#loading_fb").html(\'<div class="col-md-12 text-center"><img src="' . BASE_URL_SITO . 'img/Ellipsis-1s-200px.svg"></div></div><div class="row"><div class="col-md-12 text-center"><small>Attendere il termine del caricamento dati!</small></div>\');
                                $.ajax({								 
                                    type: "POST",								 
                                    url: "' . BASE_URL_SITO . 'ajax/statistiche/facebook_ads_ga4.php",								 
                                    data: "idsito=' . IDSITO . '&filter_query_custom_client_id=' . $filter_query_custom_client_id . '&filter_query=' . $filter_query . '&filter_ads='.$filter_ads.'&filter='.$filter.'&action=' . $_REQUEST['action'] . '&querydate=' . $_REQUEST['querydate'] . '",
                                    dataType: "html",
