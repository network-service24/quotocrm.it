...
$content .='   <!-- Table datatable-->
               <table id="cestino" class="display compact dataTable table table-striped table-hover table-bordered table-sm f-13"  style="width:100%">
                    <thead>
                        <tr>';
...
$content .= '<div class="dataTables_info" id="cestino_info_serverSide" role="status" aria-live="polite">Visualizza pagina '.($_GET['pag']==''?1:$_GET['pag']).' di '.$tot_pagine.' per '.$tot_righe.' righe</div>'."\r\n";
$content .= $link_paginazione.''."\r\n";
$content .='<style>
                #azioniPrev .dropdown-toggle::after {
                    display: none !important;
                }
                '.(!$_REQUEST['action'] && $NumeroRecordArchivio > NUMERO_RECORD?'
                .pagination{
...
$content .='<script>
     ...
     // CONFIG DATATABLE
                var table = $("#cestino").DataTable( {
                    order: [[1, \'desc\'],[2, \'desc\']], 
                    responsive: true,
                    processing:true,
                    oLanguage: {sProcessing: "<div class=\'cell preloader5 loader-block\'><div class=\'circle-5 l loader-warning\'></div><div class=\'circle-5 m loader-warning\'></div><div class=\'circle-5 r loader-warning\'></div></div><span class=\'text-primary f-w-400 f-14 f-s-intial\'>QUOTO! sta caricando i dati...<br><span class=\'\'>Attendere!!</span></span>"},
                    "paging": '.(!$_REQUEST['action']?($NumeroRecordArchivio > NUMERO_RECORD?'false':'true'):'true').',
                        ...
                        {
                            text:      \'<i class="fa fa-repeat fa-2x fa-fw"></i> Ripristina\',
                            titleAttr: \'Ripristina selezionate\',
                            ...
                            action: function () {
                                        ...
                                        if(checkbox_value){
                                            if (window.confirm("ATTENZIONE: Sicuro di voler ripristinare le richieste selezionate?")){
                                                $.ajax({
                                                    url: "'.BASE_URL_SITO.'ajax/generici/re_save_all.php",
                                                    type: "POST",
                                                    data: "idsito='.IDSITO.'&checkbox_value="+checkbox_value,
                                                    dataType: "html",
                                                    success: function(data) { 
                                                            _alert("<i class=\"fa fa-repeat \"></i> Esito:","Richiesta/e sono state ripristinate!"); 
                                                            setTimeout(function(){
                                                                location.reload();
                                                            }, 1000);                                                                                                                   
                                                        }
                                         ...
                                         _alert("<i class=\"fa fa-repeat \"></i> Attenzione:","Selezionare prima di cliccare il pulsante!"); 
                                        }
                         ...
                         {
                            text:      \'<i class="fa fa-trash fa-2x fa-fw"></i> Elimina \',
                            titleAttr: \'Elimina definitivamente i selezionati\',
                            className: \'buttonSelezioni\',
                            attr: {id: \'delete_all\'},
                            action: function () {
                                    var checkbox_value = "";
                    ...
                    "ajax": "'.BASE_URL_SITO.'crud/proposte/cestino.crud.php?idsito='.IDSITO.''.$variabili.'",
                    "deferRender": true,
                    "columns": ['."\r\n";
        ...
        $content .='    "columnDefs": [
                              {"targets": [0,6,7,10,11,12,13], "orderable": false}  
                    ...
                    $("#cestino_processing").removeClass("card"); 

                     '.(!$_REQUEST['action']?($NumeroRecordArchivio > NUMERO_RECORD?'$("#cestino_wrapper").prepend(\'<div class="dataTables_filter f-11" style="position:absolute;right:20px;top:80px">Filtro rapido disattivato finchè<br> non si riducono i record a meno di '.NUMERO_RECORD.'</div>\');$("#cestino_filter").hide();':'$(".buttons-page-length").before("<i class=\"fa fa-eye fa-2x fa-fw\"></i>");'):'').'
