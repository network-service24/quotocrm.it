...
if($_GET['azione'] == 'send' && $_GET['param'] != '') {
        ...
        $select = "SELECT * FROM hospitality_guest  WHERE Id = ".$_GET['param'];
        ...
        $mail->Subject = str_replace("[cliente]",$Nome.' '.$Cognome, $rw['Oggetto']).' - '.NOMEHOTEL;
          ...
          if($TipoRichiesta == 'Conferma') { 
            ...
            if (!$mail->send()) {
                ...
                if($TipoRichiesta=='Conferma'){
                    $redirect ='<form action="'.BASE_URL_SITO.'conferme/" method="POST" name="redirectC" id="redirectC">
                                    <input type="hidden" name="idrichiesta" value="'.$_GET['param'].'">
                                </form>
                                <script>$(document).ready(function(){$("#redirectC").submit();});</script>';
                    //$prt->_goto(BASE_URL_SITO.'conferme/res/ok');
                    // header('Location:'.BASE_URL_SITO.'conferme/res/ok');
                }
                if($TipoRichiesta=='Preventivo'){
                    $redirect ='<form action="'.BASE_URL_SITO.'preventivi/" method="POST" name="redirectP" id="redirectP">
                                    <input type="hidden" name="idrichiesta" value="'.$_GET['param'].'">
                                </form>
                                <script>$(document).ready(function(){$("#redirectP").submit();});</script>';
