<?php
	$select ='SELECT
				...
				hospitality_guest.Archivia,
				hospitality_guest.Hidden 
			FROM
				hospitality_carte_credito
				INNER JOIN hospitality_guest ON hospitality_guest.Id = hospitality_carte_credito.id_richiesta 
			WHERE
				hospitality_carte_credito.id_richiesta = '.$_REQUEST['id_richiesta'].' 
				AND hospitality_carte_credito.idsito = '.IDSITO;
	...
	if($n_g >= 365){
		$Dvisibile = 0;
		$dbMysqli->query('UPDATE hospitality_carte_credito SET numero_carta  = "", visibile = 0  WHERE Id = ' .$idCarta);
		##LOGS OPERAZIONI
		$log->lwrite('idsito = '.$_SESSION['IDSITO'].', operatore = '.$_SESSION['user_accesso'].', IP = '.$_SERVER['REMOTE_ADDR'].', trascorsi 24 mesi eliminata carta di credito ID Carta = '.$idCarta);
		$log->lclose(); 
	}else{
		$Dvisibile = 1;
	}
	...
	$select ="	SELECT  
				...
				WHERE 
					hospitality_proposte.id_richiesta = ".$_REQUEST['id_richiesta']."";
            $rec = $dbMysqli->query($select);
            $PrezzoPC = '';
