 ...
 $sqln = "SELECT hospitality_tracking_ads.NumeroPrenotazione
    ...
    if($totCampn>0){
        ...
        foreach($array_campagnen as $k => $vl){
            ...
            $n_landing    = numero_campagne_da_landing_old(IDSITO,'facebook',$vl,'landing');
            $n_sito       = numero_campagne_da_sito_old(IDSITO,'facebook',$vl,'sito');
            ...
            $select10n = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                        ...
                        AND hospitality_guest.Hidden = 0
                        AND hospitality_guest.TipoRichiesta = 'Conferma'";
               $res10n = $dbMysqli->query($select10n);
               $rws10n = $res10n[0];
               if(is_array($rws10n)) {
        ...
        $legendaSn_BOX .= '<table class="display compact dataTable table table-striped table-hover table-bordered table-sm f-13" style="width:100%">
                            <tr>
        ...
        foreach ($array_fatturatoSn as $yn => $valn) {
            ...
            if($valn!="" || $valn != 0){
                ...
                $n_landingS    = numero_campagne_da_landing_old(IDSITO,'facebook',$yn,'landing');
                $n_sitoS       = numero_campagne_da_sito_old(IDSITO,'facebook',$yn,'sito');
                $numero_chiuse      = n_campagne_chiuse(IDSITO,'facebook',$yn);
...
<script>
    $(document).ready(function() {
        ...
        link = setInterval(function() {
            i++;
            $("#blink_fb").css(\'color\', i%2 == 1 ? \'#DD4B39\' : \'#2C3B41\');
        },speed);
