...
if($_REQUEST['azione']=='crea_form'){
		...
		$errore = array( 1=>'Il campo [label] e\' obbligatorio',
				...
				$id_form = $dbMysqli->insert("hospitality_form_testata",$f_header);
			...
			$numcamere_params = array(
				...
				foreach($lingue as $k=>$v){
					...
					$dbMysqli->insert("hospitality_form_testata_lang",$form_header_lang);
				...
				foreach($form_content as $k=>$v){
					array_push($id_items, $dbMysqli->insert("hospitality_form_contenuti",$v));
				}
				...
				foreach($lingue as $k=>$v){
					...
					$dbMysqli->insert("hospitality_form_contenuti_lang",$form_content_lang);
				...
				$prt->_goto(BASE_URL_SITO.'setting-form/');
