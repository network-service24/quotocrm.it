...
if(MANUTENZIONE==0){
	...
	$select     = 'SELECT  utenti.*
					,siti.https
					,siti.email
					,siti.web
					,siti.nome
					,siti.checkin_online_hospitality
					,siti.data_start_hospitality
					,siti.API_hospitality
					,siti.IdAccountAnalytics
					,siti.IdPropertyAnalytics
					,siti.ViewIdAnalytics
				FROM utenti
				INNER JOIN siti
				ON siti.idsito = utenti.idsito
				WHERE utenti.idutente = "'.$_SESSION['utente']['idutente'].'"';
	...
	$_SESSION['NOMEUTENTEACCESSI'] = NOMEUTENTEACCESSI;
	...
	if (!file_exists(BASE_PATH_SITO."uploads/".IDSITO)) {
		mkdir(BASE_PATH_SITO."uploads/".IDSITO ,0755,true);
    ...
    {
		//error_log(__FILE__);
    	require (INC_PATH_SITO.'controller/'.$Template.'.php');
		//error_log(__FILE__);
    }
    //carico il template
    if(file_exists(INC_PATH_SITO.'template/'.$Template.'.tpl.php'))
    {
    	...
    	require (INC_PATH_SITO.'template/'.$Template.'.tpl.php');

    }
    else
    {
    	require(INC_PATH_SITO.'template/error404.tpl.php');
    }
