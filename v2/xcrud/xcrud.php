<?php
/** f0ska xCRUD v.1.6.24-1; 10/2014 */
...
class Xcrud
{
    ...
    public static function get_requested_instance()
    {
        ...
        if (isset($_SESSION['xcrud_session'][$inst_name]['key']) && $_SESSION['xcrud_session'][$inst_name]['key'] == $key)
        {
            ...
            self::error('<a href="javascript:location.reload()" class="text-white">Refresh Pagina</a><br><strong>La chiave di verifica non è aggiornata </strong><br />
                            Ciò significa che il tuo browser ha memorizzato nella cache una versione precedente di questa pagina con una vecchia chiave (per motivi di sicurezza la chiave di verifica viene generata ad ogni richiesta)<br />
                            Come mai? Forse hai premuto il pulsante Indietro nel tuo browser o hai aperto un segnalibro dall\'ultima sessione. <br /><strong>Basta ricaricare la pagina, non è successo niente :)</strong>');
    }
    ...
    public static function load_js()
    {
        ...
        if (self::$instance)
        {
            ...
            $lang_arr = $instance->_get_language();
        }
        else
        {
        ...
        if (Xcrud_config::$load_googlemap)
            $out .= '<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCEhD0s4UEJdItPacNMZNLE_aoyLYGAHL8&language=' . $language . '"></script>';
