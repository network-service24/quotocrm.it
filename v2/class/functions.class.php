<?php
	/**
	 * classe di Marcello Visigalli marcello.visigalli@gmail.com
	 */
	class functions{

			function prepare_query($tabella,$dati,$type,$db,$param_update=null,$id=null){
			global $db;
			
			$db->query("SHOW COLUMNS FROM $tabella");
			$table_result = $db->result();
			foreach($table_result as $table_struct) $campi_tabella[] = $table_struct["Field"];
				...
				switch($type){
					...
					case "update":
						...
						if (($id) && (!$param_update)) $param_update = "id";
			...
			function fatturatoReferrer($n_format=null,$referrer){
				...
				$sel = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
							...
							AND hospitality_utm_ads.referrer = '".$referrer."'
