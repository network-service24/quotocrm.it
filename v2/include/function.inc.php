...
function menu_pannelli($idsito){
	global $db;
		$tooltip  = '';
		$pannelli = '';
        $db->query("SELECT * FROM hospitality_pannelli_esterni WHERE idsito = ".$idsito);
	    $r = $db->result();
		$tot = sizeof($r);
		if($tot>0){
			foreach($r as $key => $row){
                    ...
                    $pannelli .='<li class="'.$GLOBALS['ActiveMenu']['pannello'].'">
			                        <a href="'.BASE_URL_SITO.'pannello-pannelli_post/'.$row['idpannello'].'" >';
                                        if($row['ico_image']!=''){
                                                $pannelli .='<img src="'.BASE_URL_SITO.'uploads/'.$idsito.'/'.$row['ico_image'].'" style="width:20px;height:20px">';
                                        }elseif($row['font_awesome']!=''){
                                            $pannelli .='<i class="'.$row['font_awesome'].'"></i>';
                                        }
                                        if($row['ico_image']=='' && $row['font_awesome']==''){
                                            $pannelli .='<i class="fa fa-table"></i>';

                                        }

			        		$pannelli .='<span '.$tooltip.'>'.$row['nome_pannello'].'</span>
			                        </a>
								</li>';
...
function check_setup()
{
    ...
    $res = $db->query($sel);
    $chk = sizeof($db->result($res));

    return $chk;
}
function n_conferme_send($output=null){
	global $db,$db_suiteweb;
    $permessi_user = check_permessi();
	$q = $db->query('SELECT COUNT(Id) as n_conferme FROM hospitality_guest  WHERE TipoRichiesta = "Conferma" AND idsito = '.IDSITO.' '.($permessi_user['UNIQUE']==1?' AND ChiPrenota = "'.NOMEUTENTEACCESSI.'"':'').' AND Archivia = 0 AND Chiuso = 0 AND DataInvio is Null AND Hidden = 0 AND NoDisponibilita = 0');
	$rw = $db->row($q);
	if($rw['n_conferme'] > 0){
        if(!$output){
            return '<label class="badge bg-teal pull-right" id="notify_conferme">'.$rw['n_conferme'].'</label>';
        }else{
...
function n_preventivi_send($output=null){
    global $db,$db_suiteweb;
    $permessi_user = check_permessi();
	$q = $db->query('SELECT COUNT(Id) as n_preventivi FROM hospitality_guest  WHERE TipoRichiesta = "Preventivo" AND idsito = '.IDSITO.' '.($permessi_user['UNIQUE']==1?' AND ChiPrenota = "'.NOMEUTENTEACCESSI.'"':'').' AND Archivia = 0 AND DataInvio is Null AND Hidden = 0 AND NoDisponibilita = 0');
	$rw = $db->row($q);
	if($rw['n_preventivi'] > 0){
        if(!$output){
            return '<label class="badge bg-maroon pull-right" id="notify_preventivi">'.$rw['n_preventivi'].'</label>';
        }else{
...
function n_cestino($output=null){
    global $db,$db_suiteweb;
    $permessi_user = check_permessi();
	$q = $db->query('SELECT COUNT(Id) as n_richieste FROM hospitality_guest  WHERE idsito = '.IDSITO.' '.($permessi_user['UNIQUE']==1?' AND ChiPrenota = "'.NOMEUTENTEACCESSI.'"':'').' AND Hidden = 1');
	$rw = $db->row($q);
	if($rw['n_richieste'] > 0){
        if(!$output){
            return '<label class="badge bg-black pull-right" id="notify_cestino">'.$rw['n_richieste'].'</label>';
        }else{
            return $rw['n_preventivi'];
        }
	}
...
function n_annullate($output=null){
    global $db,$db_suiteweb;
    $permessi_user = check_permessi();
	$q = $db->query('SELECT COUNT(Id) as n_richieste FROM hospitality_guest  WHERE idsito = '.IDSITO.' '.($permessi_user['UNIQUE']==1?' AND ChiPrenota = "'.NOMEUTENTEACCESSI.'"':'').'  AND NoDisponibilita = 1 AND Hidden = 0 AND Disdetta = 0 AND Archivia = 0');
	$rw = $db->row($q);
	if($rw['n_richieste'] > 0){
        return '<label class="badge bg-gray pull-right">'.$rw['n_richieste'].'</label>';
	}
}
function n_archivio($output=null){
    global $db,$db_suiteweb;
    $permessi_user = check_permessi();
	$q = $db->query('SELECT COUNT(Id) as n_archiviate FROM hospitality_guest  WHERE idsito = '.IDSITO.' '.($permessi_user['UNIQUE']==1?' AND ChiPrenota = "'.NOMEUTENTEACCESSI.'"':'').' AND Archivia = 1');
	$rw = $db->row($q);
	if($rw['n_archiviate'] > 0){
        if(!$output){
            return '<label class="badge bg-black text-gray pull-right">'.$rw['n_archiviate'].'</label>';
        }else{
...
function tot_preventivi(){
	...
	$db->query('SELECT COUNT(Id) as tot_preventivi FROM hospitality_guest  WHERE TipoRichiesta = "Preventivo"                  
				...
				AND
					hospitality_guest.NoDisponibilita = 0 AND idsito = '.IDSITO.' AND '.($_REQUEST['date']==''?' (DataRichiesta >= "'.date('Y').'-01-01" AND DataRichiesta <= "'.date('Y').'-12-31")':' (DataRichiesta >= "'.$prima_data.'" AND DataRichiesta <= "'.$seconda_data.'")').'');
	$rw = $db->row();
...
function tot_invii(){
	...
	$db->query('SELECT COUNT(Id) as tot_invii FROM hospitality_guest  WHERE TipoRichiesta = "Preventivo" AND idsito = '.IDSITO.'                             
                            ...
                            AND
                                hospitality_guest.Accettato = 0
                            AND
                                hospitality_guest.NoDisponibilita = 0 AND DataInvio != "" AND '.($_REQUEST['date']==''?' (DataRichiesta >= "'.date('Y').'-01-01" AND DataRichiesta <= "'.date('Y').'-12-31")':' (DataRichiesta >= "'.$prima_data.'" AND DataRichiesta <= "'.$seconda_data.'")').'');
	$rws = $db->row();
...
function tot_prenotazioni(){
  ...
  $seconda_data = $seconda_data;
	$db->query('SELECT COUNT(Id) as tot_prenotazioni FROM hospitality_guest  WHERE TipoRichiesta = "Conferma" AND idsito = '.IDSITO.'                 
                AND
					hospitality_guest.Hidden = 0
				AND
				...
				AND 
					hospitality_guest.NoDisponibilita = 0  AND '.($_REQUEST['date']==''?' (DataRichiesta >= "'.date('Y').'-01-01" AND DataRichiesta <= "'.date('Y').'-12-31")':' (DataRichiesta >= "'.$prima_data.'" AND DataRichiesta <= "'.$seconda_data.'")').'');
	$rwc = $db->row();
...
function tot_preno_archiviate(){
  ...
  $seconda_data = $seconda_data.' 23:59:59';
	$db->query('SELECT COUNT(Id) as tot_prenotazioni FROM hospitality_guest  WHERE TipoRichiesta = "Conferma" AND idsito = '.IDSITO.' AND Hidden = 0 AND Chiuso = 1 AND Archivia = 1 AND '.($_REQUEST['date']==''?' (DataChiuso >= "'.date('Y').'-01-01 00:00:00" AND DataChiuso <= "'.date('Y').'-12-31 23:59:59")':' (DataChiuso >= "'.$prima_data.'" AND DataChiuso <= "'.$seconda_data.'")').'');
	$rwc = $db->row();
	return $rwc['tot_prenotazioni'];
}
function tot_annullate(){
  ...
  $prima_data   = $prima_data;
  $seconda_data = $seconda_data;
	$db->query('SELECT COUNT(Id) as tot_annullate FROM hospitality_guest  WHERE TipoRichiesta = "Conferma" AND idsito = '.IDSITO.' AND Archivia = 0 AND Hidden = 0 AND NoDisponibilita = 1  AND '.($_REQUEST['date']==''?' (DataRichiesta >= "'.date('Y').'-01-01" AND DataRichiesta <= "'.date('Y').'-12-31")':' (DataRichiesta >= "'.$prima_data.'" AND DataRichiesta <= "'.$seconda_data.'")').'');
	$rwc = $db->row();
...
function tot_disdetta(){
  ...
  $prima_data   = $prima_data.' 00:00:00';
  $seconda_data = $seconda_data.' 23:59:59';
	$db->query('SELECT COUNT(Id) as tot_disdette FROM hospitality_guest  WHERE TipoRichiesta = "Conferma" AND idsito = '.IDSITO.' AND Archivia = 0 AND Hidden = 0 AND Disdetta = 1 AND Chiuso = 1 AND '.($_REQUEST['date']==''?' (DataChiuso >= "'.date('Y').'-01-01 00:00:00" AND DataChiuso <= "'.date('Y').'-12-31 23:59:59")':' (DataChiuso >= "'.$prima_data.'" AND DataChiuso <= "'.$seconda_data.'")').'');
	$rwc = $db->row();
...
function tot_fatturato($n_format=null){
  ...
  $seconda_data = $seconda_data;
	...
	$db->query('SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                ...
                AND '.($_REQUEST['date']==''?' ((DataRichiesta>= "'.date('Y').'-01-01" AND DataRichiesta <= "'.date('Y').'-12-31") OR (DataChiuso >= "'.date('Y').'-01-01 00:00:00" AND DataChiuso <= "'.date('Y').'-12-31 23:59:59"))':' ((DataRichiesta >= "'.$prima_data.'" AND DataRichiesta <= "'.$seconda_data.'") OR (DataChiuso >= "'.$prima_data.' 00:00:00" AND DataChiuso <= "'.$seconda_data.' 23:59:59"))').'');
    $rwc = $db->row();
        ...
        foreach ($rwc as $key => $value) {
            ...
            if(!strstr($value['Agent'],'Mobile')){
                ...
                $select_d ='SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                ...
                                AND hospitality_guest.Disdetta = 0
                                AND hospitality_guest.TipoRichiesta = "Conferma"';
                $res_d = $db->query($select_d);
                $desktop = $db->row($res_d);
...
//     global $db,$prima_data,$seconda_data,$primo_anno,$secondo_anno;
//
...
//                         <!-- <th class="text-right col-md-2 nowrap"><small>Media</small></th> -->
//                         <!-- <th class="text-right col-md-2 nowrap"><small>ADR <i class="fa fa-exclamation-circle text-info" style="margin-left:10px;cursor:pointer" title="Calcolato sui giorni di attività della struttura<br>(Min Arrivo - Max Partenza)<br>Tot Fatturato per camera : giorni di attività : nr.camere!" data-html="true" data-toogle="tooltip"></i></small></th> -->
//                         <!-- <th class="text-center col-md-1 nowrap"><small>ADR <small><small>(punteggio)</small></small></th> -->
...
//         }else{
//             $anno = ' AND YEAR(DataArrivo) >= "'.$primo_anno.'" AND YEAR(DataPartenza) <= "'.$secondo_anno.'" ';
//         }
//
//
//         foreach ($re as $c => $vl) {
...
//                         AND hospitality_guest.Chiuso = 1
//                         AND hospitality_guest.Disdetta = 0
//                         AND '.($_REQUEST['date'] == ''?' hospitality_guest.DataArrivo >= "'.date('Y').'-01-01" AND hospitality_guest.DataPartenza <= "'.date('Y').'-12-31"': ' hospitality_guest.DataArrivo >= "'.$prima_data.'" AND hospitality_guest.DataPartenza <= "'.$seconda_data.'"').'
//                         AND hospitality_guest.TipoRichiesta = "Conferma" ';
//             $res  = $db->query($select);
//             $rec = $db->row($res);
//
...
//                         <!-- <td class="text-right nowrap"><small>'.($rec['fatturato']>0?number_format(($rec['fatturato']/$rec['nCamere']) ,2,',','.'):'0,00').' <i class="fa fa-euro"></i></small></td> -->
//                         <!-- <td class="text-right nowrap"><small>'.($rec['fatturato']>0?number_format($tariffa_media ,2,',','.'):'0,00').' <i class="fa fa-euro"></i></small></td> -->
//                         <!-- <td class="text-center nowrap"><small>'.number_format($ADR,2,',','.').'</small></td> -->
...
//         }
//             // $op .='<tr>
//             //             <td class="text-center nowrap" colspan="6"><small>Tabella statistiche in corso di aggiornamento <br />Al più presto tornerà onlne!</small></td>
//             //         </tr>';
//         $op .= '</table>';
//
//         return $op;
...
function fatturato_template(){
    ...
    $select = " SELECT *
                ...
                WHERE idsito = ".IDSITO."
                AND(TemplateType = 'custom1' OR TemplateType = 'custom2' OR TemplateType = 'custom3' OR TemplateName = 'default' OR TemplateName = 'smart')";
    $res = $db->query($select);
    $rws = $db->result($res);
...
//     $colore_leadtime = '';
//
//     $select = 'SELECT DataRichiesta, NumeroPrenotazione
//                 FROM hospitality_guest
//                 WHERE 1=1
//                 AND '.($_REQUEST['date'] == ''?' hospitality_guest.DataRichiesta >= "'.date('Y').'-01-01" AND hospitality_guest.DataRichiesta <= "'.date('Y').'-12-31"': ' hospitality_guest.DataRichiesta >= "'.$prima_data.'" AND hospitality_guest.DataRichiesta <= "'.$seconda_data.'"').'
//                 AND hospitality_guest.idsito = '.IDSITO.'
...
//
//             $valore .= '<div class="row">';
//             $valore .= '<div class="col-md-6 text-justify">Il Tempo medio dall\'arrivo o inserimento di una richiesta (preventivo) alla sua trasformazione in <b>Prenotazione Confermata</b> è pari a:</div>';
//             $valore .= '<div class="col-md-2 text-center text30"><span>&#10230;</span></div>';
//
//             $valore .= '<div class="col-md-4">';
//
//             if($formato == true){
//                 $valore .= '<b class="text30 text-green">'.$media_format.' <i id="leadtime" class="fa fa-calendar"></i></b>';
//             }else{
...
function fatturato_post_upselling(){
    global $db,$filter_query,$filter_query_p;
...
function prenotazioni_fonte(){
    ...
    if($tot>0){
        ...
        foreach ($array_fatturato as $k => $v) {
            ...
            // $stringa .= '<div class="row">';
            // $stringa .= '<div class="col-md-6"><b>TOTALE</b></div><div class="col-md-6"><i class="fa fa-euro"></i> '.number_format($totale,2,',','.').'</div>';
            // $stringa .= '</div>';
			...
			$footer = '<table class="tbl_body" cellpadding="0px" cellspacing="0px" border="0px" align="center">
								<tr>
									<td valign="top" class="footer" style="padding:10px 3px; text-align:left;">
										 <strong>'.NOME_AMMINISTRAZIONE.' <sub></sub></strong> &egrave; una realizzazione:<br />
											<br/>
											<img height="27" src="'.BASE_URL_SUITEWEB.'img/ns.png" align="left" style="margin-right:5px;"> Network Service srl - Via Valentini A. e L., 11 47922 Rimini (RN) | P.I. 02675250407<br />
...
function check_double_syncro_form_sito($idsito,$NumeroPrenotazione){
    ...
    $risultato = $db->query($select);
    $row       = $db->row($risultato);
    return $row['numero'];
}

function syncro_form_sito(){
        global $db_suiteweb,$db;
...
function check_comunicazioni(){
	global $db_suiteweb;
	    ...
	    $r = $db_suiteweb->query($s);
        $rSW = $db_suiteweb->row($r);
	      if($rSW){
	        $button ='<li class="dropdown notifications-menu">
	        				<a style="cursor:pointer;" data-toggle="modal" data-target="#comunicazioni" title="Ri-Apri la modale delle Comunicazioni">
	        					<span class="hidden-xs">Comunicazioni da:&nbsp;</span><img src="'.BASE_URL_SITO.'img/logo_nws.png">
	        				</a>
	        			</li>';
	      }
	     return $button;
	...
	function data_scadenza_software(){
		...
		if($differenza <= 31){
			...
			$box ='<li class="dropdown tasks-menu">
	                 	<span class="label label-info">Scadenza Quoto!: '.$data_end .'</span>
	             </li>';
		}
	...
	function scadenza_software(){
		...
		$select     = "SELECT DATEDIFF(data_end_hospitality,'".date('Y-m-d')."') AS DiffDate, data_start_hospitality,data_end_hospitality,no_rinnovo_hospitality  FROM siti WHERE idsito = ".IDSITO." AND hospitality = 1";
		$res        = $db_suiteweb->query($select);
		$row        = $db_suiteweb->row($res);
		$differenza = $row['DiffDate'];
		...
		$data_end   = $d_e_tmp[2].'-'.$d_e_tmp[1].'-'.$d_e_tmp[0];
			...
			$box ='<li class="dropdown tasks-menu">
	                <a aria-expanded="true" href="#" class="dropdown-toggle" data-toggle="dropdown">
	                  ...
	                  <span class="label label-danger">'.$differenza.'</span>
	                </a>
	                <ul class="dropdown-menu">
	                  <li class="header">Linea temporale di '.NOME_AMMINISTRAZIONE.'</li>
	                  <li>
	                    <!-- inner menu: contains the actual data -->
	                    <div style="position: relative; overflow: hidden; width: auto; height: auto;" class="slimScrollDiv"><ul style="overflow: hidden; width: 100%; height: auto" class="menu">
	                      <li>
	                        <a href="#">
	                        <small class="text-center">Tempo passato dalla attivazione alla scadenza</small>
	                          <h3>
	                            <small class="pull-left"><b>'.$data_start.'</b></small>
	                            <small class="pull-right"><b>'.$data_end.'</b></small>
	                          </h3>
	                          <div class="progress xs">
	                            <div class="progress-bar progress-bar-red" style="width: '.$diff_data.'%" role="progressbar" aria-valuenow="'.$diff_data.'" aria-valuemin="0" aria-valuemax="100">
	                            </div>
	                          </div>
	                          <small class="pull-right">sono rimasti '.$differenza.' gg</small>
	                        </a>
	                      </li>
						  <li>
	                        <div style="float:left!important;width:100%!important;padding:10px!important"><small>'.NOME_AMMINISTRAZIONE.' '.($row['no_rinnovo_hospitality']==0?'verrà rinnovato automaticamente!':'non verrà più rinnovato alla scadenza!').'</small></div>
	                      </li>
	                    </ul>
	                    <div style="background: rgb(0, 0, 0) none repeat scroll 0% 0%; width: 3px; position: absolute; top: 0px; opacity: 0.4; display: none; border-radius: 7px; z-index: 99; right: 1px; height: 186.916px;" class="slimScrollBar"></div><div style="width: 3px; height: 100%; position: absolute; top: 0px; display: none; border-radius: 7px; background: rgb(51, 51, 51) none repeat scroll 0% 0%; opacity: 0.2; z-index: 90; right: 1px;" class="slimScrollRail"></div></div>
	                  </li>
	                  <li class="footer"></li>
	                </ul>
	              </li>';
    ...
    function guida(){
	          ...
	          $guida .='<li class="dropdown notifications-menu">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                <span class="hidden-xs">Istruzioni d\'uso </span><i class="fa fa-question-circle fa-lg"></i>
                            </a>
                            <ul class="dropdown-menu" style="height:240px!important">
                                <li class="header">Tutorial tipo: <b>Frequently Asked Questions</b></li>
                                <li>
                                    <!-- inner menu: contains the actual data -->
                                    <div style="position: relative; overflow: hidden; width: auto; height: 230px!important;" class="slimScrollDiv">
                                    <ul  class="menu">
                                        <li>
                                            <a href="'.BASE_URL_SITO.'tutorial/faq_quoto.pdf" target="_blank">
                                                <i class="fa fa-file-pdf-o text-red"></i> Visualizza, scarica o stampa le istruzioni
                                            </a>
                                        </li>
                                     <!--   <li>
                                            <a href="'.BASE_URL_SITO.'tutorial/aggiornamenti_quoto.pdf" target="_blank" title="Storico degli aggiornamenti, upgrade e update fatti su e per QUOTO dalla sua messa online ad oggi!">
                                                <i class="fa fa-file-pdf-o text-purple"></i> <small>Storico Upgrade ed Update di e per QUOTO!</small>
                                            </a>
                                        </li>-->
                                        <li>
                                            <a href="https://resources.suiteweb.it/ApiQuoto/" target="_blank" title="API per sincronizzare richieste di un form esterno con QUOTO (per clienti con sito web non su server Network Service)">
                                                <i class="fa fa-code text-green"></i> <small>API per sincro richieste form esterno</small>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="https://quoto.online/tutorial/Buoni-Voucher-Quoto-2.pdf" target="_blank" title="Istruzioni d\'uso Buono Voucher">
                                                <i class="fa fa-file-pdf-o text-red"></i> <small>Tutorial Buono Voucher</small>
                                            </a>
                                        </li>
                                       
                                        <li>
                                            <a href="mailto:'.MAIL_ASSISTENZA.'?subject='.NOMEHOTEL.': chiedo assistenza sul software QUOTO!" title="Invia una e-mail a Network Service!">
                                                <i class="fa fa-question-circle text-info"></i> <small>Chiedi assistenza a: '.MAIL_ASSISTENZA.'</small>
                                            </a>
                                        </li>';
                        $guida .='  </ul>
                        			</div>
    ...
    function video_guida(){
	          ...
	          $guida .='<style>
	          			ul.menu.video{
                            ...
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                <span class="hidden-xs">Video tutorial </span><i class="fa fa-youtube fa-lg"></i>
                            </a>

                            <ul class="dropdown-menu">
                                <li>
                                    <!-- inner menu: contains the actual data -->
                                    <!-- <div style="position: relative; overflow: hidden; width: auto; height: 454px;" class="slimScrollDiv">-->
                                    <div style="position: relative; overflow: hidden; width: auto; height:290px;" class="slimScrollDiv">
                                    <ul  style="max-height: auto!important; width: auto!important" class="menu video">
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Novità di QUOTO" data-idvideo="EB9hT1fzLKI">
                                                <i class="fa fa-youtube-play info text-red"></i> Novità di QUOTO
                                            </a>
                                        </li>
                                       <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Come creare un preventivo" data-idvideo="vy6nhrzuwE4">
                                                <i class="fa fa-youtube-play info text-red"></i> Come creare un preventivo
                                            </a>
                                        </li>
                                       <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Conferma in trattativa ed invio voucher" data-idvideo="ty9dEiOdk2g">
                                                <i class="fa fa-youtube-play info text-red"></i> Conferma in trattativa ed invio voucher
                                            </a>
                                        </li>
                                       <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Modulo Check-in Online" data-idvideo="h2DlmWi6gi4">
                                                <i class="fa fa-youtube-play info text-red"></i> Modulo di Check-In Online
                                            </a>
                                        </li>

                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Scelta dei servizi aggiuntivi da parte del cliente " data-idvideo="QMX2XPjEpEY">
                                                <i class="fa fa-youtube-play info text-red"></i> Scelta dei servizi aggiuntivi da parte del cliente 
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Template e testi per target cliente " data-idvideo="SeEJRnvoVQo">
                                                <i class="fa fa-youtube-play info text-red"></i> Template e testi per target cliente 
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Novità d\'autunno 2021 di Quoto!" data-idvideo="WPem5MxjW1c">
                                                <i class="fa fa-youtube-play info text-red"></i> Novità d\'autunno 2021 di Quoto! 
                                            </a>
                                        </li>
                                <!--         <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Configurazione 3" data-idvideo="UmXO22FoPFc">
                                                <i class="fa fa-youtube-play info text-red"></i> Configurazione &nbsp;<small class="label bg-green">3°</small>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Configurazione 4" data-idvideo="dsgSPEgLTQQ">
                                                <i class="fa fa-youtube-play info text-red"></i> Configurazione &nbsp;<small class="label bg-teal">4°</small>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Configurazione 5" data-idvideo="p7x-wu79WgQ">
                                                <i class="fa fa-youtube-play info text-red"></i> Configurazione &nbsp;<small class="label bg-fuchsia">5°</small>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Contenuti & Template" data-idvideo="ASDxfYFjetM">
                                                <i class="fa fa-youtube-play info text-red"></i> Contenuti & Template &nbsp;<small class="label bg-blue">6°</small>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Uso di QUOTO Giornaliero 1" data-idvideo="gZKgEUAOsO4">
                                                <i class="fa fa-youtube-play info text-red"></i> Uso di QUOTO Giornaliero <b>1°</b> video
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Uso di QUOTO Giornaliero 2" data-idvideo="B9nTIBaNyFk">
                                                <i class="fa fa-youtube-play info text-red"></i> Uso di QUOTO Giornaliero <b>2°</b> video
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Uso di QUOTO Giornaliero 3" data-idvideo="9xkiHfyrzQI">
                                                <i class="fa fa-youtube-play info text-red"></i> Uso di QUOTO Giornaliero <b>3°</b> video
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Integrazione di SimpleBooking in QUOTO!" data-idvideo="qvwsKWTU6fo">
                                                <i class="fa fa-youtube-play info text-red"></i> Integrazione con SimpleBooking
                                            </a>
                                        </li>
                                        <li>
                                            <a href="" class="btn_mod" data-toggle="modal" data-target="#video" data-titolo="Procedura per il Checkin Online" data-idvideo="ZZF2nJEY1Sg">
	...
	function dettaglio_richiesta($id,$idsito){
...
function n_notifiche_cs($output=null){
	...
	$q = $db->query('SELECT * FROM hospitality_customer_satisfaction  WHERE  idsito = '.IDSITO.' AND data_compilazione ="'.date('Y-m-d').'" GROUP BY id_richiesta');
	$rw = sizeof($db->result($q));
	if($rw > 0){
        if(!$output){
            return '<span class="badge bg-light-blue pull-right" id="notify_cs">'.$rw.'</span>';
        }else{
            return $rw;
        }
...
function conferma_in_arrivo($primary_key){
    ...
    if($tot > 0){
        ...
        foreach ($res as $key => $value) {
            ...
            $sistemazione .= '<div style=\'float:right\'><a href=\'https://'.$_SERVER['HTTP_HOST'].'/v2/print_pdf/'.base64_encode($primary_key).'\' target=\'_blank\' class=\'btn btn-success btn-xs\'>Print PDF</a></div><br>';
...
function n_checkin($output=null){
	...
	$q = $db->query('SELECT * FROM hospitality_checkin  WHERE  idsito = '.IDSITO.' AND data_compilazione >= "'.date('Y-m-d  00:00:00').'" AND data_compilazione < "'.date('Y-m-d  23:59:59').'" GROUP BY Prenotazione');
	$rw = sizeof($db->result($q));
	if($rw > 0){
        if(!$output){
            return '<label class="badge bg-orange pull-righ" id="notify_schedine">'.$rw.'</label>';
        }else{
            return $rw;
        }
	}
...
function check_permessi()
{
    ...
    $esec   = $db_suiteweb->query($select);
    $record = sizeof($db_suiteweb->result($esec));
    ...
    $esec2   = $db_suiteweb->query($select2);
    $record2 = sizeof($db_suiteweb->result($esec2));
     ...
     global $db_suiteweb;
        ...
        if(is_array($rSW)) {
          ...
          if($check==0){
                ...
                $re = $db_suiteweb->query($se);
                $rs = $db_suiteweb->row($re);
...
function InformativaPrivacy()
{
    ...
    $dati_ = $db_suiteweb->query($sql_dati);
    $dati  = $db_suiteweb->row($dati_);
...
function tot_ticket_risposta($idsito,$output=null){
	...
	$db_suiteweb->query("SELECT COUNT(id) as tot_ticket FROM ticket  WHERE idsito = ".$idsito." AND stato = 'Chiuso' AND letto = 0 AND idcat = ".CATEGORIA_TICKET_QUOTO);
	$rwr = $db_suiteweb->row();
	if($rwr['tot_ticket']>0){
        if(!$output){
...
function check_pms($idsito)
{
    ...
    $Qquery = $db->query($Qcheck);
    $Tcheck = $db->row($Qquery);
    if(is_array($Tcheck)) {
        if($Tcheck > count($Tcheck)) // se la pagina richiesta non esiste
         $check = $Tcheck['Pms']; // restituire la pagina con il numero più alto che esista
    }else{
        $check = 0;
    }
...
function check_pms_cinquestelle($idsito){
    global $db;
       $Qcheck = "SELECT * FROM hospitality_pms WHERE idsito = ".$idsito."  AND Abilitato = 1 AND Pms = 'hotelcinquestelle.cloud' ORDER BY Id DESC LIMIT 1";
       $Qquery = $db->query($Qcheck);
       $Tcheck = $db->result($Qquery);
       if(sizeof($Tcheck)>0){
           $check = 1;
...
function ico_operatore($idsito,$username){
    ...
    $db_suiteweb->query("SELECT Sesso,nome FROM utenti_quoto WHERE idsito = ".$idsito." AND username = '".addslashes($username)."' AND abilitato = 1");
    $r = $db_suiteweb->row();
    if(is_array($r)) {
        if($r > count($r)) // se la pagina richiesta non esiste
            $tot = count($r); // restituire la pagina con il numero più alto che esista
    }else{
        $tot = 0;
    }
    if($tot>0){
            ...
            foreach($array_fetch as $key => $row){
             ...
             if(in_array(0,$aperture_exists)){
                $script = ' <script>
                                $( document ).ready(function() {
                                    setTimeout(function(){
                                        open_notifica("Sono passate più di <b>24 ore</b><br>dall\'invio dell\'email<br>'.$tipo.':","'.$testo.'","plain","bottom-right","info",10000,"'.$loadercolor.'");
                                    }, 4950);
                                });
                            </script>'."\r\n";
            }
...
function check_configurazioni($idsito,$parametro){
    ...
    $res_config = $db->query($config);
    $rec_config = $db->row($res_config);
...
function count_iscritti($idsito,$id_lista=null){
    ...
    $risult          = $db->query($query_gen);
    $record          = $db->row($risult);
...
function syncro_info_alberghi($idsito){
    global $db;
    ...
    $imap = $db->row($sq);
    if(is_array($imap)) {
        if($imap > count($imap)) // se la pagina richiesta non esiste
            $tot = count($imap); // restituire la pagina con il numero più alto che esista
    }else{
        $tot = 0;
    }
      if($tot>0){
            $InfoAlberghiButton = '<a href="#" class="btn bg-purple btn-xs" id="CheckBtn">Syncro Info Alberghi</a>
                                    <div id="pul_hide"></div>
                                    <script>
                                    $(document).ready(function() {
                                        if(leggiCookie(\'syncro_imap'.IDSITO.'\')) {
                                        $("#CheckBtn").css(\'display\',\'none\');
                                        $("#pul_hide").html(\'<small class="text-purple"><b>Una volta cliccato il pulsante, scompare e torna visibile dopo un ora!</b></small>\');
                                        }else{
                                            $("#pul_hide").hide();
                                        }
                                        $(\'#CheckBtn\').click(function(){
                                                ...
                                                $.ajax({
                                                    type: "POST",
                                                    url: "'.BASE_URL_SITO.'ajax/import_imap.php",
                                                    data: "idsito=" + idsito,
                                                    dataType: "html",
                                                    success: function(data){
                                                        $("#ctrl").html(\'<small class="text-purple"><b>Sincronizzazione effettuata!!</b></small>\');
                                                            setTimeout(function(){
                                                                    location.reload();
                                                            },2000);
                                                    },
...
function check_syncro_info_alberghi($idsito){
    global $db;
    ...
    if($rr > 0){
        ...
        $data_import    =  '<small><i class="fa fa-refresh text-purple" aria-hidden="true"></i> Ultima sincronizzazione con nuove richieste da <b>Info Alberghi</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'<br>Clicca sul pulsante per sincronizzare le email di Info Alberghi!</small>';
    }
    return $data_import;
}
function syncro_gabiccemare($idsito){
    global $db;
    ...
    $imap2 = $db->row($sq2);
        if($imap2['ServerEmail'] !='' && $imap2['UserEmail'] !='' && $imap2['PasswordEmail'] !=''){
            $GabicceMareButton = '<a href="#" class="btn bg-maroon btn-xs" id="CheckBtn2">Syncro GabicceMare.com</a>
                                        <div id="pul_hide2"></div>
                                        <script>
                                        $(document).ready(function() {
                                            if(leggiCookie(\'syncro_imap_gabiccemare\')) {
                                            $("#CheckBtn2").css(\'display\',\'none\');
                                            $("#pul_hide2").html(\'<small class="text-maroon"><b>Una volta cliccato il pulsante, scompare e torna visibile dopo un ora!</b></small>\');
                                            }else{
                                                $("#pul_hide2").hide();
                                            }
                                            $(\'#CheckBtn2\').click(function(){
                                                    ...
                                                    $.ajax({
                                                        type: "POST",
                                                        url: "'.BASE_URL_SITO.'ajax/import_imap_gabiccemare.php",
                                                        data: "idsito=" + idsito,
                                                        dataType: "html",
                                                        success: function(data){
                                                            $("#ctrl2").html(\'<small class="text-maroon"><b>Sincronizzazione effettuata!!</b></small>\');
                                                                setTimeout(function(){
                                                                        location.reload();
                                                                },2000);
                                                        },
...
function check_syncro_gabiccemare($idsito){
    global $db;
    ...
    if($rr2 > 0){
        ...
        $dataS   =  explode("-",$datS[0]);
        $dataH   =  explode(":",$datS[1]);
        $data_import_gabicce    =  '<small><i class="fa fa-refresh text-purple" aria-hidden="true"></i> Ultima sincronizzazione con nuove richieste da <b>GabicceMare.com</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'<br>Clicca sul pulsante per sincronizzare le email di GabicceMare.com!</small>';
    }
    return $data_import_gabicce;
}
function syncro_italyfamilyhotels($idsito){
    global $db;
    // inclusione per import email di gabiccemare.com
    ...
    $imap2 = $db->row($sq2);
        if($imap2['ServerEmail'] !='' && $imap2['UserEmail'] !='' && $imap2['PasswordEmail'] !=''){
            $GabicceMareButton = '<a href="#" class="btn bg-green btn-xs" id="CheckBtn3">Syncro ItalyFamilyHotels.it</a>
                                        <div id="pul_hide3"></div>
                                        <script>
                                        $(document).ready(function() {
                                            if(leggiCookie(\'syncro_imap_italyfamilyhotels\')) {
                                            $("#CheckBtn3").css(\'display\',\'none\');
                                            $("#pul_hide3").html(\'<small class="text-info"><b>Una volta cliccato il pulsante, scompare e torna visibile dopo un ora!</b></small>\');
                                            }else{
                                                $("#pul_hide3").hide();
                                            }
                                            $(\'#CheckBtn3\').click(function(){
                                                    ...
                                                    $.ajax({
                                                        type: "POST",
                                                        url: "'.BASE_URL_SITO.'ajax/import_imap_italyfamilyhotels.php",
                                                        data: "idsito=" + idsito,
                                                        dataType: "html",
                                                        success: function(data){
                                                            $("#ctrl3").html(\'<small class="text-info"><b>Sincronizzazione effettuata!!</b></small>\');
                                                                setTimeout(function(){
                                                                        location.reload();
                                                                },2000);
                                                        },
...
function check_syncro_italyfamilyhotels($idsito){
    global $db;
    ...
    if($rr2 > 0){
        ...
        $dataS   =  explode("-",$datS[0]);
        $dataH   =  explode(":",$datS[1]);
        $data_import_italyfamilyhotels    =  '<small><i class="fa fa-refresh" aria-hidden="true"></i> Ultima sincronizzazione con nuove richieste da <b>ItalyFamilyHotels.it</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'<br>Clicca sul pulsante per sincronizzare le email di ItalyFamilyHotels.it!</small>';
    }
    return $data_import_italyfamilyhotels;
}
function syncro_riccioneinhotel($idsito){
    global $db;
    // inclusione per import email di gabiccemare.com
    ...
    $imap2 = $db->row($sq2);
        if($imap2['ServerEmail'] !='' && $imap2['UserEmail'] !='' && $imap2['PasswordEmail'] !=''){
            $RiccioneinHotelButton = '<a href="#" class="btn btn-warning btn-xs" id="CheckBtn4">Syncro RiccioneInHotel.com</a>
                                        <div id="pul_hide4"></div>
                                        <script>
                                        $(document).ready(function() {
                                            if(leggiCookie(\'syncro_imap_riccioneinhotel\')) {
                                            $("#CheckBtn4").css(\'display\',\'none\');
                                            $("#pul_hide4").html(\'<small class="text-info"><b>Una volta cliccato il pulsante, scompare e torna visibile dopo un ora!</b></small>\');
                                            }else{
                                                $("#pul_hide4").hide();
                                            }
                                            $(\'#CheckBtn4\').click(function(){
                                                    ...
                                                    $.ajax({
                                                        type: "POST",
                                                        url: "'.BASE_URL_SITO.'ajax/import_imap_riccioneinhotel.php",
                                                        data: "idsito=" + idsito,
                                                        dataType: "html",
                                                        success: function(data){
                                                            $("#ctrl4").html(\'<small class="text-info"><b>Sincronizzazione effettuata!!</b></small>\');
                                                                setTimeout(function(){
                                                                        location.reload();
                                                                },2000);
                                                        },
...
function check_syncro_riccioneinhotel($idsito){
    global $db;
    ...
    if($rr2 > 0){
        ...
        $dataS   =  explode("-",$datS[0]);
        $dataH   =  explode(":",$datS[1]);
        $data_import_riccioneinhotel    =  '<small><i class="fa fa-refresh" aria-hidden="true"></i> Ultima sincronizzazione con nuove richieste da <b>RiccioneInHotel.com</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'<br>Clicca sul pulsante per sincronizzare le email di RiccioneInHotel.com!</small>';
    }
    return $data_import_riccioneinhotel;
}
function syncro_cesenaticobellavita($idsito){
    global $db;
    // inclusione per import email di gabiccemare.com
    ...
    $imap2 = $db->row($sq2);
        if($imap2['ServerEmail'] !='' && $imap2['UserEmail'] !='' && $imap2['PasswordEmail'] !=''){
            $CesenaticoBellaVitaButton = '<a href="#" class="btn btn-primary btn-xs" id="CheckBtn5">Syncro cesenaticobellavita.it</a>
                                            <div id="pul_hide5"></div>
                                            <script>
                                            $(document).ready(function() {
                                                if(leggiCookie(\'syncro_imap_cesenaticobellavita\')) {
                                                $("#CheckBtn5").css(\'display\',\'none\');
                                                $("#pul_hide5").html(\'<small class="text-info"><b>Una volta cliccato il pulsante, scompare e torna visibile dopo un ora!</b></small>\');
                                                }else{
                                                    $("#pul_hide5").hide();
                                                }
                                                $(\'#CheckBtn5\').click(function(){
                                                        ...
                                                        $.ajax({
                                                            type: "POST",
                                                            url: "'.BASE_URL_SITO.'ajax/import_imap_cesenaticobellavita.php",
                                                            data: "idsito=" + idsito,
                                                            dataType: "html",
                                                            success: function(data){
                                                                $("#ctrl5").html(\'<small class="text-info"><b>Sincronizzazione effettuata!!</b></small>\');
                                                                    setTimeout(function(){
                                                                            location.reload();
                                                                    },2000);
                                                            },
...
function check_syncro_cesenaticobellavita($idsito){
    global $db;
    ...
    if($rr2 > 0){
        ...
        $dataS   =  explode("-",$datS[0]);
        $dataH   =  explode(":",$datS[1]);
        $data_import_cesenaticobellavita    =  '<small><i class="fa fa-refresh" aria-hidden="true"></i> Ultima sincronizzazione con nuove richieste da <b>cesenaticobellavita.it</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'<br>Clicca sul pulsante per sincronizzare le email di cesenaticobellavita.it!</small>';
    }
    return $data_import_cesenaticobellavita;
}
function syncro_italybikehotels($idsito){
    global $db;
    ...
    $imap2 = $db->row($sq2);
        if($imap2['ServerEmail'] !='' && $imap2['UserEmail'] !='' && $imap2['PasswordEmail'] !=''){
            $Button = '<a href="#" class="btn btn-danger btn-xs" id="CheckBtn7">Syncro italybikehotels.it</a>
                                            <div id="pul_hide7"></div>
                                            <script>
                                            $(document).ready(function() {
                                                if(leggiCookie(\'syncro_imap_italybikehotels\')) {
                                                $("#CheckBtn7").css(\'display\',\'none\');
                                                $("#pul_hide7").html(\'<small class="text-info"><b>Una volta cliccato il pulsante, scompare e torna visibile dopo un ora!</b></small>\');
                                                }else{
                                                    $("#pul_hide7").hide();
                                                }
                                                $(\'#CheckBtn7\').click(function(){
                                                        ...
                                                        $.ajax({
                                                            type: "POST",
                                                            url: "'.BASE_URL_SITO.'ajax/import_imap_italybikehotels.php",
                                                            data: "idsito=" + idsito,
                                                            dataType: "html",
                                                            success: function(data){
                                                                $("#ctrl7").html(\'<small class="text-info"><b>Sincronizzazione effettuata!!</b></small>\');
                                                                    setTimeout(function(){
                                                                            location.reload();
                                                                    },2000);
                                                            },
...
function check_syncro_italybikehotels($idsito){
    global $db;
    ...
    if($rr2 > 0){
        ...
        $dataS   =  explode("-",$datS[0]);
        $dataH   =  explode(":",$datS[1]);
        $data_import_    =  '<small><i class="fa fa-refresh" aria-hidden="true"></i> Ultima sincronizzazione con nuove richieste da <b>italybikehotels.it</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'<br>Clicca sul pulsante per sincronizzare le email di italybikehotels.it!</small>';
    }
    return $data_import_;
}
function syncro_familygo($idsito){
    global $db;
    // inclusione per import email di familygo.eu
    ...
    $imap2 = $db->row($sq2);
        if($imap2['ServerEmail'] !='' && $imap2['UserEmail'] !='' && $imap2['PasswordEmail'] !=''){
            $CesenaticoBellaVitaButton = '<a href="#" class="btn btn-primary btn-xs" id="CheckBtn6">Syncro familygo.eu</a>
                                            <div id="pul_hide6"></div>
                                            <script>
                                            $(document).ready(function() {
                                                if(leggiCookie(\'syncro_imap_familygo\')) {
                                                $("#CheckBtn6").css(\'display\',\'none\');
                                                $("#pul_hide6").html(\'<small class="text-info"><b>Una volta cliccato il pulsante, scompare e torna visibile dopo un ora!</b></small>\');
                                                }else{
                                                    $("#pul_hide6").hide();
                                                }
                                                $(\'#CheckBtn6\').click(function(){
                                                        ...
                                                        $.ajax({
                                                            type: "POST",
                                                            url: "'.BASE_URL_SITO.'ajax/import_imap_familygo.php",
                                                            data: "idsito=" + idsito,
                                                            dataType: "html",
                                                            success: function(data){
                                                                $("#ctrl6").html(\'<small class="text-info"><b>Sincronizzazione effettuata!!</b></small>\');
                                                                    setTimeout(function(){
                                                                            location.reload();
                                                                    },2000);
                                                            },
...
function check_syncro_familygo($idsito){
    global $db;
    ...
    if($rr2 > 0){
        ...
        $dataS   =  explode("-",$datS[0]);
        $dataH   =  explode(":",$datS[1]);
        $data_import_familygo    =  '<small><i class="fa fa-refresh" aria-hidden="true"></i> Ultima sincronizzazione con nuove richieste da <b>familygo.eu</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'<br>Clicca sul pulsante per sincronizzare le email di familygo.eu!</small>';
    }
    return $data_import_familygo;
}
function check_syncro_form_sito($idsito){
    global $db;
    $s = "SELECT data FROM hospitality_data_syncro WHERE idsito = ".$idsito."  ORDER BY id DESC";
    $r = $db->query($s);
    $w = $db->row($r);
    if($w > 0){
        ...
        $dataH   =  explode(":",$datS[1]);
/*         $data_syncro    =  '<small>  
                                <i class="fa fa-refresh text-orange" aria-hidden="true"></i> Ultima sincronizzazione con <b>nuove</b> richieste dal vostro <b>Sito Web</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'
                                <br>
                                <i class="fa fa-exclamation-circle text-info" aria-hidden="true"></i> La sincronizzazione [<b class="text-info">DA SuiteWeb A QUOTO</b>] delle richieste provenienti dal vs. Sito Web,   viene <b>lanciata ogni ora al decimo minuto</b> (ES: 08:10, 09:10, ecc)!
                                <br>
                                    Se avete urgenza d\'inserire le e-mail ricevute dal form del vostro sito web, potete farlo manualmente '.syncro_button_form_sito().'

                                <i class="fa fa-exclamation-triangle text-red" aria-hidden="true"></i> Se il vostro sito NON usa il form di QUOTO ma uno generico, accertarsi di avere (almeno) i campi: Nome, Cognome, Data Arrivo e Data Partenza, <b class="text-red">OBBLIGATORI!</b>
                            </small>'; */
        $data_syncro    =  '<small>  
                            <i class="fa fa-refresh text-orange" aria-hidden="true"></i> Ultima sincronizzazione con <b>nuove</b> richieste dal vostro <b>Sito Web</b>: '.$dataS[2].'-'.$dataS[1].'-'.$dataS[0].' alle '.$dataH[0].':'.$dataH[1].':'.$dataH[2].'
                            <br>
                            <i class="fa fa-exclamation-circle text-info" aria-hidden="true"></i> La sincronizzazione delle E-mail dal form del vs. Sito Web,  viene ora <b>lanciata in REALTIME</b> (ES: arriva l\'email ed immediatamente si sincronizza su QUOTO)!
                        </small>';
    }
    return $data_syncro;
}
...
function func_menu_facebook($idsito){
  ...
  if(sizeof($variable)>0){
    ...
    $rws = $db->row($res);
    if(is_array($rws)) {
        if($rws > count($rws)) // se la pagina richiesta non esiste
            $tot = count($rws); // restituire la pagina con il numero più alto che esista
    }else{
        $tot = 0;
    }
    if($tot>0){
...
function func_check_accessi($idsito){
  ...
  $result = $db_suiteweb->query($select);
  $record = $db_suiteweb->result($result);
...
function check_data_syncro_listino_parity($idsito){
    ...
    $w = $db->row($r);
    if(is_array($w)) {
        if($w > count($w)) // se la pagina richiesta non esiste
            $tot = count($w); // restituire la pagina con il numero più alto che esista
    }else{
        $tot = 0;
    }
    if($tot >0){
...
function n_campagne_arrivate($idsito,$tracking,$campagna){
	...
	$q = $db->query("SELECT COUNT(Id) as n_preventivi FROM hospitality_tracking_ads  WHERE  hospitality_tracking_ads.idsito = ".$idsito." AND hospitality_tracking_ads.Tracking = '".$tracking."' AND hospitality_tracking_ads.Campagna = '".$campagna."'");
	$rw = $db->row($q);
...
/*function numero_campagne_da_landing($idsito,$tracking,$campagna,$NPreno,$clientid){
    global $db,$filter_query_client_id;
    ...
    $q = "  SELECT 
                COUNT(hospitality_client_id.id) as n_preventivi  
            FROM 
                hospitality_client_id
            INNER JOIN 
                hospitality_custom_dimension ON hospitality_custom_dimension.clientid = hospitality_client_id.CLIENT_ID
            ...
            AND 
                hospitality_custom_dimension.urlpath LIKE '%?res=sent'
            AND 
                (hospitality_custom_dimension.urlpath !='/?res=sent' OR hospitality_custom_dimension.urlpath != '/' OR hospitality_custom_dimension.urlpath  != '/index.php' OR hospitality_custom_dimension.urlpath NOT LIKE '%.php%') 
";
    $r  = $db->query($q);
    $rw = $db->row($r);
     if(is_array($rw)) {
...
/*function numero_campagne_da_sito($idsito,$tracking,$campagna,$NPreno,$clientid){
    global $db,$filter_query_client_id;
...
function check_vista(){
    ...
    $record_ = $db_suiteweb->query($select);
    $record  = $db_suiteweb->row($record_);
...
function get_account_analytics($idsito){
    ...
    $rs = $db_suiteweb->query($sql);
    $rc = $db_suiteweb->row($rs);
  
    return $rc;
  }
  function get_listmenu_template($idsito){
    global $db;
    ...
    $sql = "SELECT  TemplateName
            ...
            AND TemplateName != 'smart'
            AND TemplateType != 'custom4'
            AND TemplateType != 'custom5'
            AND TemplateType != 'custom6'
            AND TemplateType != 'custom7'
            AND TemplateType != 'custom8'
            AND TemplateType != 'custom9'";
    $rs = $db->query($sql);
    $rc = $db->result($rs);
    ...
    if($tot > 0){
        $n = 1;
        $ClassColor = '';
        foreach ($rc as $ky => $val) {
            switch($n){
                case 1:
                    $ClassColor = 'text-orange';
                break;
                case 2:
                    $ClassColor = 'text-fuchsia';
                break;
                case 3:
            ...
            $menu_template .= ' <li class="'.$GLOBALS['ActiveMenu']['templates'].'">
                                    <a href="'.BASE_URL_SITO.'templates-anteprima_web_custom'.$n.'/"><i class="fa  fa-windows"></i> <span>Anteprima <small>Template <span class="'.$ClassColor.'">'.$val['TemplateName'].'</span></small></span></a>
                                </li>'."\r\n";
            $n++;
        }
    }else{
...
function check_controllo_servizi($idsito){
  ...
  function check_numero_servizi($idsito){
    global $db;
        ...
        $result_num = $db->query($num_servizi);
        $r = $db->row($result_num);
        $numero =  $r['num_serv'];
    ...
    if(sizeof($record)>0){
        ...
        foreach ($record as $key => $value) {
            ...
            $body .= '<tr id="riga'.$value['id'].'">
                        ...
                        <td class="text-left nowrap">'.$value['user'].'</td>
        ...
        foreach ($record as $key => $value) {
            ...
            $modali .= '<div class="modal fade" id="idChat'.$value['id'].'"  role="dialog" aria-labelledby="idChat'.$value['id'].'">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title" id="myModalLabel">Chat in attesa di risposta!</h4>
                                        <div style="position:absolute;top:10px;right:10px;">
                                            <button type="button" id="pul_close'.$value['id'].'" class="btn btn-danger btn-md" data-dismiss="modal" aria-label="Close" style="float:right"><span aria-hidden="true">chiudi</span></button>
                                            <div class="clearfix"></div>
                                            <small style="float:right">Clicca qui per ricaricare il box delle Chat in attesa!</small>
                                        </div>
                                        <div style="clear:both;width:100%;margin-top:10px;">&nbsp;</div>
                                </div>
                                    <div class="modal-body">
                                         <iframe src="'.BASE_URL_SITO.'ajax/dashboard_chat.php?NumeroPrenotazione='.$value['NumeroPrenotazione'].'&idsito='.IDSITO.'&id_notify='.$value['id'].'" frameborder="no" scrolling="yes" onload="resizeIframe(this)" style="min-height:800px;width:100%"></iframe>
                                    </div>
                                </div>
                            </div>
                        </div>
...
function check_control_modify($idsito,$id_richiesta,$operatore){
    ...
    $resultCheck = $db->query($selectCheck);
    $recordCheck = $db->row($resultCheck);
    if(is_array($recordCheck)) {
        if($recordCheck > count($recordCheck)) 
            $check = count($recordCheck); 
    }else{
        $check = 0;
    }
    ...
    if($check == 0){
        ...
        $resultCheckOp = $db->query($selectCheckOp);
        $recordCheckOp = $db->row($resultCheckOp);
        if(is_array($recordCheckOp)) {
            if($recordCheckOp > count($recordCheckOp)) 
                $checkOp = count($recordCheckOp); 
        }else{
            $checkOp = 0;
        }
...
function check_data_password_overfade(){
    ...
    if(sizeof($rec)==0){
        ...
        if(date('Y-m-d')>=$data_account){
            $output ='    <div class="modal fade" id="change_password" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                <h4 class="modal-title text-center">Sono passati 3 mesi, è obbligatorio modificare <br>la tua password di accesso a QUOTO!</h4>
                            </div>
                                <div class="modal-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-8">
                                            ...
                                            <form name="update_pass" id="update_pass" method="post">
                                                ...
                                                <div class="form-group">
                                                    <input type="text" name="check_password" id="check_password" value="'.base64_decode($op_passowrd).'" class="form-control" placeholder="Crea la tua nuova Password" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="La password deve contenere un numero e una lettera maiuscola e minuscola e almeno 8 caratteri e non più di 18"  minlength="8" maxlength="18" required />
                                                </div> 
                                                <div class="footer">  
                                                    ...
                                                    <button type="submit" class="btn btn-info btn-block" id="pul_change_pass">Salva nuova password</button>  
                                                </div>
                                            </form>
                                            <script>
                                                $(document).ready(function(){
...
$select = " 	SELECT COUNT(Id) as n
 ...
 function CountRichiesteRicevuteAds($idsito, $s, $m, $dal, $al)
{
    global $dbMysqli;
 ...
 function CountRichiesteInviateAds($idsito, $s, $m, $dal, $al)
{
    global $dbMysqli;
 ...
 function CountRichiesteConfermateAds($idsito, $s, $m, $dal, $al)
{
    global $dbMysqli;
...
function fatturatoPerSitoweb($idsito, $dal, $al)
{
    global $dbMysqli;
...
function fatturatoBePerFonte($idsito, $s, $m, $dal, $al)
    {
        global $dbMysqli;
    ...
    function check_non_archiviate($idsito,$anno){
        global $db;
    ...
    function archivia_anno($idsito,$anno){
        global $db;
