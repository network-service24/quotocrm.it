...
if($_REQUEST['azione'] != '' && $_REQUEST['param'] != '' && $_REQUEST['valore'] != ''){
		...
		$rec = $db->result($risultato);
		if(sizeof($rec)>0){
			foreach ($rec as $k => $val) {
				$db->query("DELETE from hospitality_checkin WHERE Id = ".$val['id']);
			}			
		}

		//header('Location:'.BASE_URL_SITO.'schedine_alloggiati/');
		$prt->_goto(BASE_URL_SITO.'schedine_alloggiati/'); 
}