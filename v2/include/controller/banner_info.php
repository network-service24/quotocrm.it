<?php
    $sel = "SELECT * FROM hospitality_banner_info_lang WHERE idsito = ".IDSITO;
    $rs  = $db->query($sel);
    $rws = $db->row($rs);
    if(is_array($rws)) {
        if($rws > count($rws))
    ...
    $select = "SELECT * FROM hospitality_banner_info WHERE idsito = ".IDSITO;
    $res    = $db->query($select);
    $rw     = $db->row($res);
    if(is_array($rw)) {
        if($rw > count($rw))
    ...
    if($tot == 0){
        $insert = "INSERT INTO hospitality_banner_info(idsito,Titolo) VALUES('".IDSITO."','Misure anti COVID-19')";
        $ins    = $db->query($insert);
        $id     = $db->insert_id($ins);
    }else{
        $id = $rw['id'];
    ...
    $xcrud->table('hospitality_banner_info');
    $xcrud->where('hospitality_banner_info.idsito', IDSITO);
  ...
  /* subselect per visualizzare le icone delle lingue relative ai testi presenti */
    $xcrud->subselect('Testi presenti','SELECT GROUP_CONCAT(DISTINCT CONCAT(Lingua," ") SEPARATOR " ") AS lingue FROM hospitality_banner_info_lang WHERE IdBannerInfo = {Id}');
    $xcrud->column_callback('Testi presenti','show_flags');


    $xcrud->create_action('Attiva_banner', 'abilita_info_banner'); // action callback, function publish_action() in functions.php
    $xcrud->create_action('Disattiva_banner', 'disabilita_info_banner');
    $xcrud->button('#', 'Disabilita', 'icon-close glyphicon glyphicon-ok', 'xcrud-action',
        array(  // set action vars to the button
            'data-task' => 'action',
            'data-action' => 'Disattiva_banner',
            'data-primary' => '{Id}'),
    ...
    $evento_lang = $xcrud->nested_table('Gestione testi in lingua','id','hospitality_banner_info_lang','IdBannerInfo');
