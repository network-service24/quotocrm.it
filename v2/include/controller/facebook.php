...
$anniprima = (date('Y')-$diff_anni);
 ...
 $sqln = $db->query("SELECT hospitality_tracking_ads.NumeroPrenotazione
                         FROM hospitality_tracking_ads
                         WHERE hospitality_tracking_ads.idsito = ".IDSITO."
                         AND hospitality_tracking_ads.Tracking = 'facebook'
                         AND Campagna != ''");
    $rwsn = $db->result($sqln);
    $check_socialNew  = sizeof($rwsn);
    if($check_socialNew > 0){
      foreach ($rwsn as $key => $valuen) {
        $NumeriPrenoNew[] = $valuen['NumeroPrenotazione'];
      }
        $NumeriNew = implode(',',$NumeriPrenoNew);
        ...
        $selectn = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                ...
                                WHERE hospitality_guest.NumeroPrenotazione IN (". $NumeriNew.")
                               ".$filter_query ."
...
<script>
    $(document).ready(function() {
        ...
        link = setInterval(function() {
            i++;
            $("#blink_fb").css(\'color\', i%2 == 1 ? \'#DD4B39\' : \'#FFFFFF\');
        },speed);
});
</script>'."\r\n";
