<?php
    $select = "SELECT * FROM hospitality_giorni_checkinonline WHERE idsito = ".IDSITO;
    $res    = $db->query($select);
    $rw     = $db->row($res);
    if(is_array($rw)) {
        if($rw > count($rw))
    ...
    if($tot == 0){
        $insert = "INSERT INTO hospitality_giorni_checkinonline(idsito) VALUES('".IDSITO."')";
        $ins    = $db->query($insert);
        $id     = $db->insert_id($ins);
    }else{
        $id = $rw['id'];
    ...
    $xcrud->table('hospitality_giorni_checkinonline');
    $xcrud->where('hospitality_giorni_checkinonline.idsito', IDSITO);                    
    ...
    $xcrud2->table('hospitality_giorni_checkinonline');
    $xcrud2->where('hospitality_giorni_checkinonline.idsito', IDSITO);                    
    ...
    $xcrud2->label('numero_giorni', 'Numero Giorni PRIMA del CheckIn');

    $xcrud2->change_type('numero_giorni','select','','0,1,2,3,4,5');


    $xcrud2->table_name('<small>Scegliere quanti  giorni PRIMA del Check In deve partire automaticamente l\'email per la compilazione da parte del cliente del Check-In OnLine</small>');
    $xcrud2->unset_print();
    $xcrud2->unset_add();
    $xcrud2->unset_remove();
    $xcrud2->unset_view();
    ...
    $xcrud_reselling->where('hospitality_dizionario.etichetta = "TESTOMAIL_CHECKIN" OR hospitality_dizionario.etichetta = "OGGETTO_CHECKIN"');
  ...
  /* subselect per visualizzare le icone delle lingue relative ai testi presenti */
    ...
    $xcrud_reselling->table_name((CHECKINONLINE==1?'<small>Contenuti: dell\'email e whatsapp per il Check-In OnLine!</small':'<small>Contenuti: email per il Check-In OnLine!</small>'));
