...
<script>
    ...
    if(sizeof($rec)>0){
        ...
        foreach($rec as $ch => $vl){
            $TipiPagamento .='
                            ...
                            '.($vl['TipoPagamento']=='Stripe'?'<td class="text-center no_border_top no_padding nowrap"><b>Stripe</b><br><input type="checkbox" class="js-switch" value="1" '.($vl['TipoPagamento']=='Stripe'?'checked="checked"':'').' name="GBS"></td>':'').'
     ...
     */
    $ins_pag = "INSERT INTO hospitality_rel_pagamenti_preventivi(idsito,id_richiesta,CC,BB,VP,PP,GB,GBVP,GBS,GBNX) VALUES ('".$idsito."','".$IdRichiesta."','".$CC."','".$BB."','".$VP."','".$PP."','".$GB."','".$GBVP."','".$GBS."','".$GBNX."')";
    $db->query($ins_pag);
