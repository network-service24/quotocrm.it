<?php
      ...
      if($_REQUEST['idsito']){
         ...
         $query = "SELECT siti.idsito, siti.nome, siti.web,siti.https
            ...
            $EtichettaMenu   = 'Banner COVID-19';

            $TitoloBanner    = 'L\'ospitalità di '.$v['nome'].' dopo il Covid-19';
            $SottoTitoloBanner = 'Ecco la risposta di '.$v['nome'].' per garantire soggiorni ancor più sicuri e sereni!';
            $LinkBanner      = 'no-covid-19.php';
            $Titolo          = 'NO COVID-19';
            $SottoTitolo     = 'Senza rinunciare al piacere di un soggiorno all\'insegna del relax e del divertimento, in perfetto stile '.$v['nome'].'.';
            $TestoPrimario   = 'Abbiamo riesaminato molti aspetti della nostra ospitalità, in modo da poter garantire che ogni momento di un soggiorno all\' '.$v['nome'].' possa essere un\'esperienza piacevole e sicura.<br><br>La sicurezza e la serenità dei nostri Ospiti vengono al primo posto.';
            $TestoSecondario = 'Alcuni dei nostri accorgimenti riguardano ad esempio:
                                <br><br>
            ...
            $TitoloBannerENG    = 'The hospitality of '.$v['nome'].' after Covid-19';
            $SottoTitoloBannerENG = 'Here is the answer for '.$v['nome'].' to ensure even safer and more peaceful stays!';
            $LinkBannerENG      = '/en/no-covid-19.php';
            $TitoloENG          = 'NO COVID-19';
            $SottoTitoloENG     = 'Without giving up the pleasure of a stay of relaxation and fun, in perfect style '.$v['nome'].'.';
            $TestoPrimarioENG   = 'We have reviewed many aspects of our hospitality, so that we can ensure that every moment of your stay at '.$v['nome'].' can be a pleasant and safe experience. <br> <br> The safety and serenity of our Guests come first.';
            $TestoSecondarioENG = 'Some of our measures concern for example:
                                        <br><br>
            ...
            $TitoloBannerFRA    = 'L\'hospitalité de '.$v['nome'].' après Covid-19';
            $SottoTitoloBannerFRA = 'Voici la réponse pour '.$v['nome'].' pour assurer des séjours encore plus sûrs et plus paisibles!';
            $LinkBannerFRA      = '/fr/no-covid-19.php';
            $TitoloFRA          = 'NO COVID-19';
            $SottoTitoloFRA     = 'Sans renoncer au plaisir d\'un séjour de détente et de plaisir, dans un style parfait '.$v['nome'].'.';
            $TestoPrimarioFRA   = 'Nous avons passé en revue de nombreux aspects de notre hospitalité, afin que nous puissions nous assurer que chaque instant de votre séjour au '.$v['nome'].' peut être une expérience agréable et sûre. <br> <br> La sécurité et la sérénité de nos clients passent avant tout.';
            $TestoSecondarioFRA = 'Certaines de nos mesures concernent par exemple:
                                    <br><br>
        ...
        $TitoloBannerDEU    = 'Die Gastfreundschaft von '.$v['nome'].' nach Covid-19';
        $SottoTitoloBannerDEU = 'Hier ist die Antwort für '.$v['nome'].' um noch sicherere und friedlichere Aufenthalte zu gewährleisten!';
        $LinkBannerDEU      = '/de/no-covid-19.php';
        $TitoloDEU          = 'NO COVID-19';
        $SottoTitoloDEU     = 'Ohne auf das Vergnügen eines erholsamen und unterhaltsamen Aufenthalts in perfektem Stil zu verzichten '.$v['nome'].'.';
        $TestoPrimarioDEU   = 'Wir haben viele Aspekte unserer Gastfreundschaft überprüft, damit wir sicherstellen können, dass jeder Moment eines Aufenthalts im '.$v['nome'].' kann eine angenehme und sichere Erfahrung sein. <br> <br> Die Sicherheit und Gelassenheit unserer Gäste steht an erster Stelle.';
        $TestoSecondarioDEU = 'Einige unserer Maßnahmen betreffen zum Beispiel:
                                <br><br>
            ...
            $select = "SELECT * FROM comunicazioni_siti WHERE idsito = ".$_REQUEST['idsito']."";
            $rec = $db_suiteweb->query($select);
            $tot = sizeof($rec);

            if($tot==0){
    
                $insert = "INSERT INTO comunicazioni_siti(idsito,EtichettaMenu) VALUES('".$_REQUEST['idsito']."', '".addslashes($EtichettaMenu)."')";
                $db_suiteweb->query($insert);  
                $select = "SELECT Id FROM comunicazioni_siti ORDER BY ID DESC";
                $r      = $db_suiteweb->query($select);  
                $rec    = $r[0];
                $IdComunicazioneSiti = $rec['Id'];
                
                $rows     = $db_suiteweb->query('SELECT codlingua FROM lingue_siti WHERE lingue_siti.idsito = '.$_REQUEST['idsito'].' ORDER BY codlingua DESC');
 
                foreach($rows as $key => $value) {

                        switch($value['codlingua']){
                            case"it":
                                $TitB    = $TitoloBanner;
                                $SotTitB = $SottoTitoloBanner;
                                $LinkB   = $LinkBanner;
                                $Tit     = $Titolo;
                                $SotTit  = $SottoTitolo;
                                $TextP   = $TestoPrimario;
                                $TextS   = $TestoSecondario;
                            break;
                            case"en":
                                $TitB    = $TitoloBannerENG;
                                $SotTitB = $SottoTitoloBannerENG;
                                $LinkB   = $LinkBannerENG;
                                $Tit     = $TitoloENG;
                                $SotTit  = $SottoTitoloENG;
                                $TextP   = $TestoPrimarioENG;
                                $TextS   = $TestoSecondarioENG;
                            break;
                            case"fr":
                                $TitB    = $TitoloBannerFRA;
                                $SotTitB = $SottoTitoloBannerFRA;
                                $LinkB   = $LinkBannerFRA;
                                $Tit     = $TitoloFRA;
                                $SotTit  = $SottoTitoloFRA;
                                $TextP   = $TestoPrimarioFRA;
                                $TextS   = $TestoSecondarioFRA;
                            break;
                            case"de":
                                $TitB    = $TitoloBannerDEU;
                                $SotTitB = $SottoTitoloBannerDEU;
                                $LinkB   = $LinkBannerDEU;
                                $Tit     = $TitoloDEU;
                                $SotTit  = $SottoTitoloDEU;
                                $TextP   = $TestoPrimarioDEU;
                                $TextS   = $TestoSecondarioDEU;
                            break;
                            default:
                                $TitB    = $TitoloBannerENG;
                                $SotTitB = $SottoTitoloBannerENG;
                        ...
                        $ins = "INSERT INTO comunicazioni_siti_lang(idsito,
                                            IdComunicazioneSiti,
                                            Lingua,
                                            TitoloBanner,
                                            SottoTitoloBanner,
