<?php
    ...
    $xcrud->table('hospitality_infohotel');
    $xcrud->where('hospitality_infohotel.idsito', IDSITO);
  ...
  /* subselect per visualizzare le icone delle lingue relative ai testi presenti */
    $xcrud->subselect('Testi presenti','SELECT GROUP_CONCAT(DISTINCT CONCAT(Lingua," ") SEPARATOR " ") AS lingue FROM hospitality_infohotel_lang WHERE Id_infohotel = {Id}');
    $xcrud->column_callback('Testi presenti','show_flags');


    $xcrud->create_action('Attiva', 'abilita_info'); // action callback, function publish_action() in functions.php
    $xcrud->create_action('Disattiva', 'disabilita_info');
    ...
    $evento_lang = $xcrud->nested_table('Gestione testi in lingua','id','hospitality_infohotel_lang','Id_infohotel');
