<?php
	...
	if($_REQUEST['act'] == 'del'){
		...
		$s       = $db->query('SELECT ricevuta,id_richiesta FROM hospitality_altri_pagamenti  WHERE Id = '.$_REQUEST['id']);
		$rs      = $db->row($s);
		$rice    = $rs['ricevuta'];
		$id_rich = $rs['id_richiesta'];
		if (file_exists(BASE_PATH_SITO.'uploads/'.IDSITO.'/'.$rice)) {
			unlink(BASE_PATH_SITO.'uploads/'.IDSITO.'/'.$rice);
			$db->query("UPDATE hospitality_altri_pagamenti SET ricevuta = '' WHERE Id = ".$_REQUEST['id']);
		}
		header('Location:'.BASE_URL_SITO.'pagamento/&id_richiesta='.$id_rich);
	}


	$db->query('SELECT hospitality_altri_pagamenti.*,hospitality_guest.Nome,hospitality_guest.Cognome,hospitality_guest.NumeroPrenotazione,
	...
	if($ricevuta != ''){
		...
		$upload = '<div class="input-group">
						<label for="upload">
							<span class="form-control" style="cursor:pointer">
								<i class="fa fa-fw fa-save"></i>
								<small id="c">Clicca qui!</small>
								<small id="y"></small>
							</span>
							<input type="file" id="upload" name="ricevuta" class="form-control" style="display:none;border:0px">
						</label>
						<script>
							$("#upload").click(function(){
								$("i.fa-save").addClass("text-red");
								$("#c").hide();
								$("#y").html(\'<span class="text-gray">ora clicca su salva</span>\');
							});
							</script>
					</div>';
		}
