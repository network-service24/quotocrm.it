...
/*     $meno_1mese = mktime (0,0,0,(date('m')-1),date('d'),date('Y'));
    ...
    $filter_query = " AND hospitality_guest.DataRichiesta>= '".$prima_data."' AND hospitality_guest.DataRichiesta <= '".$seconda_data."' ";
...
if($tot>0){
    ...
    foreach ($array_fatturato as $k => $v) {
        ...
        $legenda .= '<div class="row">';
        $legenda .= '<div class="col-md-6"><label class="badge" style="background-color:'.$color.'">'.$label.'</label>'.($abilitato==0?' <small>(non attivo)</small>':'').'</div><div class="col-md-6"><i class="fa fa-euro"></i> '.number_format((strtolower($k)=='sito web'?($v-$FattLanding):$v),2,',','.').'</div>';
        $legenda .= '</div>';
...
$selectLP = "SELECT * FROM hospitality_fonti_provenienza WHERE idsito = ".IDSITO." AND Provenienza ='' AND Timeline = ''";
$resLP = $db->query($selectLP);
$rwsLP = $db->result($resLP);
$totLP = sizeof($rwsLP);
if($totLP>0){
    foreach ($rwsLP as $keyLP => $valueLP) {
        $NumeriPrenoLP[] = $valueLP['NumeroPrenotazione'];
    }
       $NumeriLP = implode(',',$NumeriPrenoLP);

         $selectLP_ = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                    FROM hospitality_guest
                                    INNER JOIN hospitality_proposte ON hospitality_proposte.id_richiesta = hospitality_guest.Id
                                    WHERE hospitality_guest.NumeroPrenotazione IN (".$NumeriLP.")
                                    ".$filter_query ."
                                    AND hospitality_guest.idsito = ".IDSITO."
                                    ...
                                    AND hospitality_guest.Hidden = 0
                                    AND hospitality_guest.TipoRichiesta = 'Conferma' ";
            $resLP_ = $db->query($selectLP_);
            $rwsLP_ = $db->row($resLP_);
            $fatturatoLP = $rwsLP_['fatturato'];
            if($fatturatoLP == '')$fatturatoLP = 0;
            $array_fatturatoS['LandingPage']  = $fatturatoLP;
}

foreach($array_fatturato as $chiave => $valore){
    if($chiave == 'Sito Web_1'){
      $totale_sito = $valore;
    }
}
...
if(!empty($array_fatturatoS) || !is_null($array_fatturatoS)){
    ...
    foreach ($array_fatturatoS as $y => $val) {
        ...
        if($val!="" || $val != 0){
            ...
            if($totale_sito > $totaleS){
              if($y == 'Referral/Altro' &&  $array_fatturatoS['Referral/Altro']>0){
                $val = ($val + ($totale_sito - $totaleS));
                $totaleS = ($totaleS + ($totale_sito - $totaleS));
              }elseif($array_fatturatoS['Organico']>0 && $array_fatturatoS['Diretto']<=0  && $array_fatturatoS['Facebook']<=0  && $array_fatturatoS['Newsletter']<=0 && $array_fatturatoS['PPC']<=0){
                $val = ($val + ($totale_sito - $totaleS));
                $totaleS = ($totaleS + ($totale_sito - $totaleS));
              }
            }elseif($totale_sito < $totaleS){
                if($y == 'Referral/Altro' &&  $array_fatturatoS['Referral/Altro']>0){
                  if($val > ($totaleS - $totale_sito)){
                    $val = ($val - ($totaleS - $totale_sito));
                  }else{
                    $val = (($totaleS - $totale_sito) - $val);
                  }
                  $totaleS = ($totaleS - ($totaleS - $totale_sito));
                }elseif($array_fatturatoS['Organico']>0 && $array_fatturatoS['Referral/Altro']<=0 ){
                  $val = ($val + ($totale_sito - $totaleS));
                  $totaleS = ($totaleS + ($totale_sito - $totaleS));
                }
            }
...
if($_REQUEST['action']=='check_date'){
    ...
    $res3 = $db->query($select3);
    $rws3 = $db->result($res3);
    $totOrganico2 = sizeof($rws3);
    if($totOrganico2>0){
        foreach ($rws3 as $key3 => $value3) {
            $NumeriPreno2[] = $value3['NumeroPrenotazione'];
        }
            $NumeriO2 = implode(',',$NumeriPreno2);

             $select4 = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                        FROM hospitality_guest
                                        INNER JOIN hospitality_proposte ON hospitality_proposte.id_richiesta = hospitality_guest.Id
                                        WHERE hospitality_guest.NumeroPrenotazione IN (".$NumeriO2.")
                                        ".$filter_query2 ."
                                        AND hospitality_guest.idsito = ".IDSITO."
                ...
                $res4 = $db->query($select4);
                $rws4 = $db->row($res4);
                $fatturatoOrganico2 = $rws4['fatturato'];
                if($fatturatoOrganico2 == '')$fatturatoOrganico2 = 0;
                $array_fatturatoS2['Organico']  = $fatturatoOrganico2;
    }
    $select5 = "SELECT * FROM hospitality_fonti_provenienza WHERE idsito = ".IDSITO." AND Provenienza LIKE '%".SITOWEB."%' AND Provenienza NOT LIKE '%gclid%' AND Provenienza NOT LIKE '%utm_source%' AND Provenienza NOT LIKE '%facebook%' AND Timeline NOT LIKE '%gclid%'";
    $res5 = $db->query($select5);
    $rws5 = $db->result($res5);
    $totDirect2 = sizeof($rws5);
    if($totDirect2>0){
        foreach ($rws5 as $key5 => $value5) {
            $NumeriPrenoD2[] = $value5['NumeroPrenotazione'];
        }
           $NumeriD2 = implode(',',$NumeriPrenoD2);

             $select6 = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                        FROM hospitality_guest
                                        INNER JOIN hospitality_proposte ON hospitality_proposte.id_richiesta = hospitality_guest.Id
                                        WHERE hospitality_guest.NumeroPrenotazione IN (".$NumeriD2.")
                                        ".$filter_query2 ."
                                        AND hospitality_guest.idsito = ".IDSITO."
                ...
                $res6 = $db->query($select6);
                $rws6 = $db->row($res6);
                $fatturatoDirect2 = $rws6['fatturato'];
                if($fatturatoDirect2 == '')$fatturatoDirect2 = 0;
                $array_fatturatoS2['Diretto']  = $fatturatoDirect2;
    }
    $select7 = "SELECT * FROM hospitality_fonti_provenienza WHERE idsito = ".IDSITO." AND (Provenienza LIKE '%facebook%'  OR Timeline LIKE '%facebook%')";
    $res7 = $db->query($select7);
    $rws7 = $db->result($res7);
    $totSocial2 = sizeof($rws7);
    if($totSocial2>0){
        foreach ($rws7 as $key7 => $value7) {
            $NumeriPrenoS2[] = $value7['NumeroPrenotazione'];
        }
           $NumeriS2 = implode(',',$NumeriPrenoS2);

             $select8 = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                        FROM hospitality_guest
                                        INNER JOIN hospitality_proposte ON hospitality_proposte.id_richiesta = hospitality_guest.Id
                                        WHERE hospitality_guest.NumeroPrenotazione IN (".$NumeriS2.")
                                       ".$filter_query2 ."
                                        AND hospitality_guest.idsito = ".IDSITO."
                ...
                $res8 = $db->query($select8);
                $rws8 = $db->row($res8);
                $fatturatoSocial2 = $rws8['fatturato'];
                if($fatturatoSocial2 == '')$fatturatoSocial2 = 0;
                $array_fatturatoS2['Facebook']  = $fatturatoSocial2;
    }
    $select9 = "SELECT * FROM hospitality_fonti_provenienza WHERE idsito = ".IDSITO." AND (Provenienza LIKE '%utm_medium%' OR Timeline LIKE '%utm_medium%')";
    $res9 = $db->query($select9);
    $rws9 = $db->result($res9);
    $totMail2 = sizeof($rws9);
    if($totMail2>0){
        foreach ($rws9 as $key9 => $value9) {
            $NumeriPrenoM2[] = $value9['NumeroPrenotazione'];
        }
           $NumeriM2 = implode(',',$NumeriPrenoM2);

             $select10 = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                        FROM hospitality_guest
                                        INNER JOIN hospitality_proposte ON hospitality_proposte.id_richiesta = hospitality_guest.Id
                                        WHERE hospitality_guest.NumeroPrenotazione IN (".$NumeriM2.")
                                        ".$filter_query2 ."
                                        AND hospitality_guest.idsito = ".IDSITO."
                ...
                $res10 = $db->query($select10);
                $rws10 = $db->row($res10);
                $fatturatoMail2 = $rws10['fatturato'];
                if($fatturatoMail2 == '')$fatturatoMail2 = 0;
                $array_fatturatoS2['Newsletter']  = $fatturatoMail2;
    }
    $select11 = "SELECT * FROM hospitality_fonti_provenienza WHERE idsito = ".IDSITO." AND (Provenienza LIKE '%gclid%' OR Timeline LIKE '%gclid%')";
    $res11 = $db->query($select11);
    $rws11 = $db->result($res11);
    $totPPC2 = sizeof($rws11);
    if($totPPC2>0){
        foreach ($rws11 as $key11 => $value11) {
            $NumeriPrenoPPC2[] = $value11['NumeroPrenotazione'];
        }
           $NumeriPPC2 = implode(',',$NumeriPrenoPPC2);

             $select12 = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                                        FROM hospitality_guest
                                        INNER JOIN hospitality_proposte ON hospitality_proposte.id_richiesta = hospitality_guest.Id
                                        WHERE hospitality_guest.NumeroPrenotazione IN (".$NumeriPPC2.")
                                        ".$filter_query2 ."
                                        AND hospitality_guest.idsito = ".IDSITO."
                ...
                $res12 = $db->query($select12);
                $rws12 = $db->row($res12);
                $fatturatoPPC2 = $rws12['fatturato'];
                if($fatturatoPPC2 == '')$fatturatoPPC2 = 0;
                $array_fatturatoS2['PPC']  = $fatturatoPPC2;
    }
    $select13 = "SELECT * FROM hospitality_fonti_provenienza WHERE idsito = ".IDSITO." AND Provenienza NOT LIKE '%facebook%' AND Provenienza NOT LIKE '%".SITOWEB."%' AND Provenienza NOT LIKE '%google%' AND Timeline NOT LIKE '%gclid%' AND Timeline NOT LIKE '%utm_medium=email%'";
    $res13 = $db->query($select13);
    $rws13 = $db->result($res13);
    $totOTH2 = sizeof($rws13);
    if($totOTH2>0){
        foreach ($rws13 as $key13 => $value13) {
            $NumeriPrenoOTH2[] = $value13['NumeroPrenotazione'];
        }
           $NumeriOTH2 = implode(',',$NumeriPrenoOTH2);
             ...
             $select14 = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                ...
                $res14 = $db->query($select14);
                $rws14 = $db->row($res14);
                $fatturatoOTH2 = $rws14['fatturato'];
                if($fatturatoOTH2 == '')$fatturatoOTH2 = 0;
                $array_fatturatoS2['Referral/Altro']  = $fatturatoOTH2;
    }

    foreach($array_fatturato2 as $chiave2 => $valore2){
        if($chiave2 == 'Sito Web_1'){
          $totale_sito2 = $valore2;
        }
    }

    if(!empty($array_fatturatoS2) || !is_null($array_fatturatoS2)){
        $x = '1';
        foreach ($array_fatturatoS2 as $y2 => $val2) {
            ...
            if($val2 !="" || $val2 != 0){
              ...
              //###MODIFICA PER SINCRONIZZARE FATTURATO
              if($totale_sito2 > $totaleS2){
                if($y2 == 'Referral/Altro' &&  $array_fatturatoS2['Referral/Altro']>0){
                  $val2 = ($val2 + ($totale_sito2 - $totaleS2));
                  $totaleS2 = ($totaleS2 + ($totale_sito2 - $totaleS2));
                }elseif($array_fatturatoS2['Organico']>0 && $array_fatturatoS2['Diretto']<=0  && $array_fatturatoS2['Facebook']<=0  && $array_fatturatoS2['Newsletter']<=0 && $array_fatturatoS2['PPC']<=0){
                  $val2 = ($val2 + ($totale_sito2 - $totaleS2));
                  $totaleS2 = ($totaleS2 + ($totale_sito2 - $totaleS2));
                }
              }elseif($totale_sito2 < $totaleS2){
                  if($y2 == 'Referral/Altro' &&  $array_fatturatoS2['Referral/Altro']>0){
                    if($val2 > ($totaleS2 - $totale_sito2)){
                      $val2 = ($val2 - ($totaleS2 - $totale_sito2));
                    }else{
                      $val2 = (($totaleS2 - $totale_sito2) - $val2);
                    }
                    $totaleS2 = ($totaleS2 - ($totaleS2 - $totale_sito2));
                  }elseif($array_fatturatoS2['Organico']>0 && $array_fatturatoS2['Referral/Altro']<=0 ){
                    $val2 = ($val2 + ($totale_sito2 - $totaleS2));
                    $totaleS2 = ($totaleS2 + ($totale_sito2 - $totaleS2));
                  }
              }
              //#############################
                ...
                $etichette_S2[] = "'".$y2."'";
                $tortaS2[] = "{value: ".$val2.", name: '".$y2."'}";

                $legendaS2 .= '<div class="row">';
                $legendaS2 .= '<div class="col-md-6"><label class="badge" style="background-color:'.$colorS2.'">'.str_replace("http://","",$y2).'</label></div><div class="col-md-6"><i class="fa fa-euro"></i> '.number_format($val2,2,',','.').'</div>';
                $legendaS2 .= '</div>';
            }
        $x++;
        }
            $legendaS2 .= '<div class="row">';
            $legendaS2 .= '<div class="col-md-6"><b>TOTALE</b></div><div class="col-md-6"><i class="fa fa-euro"></i> '.number_format($totaleS2,2,',','.').'</div>';
            $legendaS2 .= '</div>';
