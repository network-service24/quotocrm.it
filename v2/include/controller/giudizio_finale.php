...
if($_REQUEST['azione']!='' && $_REQUEST['param']=='delete'){
        ...
        $db->query("UPDATE hospitality_guest SET CS_inviato = 0 WHERE Id = ".$_REQUEST['azione']);  
        $db->query("DELETE FROM hospitality_recensioni_send WHERE id_richiesta = " . $_REQUEST['azione']);   
...
$anniprima = (date('Y')-$diff_anni);
    ...
    $results2 = $db->query($select2);
    $record   = $db->result($results2);
    if(sizeof($record)>0){
    $tabella .= '
                    <style>
                        .span{
                    ...
                    <h2 id="bordato"> 
                    <span id="etichetta" class="span ">Valutazioni medie per Ospite</span>
                    <a id="collegamento" class="collapsed arrow_down " data-toggle="collapse" data-parent="#accordion" href="#pannello-2">
                        <i class="glyphicon glyphicon-chevron-down" style="font-size:12px;color:#000"></i>
                        
                    </a>
                </h2>
                <td id="pannello-2" class="panel-collapse in ">';
    $tabella .= '<style>.ordinamento{display:none}</style><table id="TabellaLayout" class="xcrud-list table table-striped table-hover table-bordered table-sm">
                    <thead>
                        <tr class="xcrud-th">
                            <th class="th-sm text-center">Rif.Preno</th>
                            <th class="th-sm">Ospite</th>
                            <th class="th-sm">Lingua</th>
                            <th class="th-sm">Valutazione Media</th>
                            <th class="th-sm">Recensione</th>
                            <th class="th-sm">Data Compilazione</th>
                            <th class="th-sm">Chiama Ospite</th>
                            <th class="th-sm text-center">Invita ospite alla recensione</th>
                            <th class="th-sm text-center">Elimina</th>
                        </tr>
                    </thead>'."\r\n";
    $risposte  = '';
    foreach($record as $key => $rw){
       ...
       $sel        = "SELECT hospitality_customer_satisfaction.*,hospitality_domande.Domanda
        ...
        $ro        = $db->result($re);
        if(sizeof($ro)>0){
            $link = '<a href="javascript:;" data-toggle="modal" data-target="#risposta'.$rw['id_richiesta'].'"><i class="fa fa-comment" aria-hidden="true"></i></a>';
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        ...
                                        <h4 class="modal-title" id="myModalLabel">Recensione '.$utente.' prenotazione N° '.$rec['NumeroPrenotazione'].'</h4>
                                        LEGENDA:    <img src="'.BASE_URL_SITO.'img/emoji/bad.jpg" style="width:14px;height:14px" data-toogle="tooltip" title="Bad [valore = 1]"> 
                                                    <img src="'.BASE_URL_SITO.'img/emoji/semi_bad.jpg" style="width:14px;height:14px"data-toogle="tooltip" title="Semi Bad [valore = 2]">
                                                    <img src="'.BASE_URL_SITO.'img/emoji/medium.jpg" style="width:14px;height:14px" data-toogle="tooltip" title="Medium [valore = 3]">
                                                    <img src="'.BASE_URL_SITO.'img/emoji/semi_good.jpg" style="width:14px;height:14px" data-toogle="tooltip" title="Semi Good [valore = 4]">
                                                    <img src="'.BASE_URL_SITO.'img/emoji/good.jpg" style="width:14px;height:14px" data-toogle="tooltip" title="Good [valore = 5]">
                                ...
                                <div class="modal-body">
                                    ...
                                    foreach ($ro as $ki => $valore) {
                                        $modale .= '<tr>';                                      
                                            $modale .= '<td>'.$valore['Domanda'].'</td>';
                                            $modale .= '<td class="text-center">'.func_stars($valore['recensione']).'</td>';
        ...
        $tabella .= '  <tr>
                            ...
                            <td>'.stripslashes($utente).'</td>
                            <td><img src="'.BASE_URL_SITO.'img/flags/mini/'.$rec['Lingua'].'.png"></td>
                            <td>
                                Ricevuti &nbsp;&nbsp;<b class="text-green">'.$rw['totale_recensione'].'</b> <small>punti</small>&nbsp;&nbsp; su un totale di &nbsp;&nbsp;
                                <b class="text-orange">'.$valuemax.'</b> <small>punti</small>&nbsp;&nbsp;  media del &nbsp;&nbsp;
                                <b class="text-red">'.$media.'</b>&nbsp;&nbsp;su <b class="text-purple">5</b>
                            </td>
                            ...
                            <td class="text-center">
                                <a href="'.BASE_URL_SITO.'scrivi_a/'.$rw['id_richiesta'].'/"><i class="fa fa-envelope '.(($invii!='' && $invii!=0)?'text-red':'').'" data-toogle="tooltip" title="'.(($invii!='' && $invii!=0)?'Richiesta recensione inviata N° '.$invii.' in data '. $Data_invio.'':'Invia richiesta recensione').'"></i></a>
                            </td>
                            <td class="text-center">
...
$js_grafico .= '<script src="'.BASE_URL_SITO.'material/assets/plugins/echarts/echarts-all.js"></script>'."\r\n";
$js_grafico .="
    <script>
        $(function () {
            // ============================================================== 
