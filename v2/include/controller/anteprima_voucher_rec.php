...
if($tot > 0){
    ...
    $Operatore          = addslashes($rows['ChiPrenota']);
              ...
              if($NomeProposta!='' || $TestoProposta!=''){
               ...
               $select2 = "SELECT hospitality_richiesta.NumeroCamere,
                ...
                foreach ($res2 as $ky => $val) {
                    ...
                    $FCamere .= $val['TitoloSoggiorno'].' - Nr. '.$val['NumeroCamere'].' '.$val['TipoCamere'].' '.($DataRichiestaCheck > DATA_QUOTO_V2 ?($NumAdulti!=0?'A.'.$NumAdulti:'').' '.($NumBambini!=0?'B.'.$NumBambini:'').' '.($EtaB!=''?'<small>'.ETA.' '.$EtaB.'</small>':''):'').'- €. '.number_format($val['Prezzo'],2,',','.').' - ';
                
                    $VAUCHERCamere .= '<p>'.$val['TitoloSoggiorno'].' <i class=\'fa fa-angle-right\'></i> Nr. '.$val['NumeroCamere'].' '.$val['TipoCamere'].' '.($DataRichiestaCheck > DATA_QUOTO_V2 ?($NumAdulti!=0?$ico_adulti:'').' '.($NumBambini!=0?$ico_bimbi:'').' '.($NumBambini!=0?($EtaB!=''?''.ETA.' '.$EtaB.' ':''):''):'').' - €. '.number_format($val['Prezzo'],2,',','.').'</p>';
...
if($_REQUEST['azione']!=''){
    if($TipoRichiesta == 'Conferma' && $Chiuso == 1 && $Archivia == 0){
      $TornaA = BASE_URL_SITO.'prenotazioni/';
      $Etichetta = 'alle Prenotazioni';
    }
    if($_DataValiditaVoucher != '' && $DataRiconferma == ''){
      $TornaA = BASE_URL_SITO.'buoni_voucher/';
      $Etichetta = 'ai Buoni Voucher';
    }
