...
if($_GET['azione'] == 'send' && $_GET['param'] != '') {
    ...
    if($_REQUEST['valore']=='no'){  
        ...
        $update = "UPDATE hospitality_guest SET Voucher_send = 0, Chiuso = 1, Visibile = 1, DataChiuso = '".date('Y-m-d H:i:s')."' WHERE Id = ".$_GET['param'];
        $db->query($update);
       ...
       $link = (URL_LANDING.$directory_sito.'/'.base64_encode($IdRichiesta.'_'.IDSITO.'_c').'/voucher/');
        ...
        if (!$mail->send()) {
                ...
                $prt->_goto(BASE_URL_SITO.'prenotazioni/vau/ok'); 
                 //header('Location:'.BASE_URL_SITO.'prenotazioni/vau/ok');
