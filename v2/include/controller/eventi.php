<?php
    ...
    $xcrud->table('hospitality_eventi');
    $xcrud->where('hospitality_eventi.idsito', IDSITO);
    $xcrud->order_by('hospitality_eventi.DataInizio', 'ASC');
    ...
    $xcrud->columns('Evento,Immagine,DataInizio,DataFine,Testi presenti, Abilitato');
    $xcrud->fields('Evento,Immagine,DataInizio,DataFine,OraInizio,OraFine,Indirizzo,Coordinate, Abilitato');

    $xcrud->validation_required('Evento',3);
    $xcrud->validation_required('DataInizio');
    $xcrud->validation_required('DataFine');

  /* subselect per visualizzare le icone delle lingue relative ai testi presenti */
    $xcrud->subselect('Testi presenti','SELECT GROUP_CONCAT(DISTINCT CONCAT(Lingua," ") SEPARATOR " ") AS lingue FROM hospitality_eventi_lang WHERE Id_eventi = {Id}');
    $xcrud->column_callback('Testi presenti','show_flags');


    $xcrud->create_action('Attiva', 'abilita_evento'); // action callback, function publish_action() in functions.php
    $xcrud->create_action('Disattiva', 'disabilita_evento');
    ...
    $evento_lang = $xcrud->nested_table('Gestione testi in lingua','id','hospitality_eventi_lang','Id_eventi');
