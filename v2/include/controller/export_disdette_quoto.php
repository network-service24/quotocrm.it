...
if($_REQUEST['action']=='export'){
    ...
    if($tot > 0){
        ...
        fputcsv($output, array('Nr.Prenotazione', 'Nome', 'Cognome', 'Email', 'Data Prenotazione', 'Motivazione', 'Data invio email disdetta', 'Data validita voucher', 'Caparra', 'Cifra a saldo', 'Prezzo Ssoggiorno'),';');
        ...
        foreach ($rec as $key => $val) {
                ...
                if($val['DataVoucherRecSend']!='' && $val['DataVoucherRecSend']!='0000-00-00'){
                    ...
                    $valori = array($val['NumeroPrenotazione'],$val['Nome'],$val['Cognome'],$val['Email'],gira_data($val['DataChiuso']),$val['Motivazione'],gira_data($val['DataVoucherRecSend']),gira_data($val['DataValiditaVoucher']),($acconto!=''?$acconto:'00,0'),($acconto!=''?$acconto:'00,0'),number_format(floatval($saldo),2,',','.'),$PrezzoP);
