...
############ CALCOLO RICHIESTE RICEVUTE ############


            $select = "SELECT distinct(hospitality_tracking_ads.Campagna) as campagne_confermate
                        ...
                        AND hospitality_tracking_ads.Tracking = 'facebook'
                        AND hospitality_tracking_ads.Campagna != ''
               ...
               $res = $db->query($select);
               $rws = $db->result($res);
               foreach ($rws as $ky => $val) {
                    $arr_camp[] = $val['campagne_confermate'];
                }
                if(empty($arr_camp) || is_null($arr_camp)){
                    $arr_camp[] = ''; 
                }
        $selectBis = "SELECT distinct(hospitality_tracking_ads.Campagna) as campagne_ricevute
                        FROM hospitality_tracking_ads
                        INNER JOIN hospitality_guest ON hospitality_guest.NumeroPrenotazione = hospitality_tracking_ads.NumeroPrenotazione
                        WHERE 1 = 1
                        ".$filter_query_r ."
                        ...
                        AND hospitality_guest.DataChiuso IS NULL
                        AND hospitality_guest.Disdetta = 0
                        AND hospitality_guest.Hidden = 0 
                        GROUP BY hospitality_tracking_ads.Campagna";
        $resBis = $db->query($selectBis);
        $rwsBis = $db->result($resBis);

        $tot = sizeof($rwsBis);

        if($tot > 0){
       
            foreach ($rwsBis as $key => $value) {

                if(!in_array($value['campagne_ricevute'],$arr_camp)){
                    $legendaSn_BOX2 .= '<tr>
                                            <td class="text-left wrap">'.str_replace("http://","",$value['campagne_ricevute']).'</td>
                                            <td class="text-center">'.numero_campagne_da_(IDSITO,'facebook',$value['campagne_ricevute'],'landing').'</td>   
                                            <td class="text-center">'.numero_campagne_da_(IDSITO,'facebook',$value['campagne_ricevute'],'sito').'</td>  
...
############ FIEN CALCOLO RICHIESTE RICEVUTE ############
 ...
 $sqln = "SELECT hospitality_tracking_ads.NumeroPrenotazione
  ...
  if($totaleFBn > 0){
    ...
    $select9n = "SELECT hospitality_tracking_ads.Campagna
                    FROM hospitality_tracking_ads
                    WHERE hospitality_tracking_ads.idsito = ".IDSITO."
                    AND hospitality_tracking_ads.Tracking = 'facebook'
                    AND Campagna != ''";
    ...
    if($totCampn>0){
        ...
        foreach ($array_campagnen as $keyn => $valuen) {
            ...
            $select10n = "SELECT SUM(hospitality_proposte.PrezzoP) as fatturato
                        ...
                        WHERE 1=1
                        ".$filter_query ."
                        AND hospitality_tracking_ads.Campagna = '".$valuen."' 
                        AND hospitality_tracking_ads.idsito = ".IDSITO."
                        AND hospitality_guest.FontePrenotazione = 'Sito Web'
                        AND hospitality_guest.Chiuso = 1
                        AND hospitality_guest.Disdetta = 0
               ...
               if($Check_campn > 0){
                 $fatturatoCampn = $rws10n['fatturato'];
                  if($fatturatoCampn == '')$fatturatoCampn = 0;
                    $array_fatturatoSn[$valuen]  = $fatturatoCampn;
        ...
        foreach ($array_fatturatoSn as $yn => $valn) {
            ...
            if($valn!="" || $valn != 0){
                ...
                $clean_campagnan = str_replace('-',' ',$yn);
                ...
                $legendaSn_BOX .= '<tr>
                                        <td class="text-left wrap">'.str_replace("http://","",$clean_campagnan).'</td>
                                        <td class="text-center">'.numero_campagne_da_(IDSITO,'facebook',$yn,'landing').'</td>   
                                        <td class="text-center">'.numero_campagne_da_(IDSITO,'facebook',$yn,'sito').'</td>  
                                        <td class="text-center">'.n_campagne_chiuse(IDSITO,'facebook',$yn).'</td>
