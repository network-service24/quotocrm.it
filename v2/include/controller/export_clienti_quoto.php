...
if($_REQUEST['action']=='export'){
    ...
    fputcsv($output, array('Tipo Richiesta', 'Fonte Prenotazione', 'Target', 'Lingua', 'Nome', 'Cognome', 'Email','Telefono', 'Data Arrivo', 'Data Partenza','Adulti','Bambini', 'Data Prenotazione', 'Prenotazione Confermata', 'Prenotazione Disdetta', 'Motivazione Disdetta', ' P. Annullata', 'Motivazione Annullamento', 'Questionario', 'Consenso Trattamento dati', 'Consenso invio Marketing'),';');
    ...
    if($_REQUEST['DataPrenotazione_dal']!='' && $_REQUEST['DataPrenotazione_al'] ==''){
        ...
        $q .= " AND hospitality_guest.DataChiuso >= '".$dataP_dal."' ";
      }
    if($_REQUEST['DataPrenotazione_dal']!='' && $_REQUEST['DataPrenotazione_al'] !=''){
        $dataP_dal_tmp = explode("/",$_REQUEST['DataPrenotazione_dal']);
        $dataP_dal = $dataP_dal_tmp[2].'-'.$dataP_dal_tmp[1].'-'.$dataP_dal_tmp[0];
        ...
        $q .=" AND hospitality_guest.DataChiuso >= '".$dataP_dal."'  AND hospitality_guest.DataChiuso <= '".$dataP_al."'";
    }
    ...
    if($_REQUEST['Arrivo_dal']!='' && $_REQUEST['Arrivo_al'] ==''){
        ...
        $q .= " AND hospitality_guest.DataArrivo >= '".$dataA_dal."' ";
      }
    if($_REQUEST['Arrivo_dal']!='' && $_REQUEST['Arrivo_al'] !=''){
        $dataA_dal_tmp = explode("/",$_REQUEST['Arrivo_dal']);
        $dataA_dal = $dataA_dal_tmp[2].'-'.$dataA_dal_tmp[1].'-'.$dataA_dal_tmp[0];
        ...
        $q .=" AND hospitality_guest.DataArrivo >= '".$dataA_dal."'  AND hospitality_guest.DataArrivo <= '".$dataA_al."'";
    }
    ...
    if($_REQUEST['DataRichiesta_dal']!='' && $_REQUEST['DataRichiesta_al'] ==''){
        ...
        $q .=" AND hospitality_guest.DataRichiesta >= '".$dataR_dal."'";
      }
      if($_REQUEST['DataRichiesta_dal']!='' && $_REQUEST['DataRichiesta_al'] !=''){
          $dataR_dal_tmp = explode("/",$_REQUEST['DataRichiesta_dal']);
          $dataR_dal = $dataR_dal_tmp[2].'-'.$dataR_dal_tmp[1].'-'.$dataR_dal_tmp[0];
          ...
          $q .=" AND hospitality_guest.DataRichiesta >= '".$dataR_dal."' AND hospitality_guest.DataRichiesta <= '".$dataR_al."'";
      }   
