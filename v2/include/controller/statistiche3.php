...
if ($_REQUEST['querydate'] == '' && $_REQUEST['action'] == '') {
    ...
    $filter_query = " AND ((hospitality_guest.DataRichiesta >= '$prima_data' AND hospitality_guest.DataRichiesta <= '$seconda_data' ) OR (date(hospitality_guest.DataChiuso) is not null AND date(hospitality_guest.DataChiuso) >= '$prima_data' AND date(hospitality_guest.DataChiuso) <= '$seconda_data' ))";
    ...
    foreach ($array_fatturatoS as $y => $val) {
        ...
        if ($val['fatturato'] != "" || $val['fatturato'] != 0) {
            ...
            if ($val['source'] == 'facebook' && $val['medium'] == 'social') {
                $linkFB = '<a href="' . BASE_URL_SITO . 'grafici-facebook_ads_new/" style="margin-left:20px" data-toggle="tooltip" title="Dettaglio Campagne FaceBook"><i class="fa fa-external-link fa-flip-horizontal"></i></a>';
            } else {
                $linkFB = '';
            }
            if ($val['source'] == 'google' && $val['medium'] == 'cpc') {
                $linkCPC = '<a href="' . BASE_URL_SITO . 'grafici-ppc_ads_new/" style="margin-left:20px" data-toggle="tooltip" title="Dettaglio Campagne Google"><i class="fa fa-external-link fa-flip-horizontal"></i></a>';
            } else {
                $linkCPC = '';
            }
            if ($val['source'] == 'newsletter' && $val['medium'] == 'email') {
                $linkNE = '<a href="' . BASE_URL_SITO . 'grafici-newsletter_ads_new/" style="margin-left:20px" data-toggle="tooltip" title="Dettaglio Campagne Newsletter"><i class="fa fa-external-link fa-flip-horizontal"></i></a>';
            } else {
                $linkNE = '';
            }
            ...
            $legendaS .= '<script>
                                ...
                                $(document).ready(function () {
                                    $(\'#PrenotazioniSitoWebS' . $clean_souce . '' . $clean_med . '\').on("shown.bs.modal",function(){
                                        ...
                                        $("#contentS' . $clean_souce . '' . $clean_med . '").load("' . BASE_URL_SITO . 'ajax/' . ($clean_med == 'referral' ? 'dett_preno_referral_sitoweb.php' : 'dett_preno_sitoweb.php') . '?idsito=' . IDSITO . '&sorgente="+referral+"&filtroQuery=' . urlencode($filter_query) . '", function() {
...
$rws15_ = $db->query($select15);
$rws15 = $db->result($rws15_);
...
if ($totOperatore > 0) {
    ...
    $fatturatoTotaleQuotoOP = tot_fatturato(1);
...
$rws18_ = $db->query($select18);
$rws18 = $db->result($rws18_);
...
if ($totTARGET > 0) {
    ...
    $fatturatoTotaleQuotoT = tot_fatturato(1);
...
$rws20_ = $db->query($select20);
$rws20 = $db->result($rws20_);
...
if ($totTemplate > 0) {
    ...
    $fatturatoTotaleQuotoTP = tot_fatturato(1);
...
if ($_REQUEST['action'] == 'check_date') {
    ...
    $rwsTemp_ = $db->query($selectTemp);
    $rwsTemp = $db->result($rwsTemp_);
