<?php
  $select = "SELECT * FROM hospitality_recensioni_range WHERE idsito = ".IDSITO;
  $res    = $db->query($select);
    $rw     = $db->row($res);
    if(is_array($rw)) {
        if($rw > count($rw))
     ...
     if($tot == 0){
        $insert = "INSERT INTO hospitality_recensioni_range(idsito) VALUES('".IDSITO."')";
        $ins    = $db->query($insert);
        $id     = $db->insert_id($ins);
    }else{
        $id = $rw['id'];
    ...
    $xcrud->table('hospitality_recensioni_range');
    $xcrud->where('hospitality_recensioni_range.idsito', IDSITO);                    
    ...
    $xcrud2->table('hospitality_recensioni_range');
    $xcrud2->where('hospitality_recensioni_range.idsito', IDSITO);                    

    $xcrud2->pass_var('idsito',IDSITO);

    $xcrud2->columns('punteggio_da,punteggio_a', false);
    $xcrud2->fields('punteggio_da,punteggio_a', false);

    $xcrud2->label('punteggio_da', 'Abilita range di punteggio DA');
    $xcrud2->label('punteggio_a', 'Abilita range di punteggio A');

    $xcrud2->change_type('punteggio_da','select','','1,2,3,4,5');
    $xcrud2->change_type('punteggio_a','select','','1,2,3,4,5');

    $xcrud2->table_name('<small>Imposta un Range di Punteggio perchè l\'email di Richiesta Recensioni su TripAdvisor, parta automaticamente!</small>');
    $xcrud2->unset_print();
    $xcrud2->unset_add();
    $xcrud2->unset_remove();
    $xcrud2->unset_view();
