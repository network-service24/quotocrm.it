...
/* check sul campo email in tempo reale, se email esiste CNAME */
	...
	if(sizeof($arr_r)>0){
		...
		$iscritti .= '<h2>LISTA UTENTI '.NOME_CLIENT_EMAIL.'</h2>
						...
						<thead>
							<tr class="xcrud-th">
								<th class="th-sm">Identificativo</th>
								...
								<th class="th-sm text-center">Abilita Invio</th>
		...
		foreach($arr_r as $key => $val){
			...
			$iscritti .= '<tr id="bg-riga'.$val['id'].'" '.($val['attivo']==0?'style="background-color:#FAE0DA"':'').'>
								...
								<td class="text-center"><img src="'.BASE_URL_SITO.'img/flags/mini/'.$val['lingua'].'.png" class="flag_ico"></td>
					...
					$iscritti .= '<br><b>Consenso trattamento dati</b>: '.($val['CheckConsensoPrivacy']==1?'<i class="fa fa-check-circle text-green"></i>':'<i class="fa fa-times-circle text-red"></i>');
					$iscritti .= '<br><b>Consenso invio materiale marketing</b>: '.($val['CheckConsensoMarketing']==1?'<i class="fa fa-check-circle text-green" id="marketing'.$val['id'].'" style="cursor:pointer" data-id="0"></i><span id="new_marketing_green'.$val['id'].'"></span>':'<i class="fa fa-times-circle text-red" id="marketing'.$val['id'].'" style="cursor:pointer" data-id="1"></i><span id="new_marketing_red'.$val['id'].'"></span>');    
					$iscritti .= '<br><b>Consenso profilazione</b>: '.($val['CheckConsensoProfilazione']==1?'<i class="fa fa-check-circle text-green" id="profilazione'.$val['id'].'" style="cursor:pointer" data-id="0"></i><span id="new_profilazione_green'.$val['id'].'"></span>':'<i class="fa fa-times-circle text-red" id="profilazione'.$val['id'].'" style="cursor:pointer" data-id="1"></i><span id="new_profilazione_red'.$val['id'].'"></span>');
					$iscritti .= '<br><b>Note Consensi</b>:<br><textarea rows="2" cols="50" name="NoteConsensi" id="noteconsensi'.$val['id'].'">'.$val['NoteConsensi'].'</textarea>';                  
									...
									<script type="text/javascript">
															$(document).ready(function(){
																	...
																	if (window.confirm("ATTENZIONE: Sicuro di modifcare il consenso?")){
																		...
																		$.ajax({
																				url: "'.BASE_URL_SITO.'/ajax/change_consenso_marketing_newsletter.php",
																				type: "POST",
																				...
																				$("#log'.$val['id'].'").remove();
																					if(valore_marketing==1){
																						$("#new_marketing_red'.$val['id'].'").html("<i class=\"fa fa-check-circle text-green\" id=\"marketing'.$val['id'].'\" style=\"cursor:pointer\" data-id=\"0\"></i>");                                                       
																					}else{
																						$("#new_marketing_green'.$val['id'].'").html("<i class=\"fa fa-times-circle text-red\" id=\"marketing'.$val['id'].'\" style=\"cursor:pointer\" data-id=\"1\"></i>"); 
																					}
																				}
																			});                                                               
																			return false;
																	...
																	$("#profilazione'.$val['id'].'").click(function(){
																		if (window.confirm("ATTENZIONE: Sicuro di modifcare il consenso?")){
																			...
																			$.ajax({
																				url: "/ajax/change_consenso_profilazione_newsletter_gdpr.php",
																				type: "POST",
																				...
																				success: function(data) { 
																					...
																					if(valore_profilazione==1){
																						$("#new_profilazione_red'.$val['id'].'").html("<i class=\"fa fa-check-circle text-green\" id=\"profilazione'.$val['id'].'\" style=\"cursor:pointer\" data-id=\"0\"></i>");                                                       
																					}else{
																						$("#new_profilazione_green'.$val['id'].'").html("<i class=\"fa fa-times-circle text-red\" id=\"profilazione'.$val['id'].'\" style=\"cursor:pointer\" data-id=\"1\"></i>"); 
																					}
																				}
																			});                                                               
																			return false;
																		...
																		$("#noteconsensi'.$val['id'].'").keyup(function(){
																			clearTimeout(typingTimer);
																			if ($("#noteconsensi'.$val['id'].'").val()) {
																				typingTimer = setTimeout(doneTyping(id,$("#noteconsensi'.$val['id'].'").val()), doneTypingInterval);
																			}
																		});
																		
																		//user is "finished typing," do something
																		function doneTyping (id,testo) {
																			$.ajax({
																				url: "/ajax/save_noteconsensi_gdpr.php",
																				type: "POST",
																				data: "id="+id+"&valore_noteconsensi="+testo,
																				dataType: "html",
																				success: function(data) { 
																					$("#noteconsensi'.$val['id'].'").val(data);
																				}
																			});                                                               
																			return false;
																		}
								...
								<td class="text-center">
									...
									$(document).ready(function () {
										$(\'#attivo'.$val['id'].'\').on("change", function(){
											...
											$.ajax({        
												type: "POST",         
												url: "'.BASE_URL_SITO.'ajax/save_user_newsletter_preference.php",        
		...
		$iscritti .= '<br>
					<div class="alert alert-profila alert-default-profila alert-dismissable text-center text-red">
