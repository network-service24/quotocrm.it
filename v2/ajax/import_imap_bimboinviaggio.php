...
$row     = mysqli_fetch_assoc($sq);
    ...
    if($user !='' && $pass !=''){ 
        ...
        $q = "SELECT * FROM hospitality_data_import_ottavo_portale WHERE idsito = ".$idsito." ORDER BY Id DESC";
        ...
        if(strstr($server,'gmail')){
            ...
            // grab emails
            $emails = imap_search($inbox,'FROM "no-reply@bimboinviaggio.com"" SINCE "'.$data_import.'"');
                    ...
                    foreach($emails as $key => $mail) {
                        ...
                        $maildata = $data_anno.'-'.$mesi[$data_mese].'-'.$data_giorno.' '.$ora_mail;
                            ...
                            if($maildata > $last_data_import){ 
                                ...
                                if(strstr($headerInfo->subject,'bimboinviaggio.com')){ 
                                    ...
                                    $arr_field['BV_P']      = array('Codice' => 'Codice email');                

                                    $array_contenuti_email[$tipo_modulo][] =array('BODY' => $righe); 

                                }// fine provenineza da bimboinviaggio.com
            ...
            if(!empty($array_contenuti_email)){
              ...
              foreach($array_contenuti_email as $key => $value){
                    ...
                    if($key != ''){
                     ...
                     foreach($value as $k => $val){
                        ...
                        $Camere             = '';
                                            ...
                                            //
                                            $FontePrenotazione   =   'bimboinviaggio.com';
                                            $DataRichiesta       =   date('Y-m-d');
                                            $TipoRichiesta       =   'Preventivo';

                                            $cliente_            =   explode("    ",trim($risultato[1]));
                                            $Nome                =   $cliente_[1];                                      
                                            $Cognome             =   $risultato[2]; 
                                            $Email               =   $risultato[3]; 
                                            $Cellulare           =   $risultato[4];

                                            $Data__              =   explode("    - ",$risultato[10]); 
                                            $DataArrivo_         =   explode(".", $Data__[0]);            
                                            $DataArrivo          =   trim($DataArrivo_[2]).'-'.trim($DataArrivo_[1]).'-'.trim($DataArrivo_[0]);

                                            $DataPartenza_       =   explode(".",$Data__[1]);             
                                            $DataPartenza        =   trim($DataPartenza_[2]).'-'.trim($DataPartenza_[1]).'-'.trim($DataPartenza_[0]);
                                                                                   
                                            $NumeroAdulti        =   $risultato[11];
                                            $NumeroBambini       =   $risultato[12];

                                            $EtaBambini          =   $risultato[13];
                ...
                $syncro = "INSERT INTO hospitality_data_import_ottavo_portale(idsito,data) VALUES('".$idsito."','".date('Y-m-d H:i:s')."')";
                    ...
                    foreach($emails as $key => $mail) {
                        ...
                        $maildata = $data_anno.'-'.$mesi[$data_mese].'-'.$data_giorno.' '.$ora_mail;
                            ...
                            if($maildata > $last_data_import){ 
                                ...
                                if(strstr($headerInfo->from,'bimboinviaggio.com')){ 
