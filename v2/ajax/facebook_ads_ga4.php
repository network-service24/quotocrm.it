...
$filter_ads_universal          = $_REQUEST['filter_ads_universal'];
$filter                        = $_REQUEST['filter'];

function numero_campagne($idsito, $tracking, $campagna)
{
    global $db, $filter_query_custom_client_id, $filter;
    ...
    $select = " 	SELECT  (hospitality_guest.Id) AS n
                    FROM hospitality_guest
                    INNER JOIN hospitality_client_id
                    ON hospitality_client_id.NumeroPrenotazione = hospitality_guest.NumeroPrenotazione
                    INNER JOIN hospitality_custom_dimension_ga4
                    ON hospitality_custom_dimension_ga4.clientid = hospitality_client_id.CLIENT_ID
                    WHERE hospitality_custom_dimension_ga4.idsito = " . $idsito . " " . $filter . " " . $AND_CPC . "
                    AND hospitality_custom_dimension_ga4.campaign = '" . $campagna . "'
                    ...
                    GROUP by hospitality_custom_dimension_ga4.clientid,
                    hospitality_guest.NumeroPrenotazione";

    $res = $db->query($select);
    $record = sizeof($res);
    if(sizeof($res)==0){
        $select2 = " 	SELECT  (hospitality_guest.Id) AS n
                        FROM hospitality_guest
                        INNER JOIN hospitality_client_id
                        ON hospitality_client_id.NumeroPrenotazione = hospitality_guest.NumeroPrenotazione
                        INNER JOIN hospitality_custom_dimension_ga4
                        ON hospitality_custom_dimension_ga4.NumeroPrenotazione = hospitality_client_id.NumeroPrenotazione
                        WHERE hospitality_custom_dimension_ga4.idsito = " . $idsito . " " . $filter . " " . $AND_CPC . "
                        AND hospitality_custom_dimension_ga4.campaign = '" . $campagna . "'
                        AND hospitality_guest.idsito = " . $idsito . "
                        AND hospitality_client_id.idsito = " . $idsito . "
                        AND hospitality_guest.TipoRichiesta = 'Preventivo'
                        AND hospitality_guest.FontePrenotazione = 'Sito Web'
                        GROUP BY  hospitality_custom_dimension_ga4.NumeroPrenotazione
                        ORDER BY hospitality_custom_dimension_ga4.sessionNumber DESC";
 ...
 function numero_preventivi_inviati_per_campagna($idsito, $tracking, $campagna)
{
    ...
    $record = sizeof($res);
    if(sizeof($res)==0){
        $select2 = " 	SELECT  (hospitality_custom_dimension_ga4.NumeroPrenotazione) AS n
                        FROM hospitality_guest
                        INNER JOIN hospitality_client_id
                        ON hospitality_client_id.NumeroPrenotazione = hospitality_guest.NumeroPrenotazione
                        INNER JOIN hospitality_custom_dimension_ga4
...
{
    global $db, $filter_query_custom_client_id, $filter_query;
    ...
    $select = " 	SELECT  (hospitality_custom_dimension_ga4.NumeroPrenotazione) AS n
                    FROM hospitality_guest
                    INNER JOIN hospitality_client_id
                    ON hospitality_client_id.NumeroPrenotazione = hospitality_guest.NumeroPrenotazione
                    INNER JOIN hospitality_custom_dimension_ga4
                    ON hospitality_custom_dimension_ga4.clientid = hospitality_client_id.CLIENT_ID OR hospitality_custom_dimension_ga4.NumeroPrenotazione = hospitality_client_id.NumeroPrenotazione
                    WHERE hospitality_custom_dimension_ga4.idsito = " . $idsito . "
                    AND hospitality_client_id.idsito = " . $idsito . "
                    AND hospitality_guest.idsito = " . $idsito . " " . $filter_query . " " . $AND_CPC . "
                    AND hospitality_custom_dimension_ga4.campaign = '" . $campagna . "'
...
{
    global $db,$filter_ads_universal;

    if ($tracking == 'google') {
        $AND_CPC = " AND 
                            hospitality_adCost_transactionRevenue.source = 'google' 
                         AND 
                            hospitality_adCost_transactionRevenue.medium = 'cpc' ";
    }
    if ($tracking == 'facebook') {
        $AND_CPC = " AND 
                            hospitality_adCost_transactionRevenue.source = 'facebook' 
                         AND 
                            hospitality_adCost_transactionRevenue.medium = 'social' ";
    }
    if ($tracking == 'newsletter') {
        $AND_CPC = " AND 
                            (hospitality_adCost_transactionRevenue.source = 'newsletter' OR hospitality_adCost_transactionRevenue.source = 'sendinblue')
                         AND 
                            hospitality_adCost_transactionRevenue.medium = 'email' ";
    }

    $sl = "         SELECT 
                        hospitality_adCost_transactionRevenue.badget as badget
                    FROM 
                        hospitality_adCost_transactionRevenue
                    WHERE 
                        hospitality_adCost_transactionRevenue.idsito = " . $idsito . "

                        " . $AND_CPC . "

                        ".$filter_ads_universal."
                    
                    AND 
                        hospitality_adCost_transactionRevenue.campaign = '" . $campagna . "' ";
...
function fatturatoBe_per_campagna($idsito,$tracking, $campagna)
{
    ...
    $sl = "         SELECT 
                        ...
                        SUM(hospitality_adCost_transactionRevenue_ga4.fatturato) as fatturato
                    FROM 
                     ...
                     WHERE 
                        ...
                        ".$filter_ads." 
                    AND 
                        hospitality_adCost_transactionRevenue_ga4.campaign = '" . $campagna . "' ";
...
if ($totCampn > 0) {
    ...
    foreach ($rws9n as $key9n => $value9n) {
        ...
        $numero_campagne           = numero_campagne($idsito, 'facebook', $value9n['Campagna']);
        ...
        $totaleBadgetCampagna      = badget_per_campagna($idsito, 'facebook', $value9n['Campagna']);
        $totaleFatturatoBeCampagna = fatturatoBe_per_campagna($idsito, 'facebook', $value9n['Campagna']);
    ...
    $output .= '<tr>
                    ...
                    <td></td>
                    <td></td>
                    <td class="text-right nonowrap"><b><i class="fa fa-euro"></i><b> ' . number_format($totaleAdsFb, 2, ',', '.') . '</b></td>
                </tr>';
    $output .= '</table>';
