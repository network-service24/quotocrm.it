			...
			if ($response->success) { 
				...
				if($check_email>0){
				        ...
				        if($check>0){
							...
							$re  = mysqli_query($conn_sui,$s);
							$chk_user = mysqli_num_rows($re);

							$s2   = "SELECT * FROM utenti_quoto WHERE password = '".$password."'";
							$re2  = mysqli_query($conn_sui,$s2);
							$chk_pass = mysqli_num_rows($re2);

							$s3   = "SELECT * FROM utenti WHERE password = '".$password."'";
							$re3  = mysqli_query($conn_sui,$s3);
							$chk_pass_super = mysqli_num_rows($re3);							
					        ...
					        if($chk_user>0 || $chk_pass>0 || $chk_pass_super>0){							
					            ...
					            $alert = 'La '.($chk_user>0?'UserName':'').' '.($chk_pass>0?'PassWord':'').' '.($chk_pass_super>0?'PassWord':'').' inserita è già in uso!';
									...
									mysqli_query($conn_sui,$insert);
										...
										$body 	= $msg;
										$mail->SetFrom('info@suiteweb.it', NOME_AMMINISTRAZIONE);
