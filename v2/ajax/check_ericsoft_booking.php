...
mysqli_set_charset($conn,'utf8');
        ...
        $start         = $start_tmp[2].'-'.$start_tmp[1].'-'.$start_tmp[0].'T00:00:00.000Z'; 
        $end_tmp       = explode('/',$_REQUEST['end']);
        $end           = $end_tmp[2].'-'.$end_tmp[1].'-'.$end_tmp[0].'T00:00:00.000Z'; 
...
//print_r($data_string);
                $ch = curl_init($urlHost.'GetSolutions');                                                                      
                ...
                foreach($risultato->Solutions as $key => $value){
                      ...
                      foreach($value->Resources as $ky => $val){
                        ...
                        foreach($val->Rates as $k => $vl){
                            ...
                            //$TotaleCamera  = $vl->FullTotalUserCurrency;
                            $TotaleCamera  = $vl->TotalSourceCurrency;


                         $riga_camere[$RoomCode.'_'.$TipoSogg][] = array('DISCOUNTED' => $IsDiscounted,'CAMERA' => $RoomCode,'SOGG' => $TipoSogg ,'TOTALE' => $TotaleCamera);  
                        }
                      }
                              ...
                              echo'
                                    <div class="row nascondi" id="riga_'.$p.'_'.$n_righe.'_'.$n.'">  
                                      ...
                                      <div class="col-md-'.(date('Y-m-d') > $data_quoto_v2?'3':'4').'" >
                                          ...
                                          <input type="hidden" name="TipoSoggiorno'.$p.'[]" id="TipoSoggiorno_'.$p.'_'.$n_righe.'"  value="'.$row['Id'].'"><i class="fa fa-angle-right"></i> <span class="text-orange">'.$row['TipoSoggiorno'].'</span>
                                          '.($record['DISCOUNTED']==1?'<br><div style="padding-left:20px"><small><i class="fa fa-trophy"></i> <b class="text-info"><em>Offerta</em></b></small></div>':'').'                                 
                                      </div>
                                        <div class="col-md-1">
                                          <input type="text" name="NumeroCamere'.$p.'[]" id="NumeroCamere_'.$p.'_'.$n_righe.'" class="form-control calc text-center" value="1"> 
                                        </div>
                                      '; 
                          if(date('Y-m-d') > $data_quoto_v2){ 
