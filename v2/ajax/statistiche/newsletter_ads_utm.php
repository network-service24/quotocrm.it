...
$totalePPCn = $fun->fatt_utm($idsito,$filter_query,'newsletter','email');

$rws9n      = $fun->campagne_utm($idsito,'newsletter','email');
...
if ($totCampn > 0) {
    ...
    foreach ($rws9n as $key9n => $value9n) {
        ...
        $numero_campagne   = $fun->CountRicevuteCampagnaAds($idsito,$value9n['Campagna'],'newsletter','email',$filter_query);
        $numero_prev_send  = $fun->CountInviateCampagnaAds($idsito,$value9n['Campagna'],'newsletter','email',$filter_query);
        $preno_chiuse      = $fun->CountConfermateCampagnaAds($idsito,$value9n['Campagna'],'newsletter','email',$filter_query);
        $totalePerCampagna = $fun->fatturato_per_campagna($idsito, $value9n['Campagna'],'newsletter','email',$filter_query);
