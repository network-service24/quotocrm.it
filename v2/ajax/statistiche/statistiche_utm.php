 ...
 */
$array_fatturatoS = $fun->fatt_sorgente_web($idsito,$filter_query);
...
if (!empty($array_fatturatoS) || !is_null($array_fatturatoS)) {
    ...
    foreach ($array_fatturatoS as $key => $val) {
            ...
            $ricevuteAds = $fun->CountRicevuteAds($idsito,$source, $val['medium']);

            $tot_ricevuteAds += $ricevuteAds;
            $inviateAds = $fun->CountInviateAds($idsito,$source, $val['medium']);

            $tot_inviateAds += $inviateAds;
            $confermateAds = $fun->CountConfermateAds($idsito,$source, $val['medium']);
...
$rws = $fun->fatt_sorgente_web_referrer($idsito,$filter_query);

foreach ($rws as $x => $v) {

    $riAdsNot  = $fun->CountRichiesteRicevuteNotAds($idsito,$v['referrer'], $DataRichiesta_dal, $DataRichiesta_al);
    $tot_ricevuteAdsNot += $riAdsNot;

    $inAdsNot  = $fun->CountRichiesteInviateNotAds($idsito,$v['referrer'], $DataRichiesta_dal, $DataRichiesta_al); 
    $tot_inviateAdsNot += $inAdsNot;

    $coAdsNot  = $fun->CountRichiesteConfermateNotAds($idsito,$v['referrer'],$DataRichiesta_dal, $DataRichiesta_al);
    $tot_confermateAdsNot += $coAdsNot;

    $fatAdsNot = $fun->fatturatoReferrer(1,$v['referrer']);
 ...
 */
    ...
    $confermateSitoWeb = $fun->CountRichiesteConfermate($idsito, 'Sito Web', $DataRichiesta_dal, $DataRichiesta_al);
 ...
 */
$array_fatturato  = $fun->fatt_fonti_prenotazione($idsito,$filter_query);
 ...
 * * ANNULLATE
 */
$array_fatturatoD = $fun->fatt_fonti_prenotazione_annullate($idsito,$filter_query); 
...
/**
 * * FATTURATO PER TARGET CLIENTE
 */
$array_fatturatoTARGET = $fun->fatt_target($idsito,$filter_query);
...
$fatturatoTotaleQuotoT = $fun->fatturatoTotale(1);
 ...
 *
 * */
$array_fatturatoOperatore = $fun->fatt_operatore($idsito,$filter_query);
...
$fatturatoTotaleQuotoOP = $fun->fatturatoTotale(1);
 ...
 */
$array_fatturatoTemplate = $fun->fatt_template($idsito,$filter_query);
...
$fatturatoTotaleQuotoTP = $fun->fatturatoTotale(1);
