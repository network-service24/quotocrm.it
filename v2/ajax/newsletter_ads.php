...
function numero_campagne_da_landing($idsito, $tracking, $campagna, $clientid)
{
    ...
    $q = "  SELECT COUNT(hospitality_custom_dimension.id) as n_preventivi  
             ...
             WHERE hospitality_custom_dimension.idsito = $idsito     
               ...
               AND (hospitality_custom_dimension.urlpath !='/?res=sent' AND hospitality_custom_dimension.urlpath != '/' AND hospitality_custom_dimension.urlpath  != '/index.php' AND hospitality_custom_dimension.urlpath  NOT LIKE  '%.php?res=sent%' AND hospitality_custom_dimension.urlpath NOT LIKE '%.php%') 
    ";
...
$sqln = "SELECT distinct(hospitality_client_id.NumeroPrenotazione)
            ...
            WHERE hospitality_client_id.idsito = " . $idsito . "
            AND hospitality_custom_dimension.idsito = " . $idsito . "
            AND (hospitality_custom_dimension.source = 'newsletter' OR hospitality_custom_dimension.source = 'sendinblue' OR hospitality_custom_dimension.source = 'codice-promo')
            AND hospitality_custom_dimension.medium = 'email'
            AND hospitality_custom_dimension.urlpath LIKE '%res=sent%'";
$rwsn = $db->query($sqln);
...
if ($totCampn > 0) {
    ...
    foreach ($rws9n as $key9n => $value9n) {
        ...
        $numero_landing = numero_campagne_da_landing($idsito, 'newsletter', $value9n['Campagna'], $ClientId);
        $numero_prev_send = numero_preventivi_inviati_per_campagna($idsito, 'newsletter', $value9n['Campagna'], $ClientId);
        $numero_sito = numero_campagne_da_sito($idsito, 'newsletter', $value9n['Campagna'], $ClientId);
    ...
    $output .= '<tr>
                    <td colspan="6" class="text-right"><b>TOTALE</b></td>
                    <td class="text-right wrap"><i class="fa fa-euro"></i><b> ' . number_format($totalePPCn, 2, ',', '.') . '</b></td>
                </tr>';
    $output .= '</table>';
}
$numero_landing = '';
