...
$dbSuiteweb = new MysqliDb(DB_SUITEWEB_HOST, DB_SUITEWEB_USER, DB_SUITEWEB_PASSWORD, DB_SUITEWEB_NAME);
...
/**
 * Attenzione alla nomenclatura delle campagne nelle due tabelle che non corrisponde
 * Esempio:
 *
 * TABELLA hospitality_custom_dimension
 * Nomi di campagne
 * DE | RR | brand
 * EN | RR | brand
 * IT | RD | RMKT
 ...
 * IT | RR | mesi
 *
 * TABELLA hospitality_tracking_ads
 * Nomi di campagne
 * brand
 * RMKT
 * bormio
...
switch ($type) {
    case 'richieste':
        $sql1 = function ($allColumns = true) use ($idSito, $startDate, $endDate, $sorgente, $media, $campaign) {
            return "SELECT g.Id" .
                ($allColumns ? ", g.NumeroPrenotazione, DATE_FORMAT(g.DataRichiesta, '%d/%m/%Y') as 'DataRichiesta', GROUP_CONCAT(DISTINCT(p.PrezzoP)) as 'PrezzoP', g.FontePrenotazione, GROUP_CONCAT(DISTINCT(cd.campaign)) as Campagna, GROUP_CONCAT(DISTINCT(cid.CLIENT_ID)) as ClientId" : '') .
        ...
        $sql2 = function ($allColumns = true) use ($idSito, $startDate, $endDate, $sorgente, $media, $campaign) {
            return "SELECT g.Id" .
                ($allColumns ? ", g.NumeroPrenotazione, DATE_FORMAT(g.DataRichiesta, '%d/%m/%Y') as 'DataRichiesta', GROUP_CONCAT(DISTINCT(p.PrezzoP)) as 'PrezzoP', g.FontePrenotazione, GROUP_CONCAT(DISTINCT(cd.campaign)) as Campagna, GROUP_CONCAT(DISTINCT(cid.CLIENT_ID)) as ClientId" : '') .
                " FROM hospitality_guest g 
             LEFT JOIN hospitality_proposte p ON g.ID = p.id_richiesta
        ...
        $sql = $sql1() . " 
                ...
                SELECT g.Id,
                       ...
                       g.FontePrenotazione, 
                       GROUP_CONCAT(DISTINCT(ads.Campagna)) as Campagna,
            ...
            INNER JOIN hospitality_proposte p ON g.Id = p.id_richiesta                                                                                   
                 WHERE ads.idsito = $idSito
                   AND ads.Tracking = '$sorgente'
                   AND ads.Campagna = '$campaign'
                   AND g.Id NOT IN (" . $sql1(false) . ")
                   AND g.Id NOT IN (" . $sql2(false) . ")
              GROUP BY g.Id";
        break;
    case 'inviati':
        $sql1 = function ($allColumns = true) use ($idSito, $startDate, $endDate, $sorgente, $media, $campaign) {
            return "SELECT g.Id" . ($allColumns ? ", g.NumeroPrenotazione, DATE_FORMAT(g.DataRichiesta, '%d/%m/%Y') as 'DataRichiesta', GROUP_CONCAT(DISTINCT(p.PrezzoP)) as 'PrezzoP', g.FontePrenotazione, GROUP_CONCAT(DISTINCT(cd.campaign)) as Campagna, GROUP_CONCAT(DISTINCT(cid.CLIENT_ID)) as ClientId" : '') . "                       
                  FROM hospitality_guest g 
            INNER JOIN hospitality_proposte p ON g.ID = p.id_richiesta
            INNER JOIN hospitality_client_id cid ON cid.NumeroPrenotazione = g.NumeroPrenotazione 
                                                        AND cid.idsito = $idSito 
