...
$qry     = "SELECT * FROM hospitality_imap_email  WHERE Portale = 'allinclusivehotels.it' AND idsito = ".$idsito."  AND Abilitato = 1 ORDER BY Id DESC";
...
$row     = mysqli_fetch_assoc($sq);
    ...
    if($user !='' && $pass !=''){ 
        ...
        $q = "SELECT * FROM hospitality_data_import_decimo_portale WHERE idsito = ".$idsito." ORDER BY Id DESC";
        ...
        if(strstr($server,'gmail')){
            ...
            // grab emails
            $emails = imap_search($inbox,'SUBJECT "ALL INCLUSIVE HOTELS" SINCE "'.$data_import.'"');
                    ...
                    foreach($emails as $key => $mail) {
                        ...
                        $maildata = $data_anno.'-'.$mesi[$data_mese].'-'.$data_giorno.' '.$ora_mail;
                            ...
                            if($maildata > $last_data_import){ 
                                ...
                                if(strstr($headerInfo->subject,'ALL INCLUSIVE HOTELS')){ 
                                    ...
                                    $arr_field['AIH_P']      = array('Codice' => 'Codice email');                

                                    $array_contenuti_email[$tipo_modulo][] =array('BODY' => $righe); 

                                }// fine provenineza da allinclusivehotels.it
            ...
            if(!empty($array_contenuti_email)){
              ...
              foreach($array_contenuti_email as $key => $value){
                    ...
                    if($key != ''){
                     ...
                     foreach($value as $k => $val){
                        ...
                        $Camere             = '';
                                            ...
                                            //
                                            $FontePrenotazione  =   'allinclusivehotels.it';
                ...
                $syncro = "INSERT INTO hospitality_data_import_decimo_portale(idsito,data) VALUES('".$idsito."','".date('Y-m-d H:i:s')."')";
            ...
            $search = 'SUBJECT "ALL INCLUSIVE HOTELS"';
