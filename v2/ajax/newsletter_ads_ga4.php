...
function fatturato_per_campagna($idsito, $campagna)
{
    ...
    $sl = "SELECT 
                ...
                ( SELECT hospitality_proposte.PrezzoP FROM hospitality_proposte  WHERE hospitality_proposte.id_richiesta = hospitality_guest.Id LIMIT 1) as 'fatturato'
                    ...
                    AND 
                        hospitality_custom_dimension_ga4.source = 'newsletter'
                    AND 
                        hospitality_custom_dimension_ga4.medium = 'email'
                    AND 
...
$sqln = "SELECT distinct(hospitality_client_id.NumeroPrenotazione)
            ...
            AND hospitality_custom_dimension_ga4.source = 'newsletter'
            AND hospitality_custom_dimension_ga4.medium = 'email'";
$rwsn = $db->query($sqln);
...
if ($totCampn > 0) {
    ...
    foreach ($rws9n as $key9n => $value9n) {
        ...
        $numero_campagne    = numero_campagne($idsito, 'newsletter', $value9n['Campagna']);
        $numero_prev_send  = numero_preventivi_inviati_per_campagna($idsito, 'newsletter', $value9n['Campagna']);
        $preno_chiuse      = n_campagne_chiuse_new($idsito, 'newsletter', $value9n['Campagna']);
        $totaleFatturatoBeCampagna = fatturatoBe_per_campagna($idsito, 'newsletter', $value9n['Campagna']);
