...
function tot_fatturato($n_format=null){
  ...
  function prenotazioni_per_camera(){
      global $db,$idsito,$p_data,$s_data,$p_anno,$s_anno,$date,$dataChiusoDa,$dataChiusoA;
      ...
      if(($tot)>0){
          ...
          while ($vl = mysqli_fetch_assoc($r)) {
              ...
              $select ='SELECT SUM(hospitality_richiesta.Prezzo) as fatturato, COUNT(hospitality_richiesta.TipoCamere) as nCamere
                          ...
                          AND hospitality_guest.Chiuso = 1
                          AND hospitality_guest.Disdetta = 0
                          AND '.($date == ''?' hospitality_guest.DataArrivo >= "'.date('Y').'-01-01" AND hospitality_guest.DataPartenza <= "'.date('Y').'-12-31"': ' hospitality_guest.DataArrivo >= "'.$$dataChiusoDa.'" AND hospitality_guest.DataPartenza <= "'.$dataChiusoA.'"').'
                          AND hospitality_guest.TipoRichiesta = "Conferma" ';
