...
if (!empty($array_fatturatoS) || !is_null($array_fatturatoS)) {
    ...
    foreach ($array_fatturatoS as $key => $val) {
            ...
            $tot_fatturatoBE = fatturatoBePerFonte($idsito,$source, $val['medium'],$DataRichiesta_dal, $DataRichiesta_al);
    ...
    $fattSitoweb = fatturatoTotaleSitoWeb(1);

    if ($totaleS < $fattSitoweb) {

        $ricevuteSito = CountRichiesteRicevute($idsito, 'Sito Web', $DataRichiesta_dal, $DataRichiesta_al);

        $inviateSito = CountRichiesteInviate($idsito, 'Sito Web', $DataRichiesta_dal, $DataRichiesta_al);

        $confermateSito = CountRichiesteConfermate($idsito, 'Sito Web', $DataRichiesta_dal, $DataRichiesta_al);

        $totaleDiff = ($fattSitoweb - $totaleS);

        $ricevuteAdsNot = CountRichiesteRicevuteAds($idsito, '', 'referral', $DataRichiesta_dal, $DataRichiesta_al);

        $ricevuteAdsNot = ($ricevuteSito - $tot_ricevuteAds);

        $inviateAdsNot = CountRichiesteInviateAds($idsito, '', 'referral', $DataRichiesta_dal, $DataRichiesta_al);

        $inviateAdsNot = ($inviateSito - $tot_inviateAds);

        $confermateAdsNot = CountRichiesteConfermateAds($idsito, '', 'referral', $DataRichiesta_dal, $DataRichiesta_al);
...
if ($tot > 0) {
    ...
    $output = '<table class="display compact dataTable table table-striped table-hover table-bordered table-sm f-13"  style="width:100%">
                    <tr>
                        <th class="wrap th">Metodo di Contatto</th>
                        <th class="wrap th text-center">Richieste ricevute</th>
                        <th class="wrap th text-center">Preventivi inviati</th>
                        <th class="wrap th text-center">Prenotazioni confermate</th>
                        <th class="wrap th text-center">Conversione %</th>
                        <th class="wrap th text-center">Fatturato</th>
                    </tr>';
    $ricevute = '';
    $inviate = '';
    $confermate = '';
    foreach ($array_fatturato as $k => $v) {
        ...
        $_SESSION['totaleSitoWeb'] = $v;
         ...
         $ricevute = CountRichiesteRicevute($idsito, $label, $DataRichiesta_dal, $DataRichiesta_al);
        $tot_ricevute += $ricevute;
        $inviate = CountRichiesteInviate($idsito, $label, $DataRichiesta_dal, $DataRichiesta_al);
        $tot_inviate += $inviate;
        $confermate = CountRichiesteConfermate($idsito, $label, $DataRichiesta_dal, $DataRichiesta_al);
    ...
    if ($fatturatoTotaleQuoto < $fatturatoTotaleQuotoTDashboard) {
        ...
        $output .= '<tr>
                        <td class="text-left wrap"><label style="font-weight: bold;"><i class="fa fa-info-circle"  data-toogle="tooltip" title="Tutto ciò che non ha una fonte di provenienza precisata, oppure una fonte eliminata, sostituita, ecc.!"></i> <b>Nessuna Sorgente Specifica</b></label></td>
                        <td class="text-center"></td>
                        <td class="text-center"></td>
                        <td class="text-center"></td>
                        <td class="text-center"></td>
                        <td class="text-right wrap"><i class="fa fa-euro"></i> ' . number_format(($fatturatoTotaleQuotoTDashboard-$fatturatoTotaleQuoto), 2, ',', '.') . '</td>
                    </tr>';
    ...
    $outputA = '<table class="display compact dataTable table table-striped table-hover table-bordered table-sm f-13"  style="width:100%">
                        <tr>
                            <th class="wrap th">Metodo di Contatto</th>
                            <th class="wrap th text-center">Prenotazioni annullate</th>
                            <th class="wrap th text-center">Importo</th>
                        </tr>';
        ...
        if (!empty($array_fatturatoD) || !is_null($array_fatturatoD)) {
            ...
            foreach ($array_fatturatoD as $kD => $vD) {
                ...
                if ($vD != "" || $vD != 0) {
                    ...
                    $annullate = CountRichiesteAnnullate($idsito, $labelD, $DataRichiesta_dal, $DataRichiesta_al);
...
if ($totTARGET > 0) {
    ...
    if (isset($array_fatturatoTARGET)) {
        ...
        $outputTA = '<table class="display compact dataTable table table-striped table-hover table-bordered table-sm f-13"  style="width:100%">
                        <tr>
                            <th class="wrap th" style="width:80%">Target Clienti</th>
                            <th class="wrap th text-center" style="width:20%">Fatturato</th>
                        </tr>';
...
if ($totOperatore > 0) {
    ...
    $outputOP = '<table class="display compact dataTable table table-striped table-hover table-bordered table-sm f-13"  style="width:100%">
                        <tr>
                            <th class="wrap th" style="width:80%">Operatore</th>
                            <th class="wrap th text-center" style="width:20%">Fatturato</th>
                        </tr>';
    $p_OP = '';
...
if ($totTemplate > 0) {
    ...
    $outputTP = '<table class="display compact dataTable table table-striped table-hover table-bordered table-sm f-13"  style="width:100%">
                    <tr>
                        <th class="wrap th" style="width:80%">Template Landing Page</th>
                        <th class="wrap th text-center" style="width:20%">Fatturato</th>
                    </tr>';
...
if (array_values($array_fatturatoD) > 0) {
  echo'<div class="box no-radius">
        <div class="box-body">
            <div class="row">
                <div class="col-md-8"><h4> Conferme Annullate dall\'operatore</h4></div>
...
if (isset($array_fatturatoTARGET)) {
    echo'<div class="box no-radius">
            <div class="box-body">
                <div class="row">
                    <div class="col-md-8"><h4> Fatturato per Target cliente</h4></div>
...
if (array_values($array_fatturatoOperatore) > 0){
    echo'<div class="box no-radius">
            <div class="box-body">
            <div class="row">
                    <div class="col-md-8"><h4> Fatturato per Operatore</h4></div>
...
if(array_values($array_fatturatoTemplate) > 0){
    echo' <div class="box no-radius">
            <div class="box-body">
            <div class="row">
                    <div class="col-md-8"><h4> Fatturato per Template Landing Page</h4></div>
