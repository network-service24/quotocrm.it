...
$row     = mysqli_fetch_assoc($sq);
    ...
    if($user !='' && $pass !=''){ 
        ...
        $q = "SELECT * FROM hospitality_data_import_settimo_portale WHERE idsito = ".$idsito." ORDER BY Id DESC";
        ...
        if(strstr($server,'gmail')){
            ...
            // grab emails
            $emails = imap_search($inbox,'SUBJECT "ITALY BIKE HOTELS" SINCE "'.$data_import.'"');
                    ...
                    foreach($emails as $key => $mail) {
                        ...
                        $maildata = $data_anno.'-'.$mesi[$data_mese].'-'.$data_giorno.' '.$ora_mail;
                            ...
                            if($maildata > $last_data_import){ 
                                ...
                                if(strstr($headerInfo->subject,'ITALY BIKE HOTELS')){ 
                                    ...
                                    $arr_field['IB_P']      = array('Codice' => 'Codice email');                
            ...
            if(!empty($array_contenuti_email)){
              ...
              foreach($array_contenuti_email as $key => $value){
                    ...
                    if($key != ''){
                     ...
                     foreach($value as $k => $val){
                        ...
                        $Camere             = '';
                                            ...
                                            //
                                            $FontePrenotazione  =   'italybikehotels.it';
                ...
                $syncro = "INSERT INTO hospitality_data_import_settimo_portale(idsito,data) VALUES('".$idsito."','".date('Y-m-d H:i:s')."')";
            ...
            $inbox = imap_open('{'.$server.'/notls}', $user, $pass) or die('Cannot connect to NWSCloud: ' . imap_last_error());

            //$search = 'SUBJECT "ITALY BIKE HOTELS"';
            $search = 'ALL';
                    ...
                    if($key != ''){
                     ...
                     foreach($value as $k => $val){
                        ...
                        $Camere             = '';
                                            ...
                                            $DataRichiesta      =   date('Y-m-d');
                                            $TipoRichiesta      =   'Preventivo';
                                            $cliente_           =   explode(" ", $risultato[0]);
                                            $Nome               =   $cliente_[0];
                                            $Cognome            =   $cliente_[1];
                                            $Email              =   $risultato[1];

                                            if(strlen($risultato[2])>3 && ($risultato[2]!='')){
                                                $Cellulare          =   $risultato[2];
                                                $Lingua             =   strtolower($risultato[3]);
                                                $tipoCiclo          =   $risultato[4];
                                                $DataArrivo_         =   explode("/",$risultato[8]);            
                                                $DataPartenza_       =   explode("/",$risultato[9]);
                                                $DataArrivo          =   $DataArrivo_[2].'-'.$DataArrivo_[1].'-'.$DataArrivo_[0];
                                                $DataPartenza        =   $DataPartenza_[2].'-'.$DataPartenza_[1].'-'.$DataPartenza_[0];
                                                $NumeroAdulti        =   $risultato[10];
                                                if($risultato[11]!=''){
                                                    $NumeroBambini       =   $risultato[11];
                                                }
                                                $Note                   = $risultato[12]."\r\n".($$risultato[13]!=''?'Note: '.$risultato[13]:'');
                                            }else{
                                                $Lingua              =   strtolower($risultato[2]);
                                                $tipoCiclo           =   $risultato[3];
                                                $DataArrivo_         =   explode("/",$risultato[6]);            
                                                $DataPartenza_       =   explode("/",$risultato[7]);
                                                $DataArrivo          =   $DataArrivo_[2].'-'.$DataArrivo_[1].'-'.$DataArrivo_[0];
                                                $DataPartenza        =   $DataPartenza_[2].'-'.$DataPartenza_[1].'-'.$DataPartenza_[0];
                                                $NumeroAdulti        =   $risultato[8];

                                                if(strlen($risultato[9])>3 && ($risultato[9]!='')){
                                                    $text              =   $risultato[9];
                                                }else{
                                                    $NumeroBambini     =   $risultato[9];
                                                }
                                                $Note                   = $text."\r\n".($$risultato[10]!=''?'Note: '.$risultato[10]:'');
                                            }
